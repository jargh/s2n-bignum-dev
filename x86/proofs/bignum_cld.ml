...
let bignum_cld_mc = define_assert_from_elf "bignum_cld_mc" "x86/generic/bignum_cld.o"
[
  0xf3; 0x0f; 0x1e; 0xfa;  (* ENDBR64 *)
  0x48; 0x31; 0xc0;        (* XOR (% rax) (% rax) *)
  0x48; 0x85; 0xff;        (* TEST (% rdi) (% rdi) *)
  0x74; 0x1c;              (* JE (Imm8 (word 28)) *)
...
let bignum_cld_tmc = define_trimmed "bignum_cld_tmc" bignum_cld_mc;;

let BIGNUM_CLD_EXEC = X86_MK_CORE_EXEC_RULE bignum_cld_tmc;;
...
let BIGNUM_CLD_CORRECT = prove
 (`!k a x pc.
        ensures x86
         (\s. bytes_loaded s (word pc) (BUTLAST bignum_cld_tmc) /\
              read RIP s = word pc /\
              C_ARGUMENTS [k;a] s /\
              bignum_from_memory(a,val k) s = x)
         (\s'. read RIP s' = word (pc + 0x24) /\
  ...
  ASM_CASES_TAC `k = 0` THENL
   [UNDISCH_TAC `x < 2 EXP (64 * k)` THEN
    ...
    ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_CLD_EXEC (1--3) THEN
  ...
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
    ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_CLD_EXEC (1--4) THEN
    ...
    ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_CLD_EXEC [1] THEN
    ...
    FIRST_X_ASSUM(DISJ_CASES_THEN MP_TAC) THENL
     [DISCH_THEN SUBST_ALL_TAC THEN
      ...
      UNDISCH_TAC `read (memory :> bytes (a,8 * k)) s3 = x` THEN
      ASM_REWRITE_TAC[] THEN DISCH_THEN(SUBST1_TAC o SYM) THEN
      REWRITE_TAC[BITSIZE_0; SUB_0; WORD_NEG_0; WORD_ADD_0] THEN
    ...
    TRANS_TAC EQ_TRANS `word(k - i):int64` THEN CONJ_TAC THENL
     [REWRITE_TAC[WORD_RULE `word_add (word_neg x) y = word_sub y x`] THEN
  ...
  ASSUME_TAC THENL
   [EXPAND_TAC "x" THEN
    ...
    ALL_TAC] THEN
  X86_STEPS_TAC BIGNUM_CLD_EXEC (1--5) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
...
let BIGNUM_CLD_NOIBT_SUBROUTINE_CORRECT = prove
 (`!k a x pc stackpointer returnaddress.
        ensures x86
         (\s. bytes_loaded s (word pc) bignum_cld_tmc /\
              read RIP s = word pc /\
  ...
  X86_PROMOTE_RETURN_NOSTACK_TAC bignum_cld_tmc BIGNUM_CLD_CORRECT);;
 ...
 (`!k a x pc stackpointer returnaddress.
        ensures x86
         ...
         (\s'. read RIP s' = returnaddress /\
               ...
               C_RETURN s' = word((64 * val k - bitsize x) DIV 64))
         (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI)`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_CLD_NOIBT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let bignum_cld_windows_mc = define_from_elf
   "bignum_cld_windows_mc" "x86/generic/bignum_cld.obj";;

let bignum_cld_windows_tmc = define_trimmed "bignum_cld_windows_tmc" bignum_cld_windows_mc;;

let BIGNUM_CLD_NOIBT_WINDOWS_SUBROUTINE_CORRECT = prove
 (`!k a x pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,LENGTH bignum_cld_windows_tmc); (a,8 * val k)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_cld_windows_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_NOSTACK_TAC bignum_cld_windows_tmc bignum_cld_tmc
    BIGNUM_CLD_CORRECT);;

let BIGNUM_CLD_WINDOWS_SUBROUTINE_CORRECT = prove
 (`!k a x pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,LENGTH bignum_cld_windows_mc); (a,8 * val k)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_cld_windows_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [k;a] s /\
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_CLD_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;
...
let full_spec,public_vars = mk_safety_spec ~keep_maychanges:true
    (assoc "bignum_cld" subroutine_signatures)
    BIGNUM_CLD_CORRECT BIGNUM_CLD_EXEC;;

(* Containment of a word-indexed load a[m] (8 bytes) within [a, n * 8] when m < n. *)
let CLD_LOAD_CONTAINED = prove
 (`!(a:int64) (n:num) (m:num).
      m < n /\ n < 2 EXP 64
...
let BIGNUM_CLD_SAFE = prove
 (`exists f_events. forall e k a pc.
       ...
       (MAYCHANGE [RIP; RDI; RDX; RCX; RAX] ,, MAYCHANGE SOME_FLAGS ,, MAYCHANGE [events])`,
  ...
  REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS] THEN
  ASM_CASES_TAC `k = 0` THENL
   [ ASM_REWRITE_TAC[] THEN X86_SIM_TAC BIGNUM_CLD_EXEC (1--3) THEN DISCHARGE_SAFETY_PROPERTY_TAC; ALL_TAC ] THEN
  ASM_REWRITE_TAC[ASSUME `val(word k:int64) = k`] THEN
  ENSURES_EVENTS_WHILE_UP2_TAC `k:num` `pc + 0xb` `pc + 0x1e`
   `\i s. read RSI s = a /\ read RDI s = word k /\ read RDX s = word i` THEN
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [ X86_SIM_TAC BIGNUM_CLD_EXEC (1--4) THEN ASM_REWRITE_TAC[] THEN DISCHARGE_SAFETY_PROPERTY_TAC;
     ALL_TAC;
     ENSURES_INIT_TAC "s0" THEN STRIP_EXISTS_ASSUM_TAC THEN X86_STEPS_TAC BIGNUM_CLD_EXEC (1--2) THEN ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN DISCHARGE_SAFETY_PROPERTY_TAC ] THEN
  ...
  X86_STEPS_TAC BIGNUM_CLD_EXEC (1--6) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  ...
  SUBGOAL_THEN `(~(val(word_sub (word (i+1)) (word k):int64) = 0)) <=> i + 1 < k` SUBST1_TAC THENL
   [ REWRITE_TAC[VAL_WORD_SUB_EQ_0] THEN IMP_REWRITE_TAC[VAL_WORD_EQ; DIMINDEX_64] THEN SIMPLE_ARITH_TAC; ALL_TAC ] THEN
  ...
  GEN_REWRITE_TAC I [MEMACCESS_INBOUNDS_APPEND] THEN CONJ_TAC THENL
   [ REWRITE_TAC[memaccess_inbounds; ALL; EX; FST; SND] THEN
     MATCH_MP_TAC CLD_LOAD_CONTAINED THEN CONJ_TAC THEN SIMPLE_ARITH_TAC;
...
let BIGNUM_CLD_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e k a pc stackpointer returnaddress.
        ensures x86
            (\s. bytes_loaded s (word pc) bignum_cld_tmc /\
                 ...
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [k; a] s /\ read events s = e)
            (\s. read RIP s = returnaddress /\
                 read RSP s = word_add stackpointer (word 8) /\
            ...
            (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI)`,
  X86_PROMOTE_RETURN_NOSTACK_TAC bignum_cld_tmc BIGNUM_CLD_SAFE THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_CLD_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e k a pc stackpointer returnaddress.
        ensures x86
            (\s. bytes_loaded s (word pc) bignum_cld_mc /\
            ...
            (\s. read RIP s = returnaddress /\
                 ...
                 (exists e2. read events s = APPEND e2 e /\
                      ...
                      memaccess_inbounds e2 [a,val k * 8; stackpointer,8] []))
            (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI)`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_CLD_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_CLD_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e k a pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
        [word pc,LENGTH bignum_cld_windows_tmc; a,8 * val k]
        ==> ensures x86
            (\s. bytes_loaded s (word pc) bignum_cld_windows_tmc /\
                 read RIP s = word pc /\ read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [k; a] s /\ read events s = e)
            (\s. read RIP s = returnaddress /\
                 read RSP s = word_add stackpointer (word 8) /\
                 (exists e2. read events s = APPEND e2 e /\
                      e2 = f_events a k pc (word_sub stackpointer (word 16)) returnaddress /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             MAYCHANGE [memory :> bytes (word_sub stackpointer (word 16),16)])`,
  WINDOWS_X86_WRAP_NOSTACK_TAC bignum_cld_windows_tmc bignum_cld_tmc BIGNUM_CLD_SAFE THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_CLD_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e k a pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
        [word pc,LENGTH bignum_cld_windows_mc; a,8 * val k]
        ==> ensures x86
            ...
            (MAYCHANGE [RSP] ,,
             WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE [memory :> bytes (word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_CLD_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
