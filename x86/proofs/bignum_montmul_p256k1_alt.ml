...
let bignum_montmul_p256k1_alt_tmc = define_trimmed "bignum_montmul_p256k1_alt_tmc" bignum_montmul_p256k1_alt_mc;;

let BIGNUM_MONTMUL_P256K1_ALT_EXEC = X86_MK_CORE_EXEC_RULE bignum_montmul_p256k1_alt_tmc;;
...
let BIGNUM_MONTMUL_P256K1_ALT_CORRECT = time prove
 (`!z x y a b pc.
        ...
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST bignum_montmul_p256k1_alt_tmc) /\
...
let BIGNUM_MONTMUL_P256K1_ALT_NOIBT_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
        ...
        ALL (nonoverlapping (word_sub stackpointer (word 40),40))
            [(word pc,LENGTH bignum_montmul_p256k1_alt_tmc); (x,8 * 4); (y,8 * 4)] /\
        nonoverlapping (word pc,LENGTH bignum_montmul_p256k1_alt_tmc) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_montmul_p256k1_alt_tmc /\
                  read RIP s = word pc /\
  ...
  X86_PROMOTE_RETURN_STACK_TAC
   bignum_montmul_p256k1_alt_tmc BIGNUM_MONTMUL_P256K1_ALT_CORRECT
...
let BIGNUM_MONTMUL_P256K1_ALT_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 40),48) /\
        ALL (nonoverlapping (word_sub stackpointer (word 40),40))
            [(word pc,LENGTH bignum_montmul_p256k1_alt_mc); (x,8 * 4); (y,8 * 4)] /\
        nonoverlapping (word pc,LENGTH bignum_montmul_p256k1_alt_mc) (z,8 * 4)
        ==> ensures x86
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                         memory :> bytes(word_sub stackpointer (word 40),40)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MONTMUL_P256K1_ALT_NOIBT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let bignum_montmul_p256k1_alt_windows_mc = define_from_elf
   "bignum_montmul_p256k1_alt_windows_mc" "x86/secp256k1/bignum_montmul_p256k1_alt.obj";;

let bignum_montmul_p256k1_alt_windows_tmc = define_trimmed "bignum_montmul_p256k1_alt_windows_tmc" bignum_montmul_p256k1_alt_windows_mc;;

let BIGNUM_MONTMUL_P256K1_ALT_NOIBT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 56),64) /\
        ALL (nonoverlapping (word_sub stackpointer (word 56),56))
            [(word pc,LENGTH bignum_montmul_p256k1_alt_windows_tmc); (x,8 * 4); (y,8 * 4)] /\
        nonoverlapping (word pc,LENGTH bignum_montmul_p256k1_alt_windows_tmc) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_montmul_p256k1_alt_windows_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
   bignum_montmul_p256k1_alt_windows_tmc bignum_montmul_p256k1_alt_tmc
   BIGNUM_MONTMUL_P256K1_ALT_CORRECT `[RBX; R12; R13; R14; R15]` 40);;

let BIGNUM_MONTMUL_P256K1_ALT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 56),64) /\
        ALL (nonoverlapping (word_sub stackpointer (word 56),56))
            [(word pc,LENGTH bignum_montmul_p256k1_alt_windows_mc); (x,8 * 4); (y,8 * 4)] /\
        nonoverlapping (word pc,LENGTH bignum_montmul_p256k1_alt_windows_mc) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_montmul_p256k1_alt_windows_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [z; x; y] s /\
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                         memory :> bytes(word_sub stackpointer (word 56),56)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MONTMUL_P256K1_ALT_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;
...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "bignum_montmul_p256k1_alt" subroutine_signatures)
    BIGNUM_MONTMUL_P256K1_ALT_CORRECT
    BIGNUM_MONTMUL_P256K1_ALT_EXEC;;

let BIGNUM_MONTMUL_P256K1_ALT_SAFE = time prove
 (`exists f_events.
       forall e z x y pc.
           nonoverlapping (word pc,482) (z,8 * 4)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc)
                    (BUTLAST bignum_montmul_p256k1_alt_tmc) /\
                    read RIP s = word (pc + 9) /\
                    C_ARGUMENTS [z; x; y] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 472) /\
                    (exists e2.
  ...
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars BIGNUM_MONTMUL_P256K1_ALT_EXEC);;

let BIGNUM_MONTMUL_P256K1_ALT_NOIBT_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x y pc stackpointer returnaddress.
        ...
        ALL (nonoverlapping (word_sub stackpointer (word 40),40))
        [word pc,LENGTH bignum_montmul_p256k1_alt_tmc; x,8 * 4; y,8 * 4] /\
        nonoverlapping (word pc,LENGTH bignum_montmul_p256k1_alt_tmc)
        (z,8 * 4)
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_montmul_p256k1_alt_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x; y] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 4);
              memory :> bytes (word_sub stackpointer (word 40),40)])`,
  X86_PROMOTE_RETURN_STACK_TAC bignum_montmul_p256k1_alt_tmc BIGNUM_MONTMUL_P256K1_ALT_SAFE `[RBX; R12; R13; R14; R15]` 40 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_MONTMUL_P256K1_ALT_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x y pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 40),48) /\
        ALL (nonoverlapping (word_sub stackpointer (word 40),40))
        [word pc,LENGTH bignum_montmul_p256k1_alt_mc; x,8 * 4; y,8 * 4] /\
        nonoverlapping (word pc,LENGTH bignum_montmul_p256k1_alt_mc)
        (z,8 * 4)
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_montmul_p256k1_alt_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x; y] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 4);
              memory :> bytes (word_sub stackpointer (word 40),40)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MONTMUL_P256K1_ALT_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_MONTMUL_P256K1_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x y pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 56),64) /\
        ALL (nonoverlapping (word_sub stackpointer (word 56),56))
        [word pc,LENGTH bignum_montmul_p256k1_alt_windows_tmc; x,8 * 4;
         y,8 * 4] /\
        nonoverlapping (word pc,LENGTH bignum_montmul_p256k1_alt_windows_tmc)
        (z,8 * 4)
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc)
                 bignum_montmul_p256k1_alt_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x; y] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 4);
              memory :> bytes (word_sub stackpointer (word 56),56)])`,
  WINDOWS_X86_WRAP_STACK_TAC bignum_montmul_p256k1_alt_windows_tmc bignum_montmul_p256k1_alt_tmc BIGNUM_MONTMUL_P256K1_ALT_SAFE `[RBX; R12; R13; R14; R15]` 40 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_MONTMUL_P256K1_ALT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x y pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 56),64) /\
        ALL (nonoverlapping (word_sub stackpointer (word 56),56))
        [word pc,LENGTH bignum_montmul_p256k1_alt_windows_mc; x,8 * 4;
         y,8 * 4] /\
        nonoverlapping (word pc,LENGTH bignum_montmul_p256k1_alt_windows_mc)
        (z,8 * 4)
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc)
                 bignum_montmul_p256k1_alt_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x; y] s /\
