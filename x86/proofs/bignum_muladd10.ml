...
let rec MULADD10_CONTAIN_TAC g =
  (FIRST [ MATCH_MP_TAC MULADD10_CONTAINED THEN CONJ_TAC THEN SIMPLE_ARITH_TAC;
           DISJ1_TAC THEN MULADD10_CONTAIN_TAC;
           DISJ2_TAC THEN MULADD10_CONTAIN_TAC ]) g;;

let BIGNUM_MULADD10_SAFE = prove
 (`exists f_events.
       forall e k z d pc.
           nonoverlapping (word pc,50) (z,8 * val k)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST bignum_muladd10_tmc) /\
                    read RIP s = word pc /\
                    C_ARGUMENTS [k; z; d] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 49) /\
                    (exists e2.
  ...
  W64_GEN_TAC `k:num` THEN MAP_EVERY X_GEN_TAC [`z:int64`; `d:int64`; `pc:num`] THEN
  REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS] THEN
  DISCH_THEN(REPEAT_TCL CONJUNCTS_THEN ASSUME_TAC) THEN
  ASM_CASES_TAC `k = 0` THENL
   [ ASM_REWRITE_TAC[] THEN X86_SIM_TAC BIGNUM_MULADD10_EXEC (1--4) THEN DISCHARGE_SAFETY_PROPERTY_TAC; ALL_TAC ] THEN
  ASM_REWRITE_TAC[ASSUME `val(word k:int64) = k`] THEN
  ENSURES_EVENTS_WHILE_UP2_TAC `k:num` `pc + 0x11` `pc + 0x2e`
   `\i s. read RDI s = word k /\ read RSI s = z /\ read R9 s = word 10 /\ read R8 s = word i` THEN
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [ X86_SIM_TAC BIGNUM_MULADD10_EXEC (1--5) THEN ASM_REWRITE_TAC[] THEN DISCHARGE_SAFETY_PROPERTY_TAC;
     ALL_TAC;
     ENSURES_INIT_TAC "s0" THEN STRIP_EXISTS_ASSUM_TAC THEN X86_STEPS_TAC BIGNUM_MULADD10_EXEC (1--1) THEN ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN DISCHARGE_SAFETY_PROPERTY_TAC ] THEN
  ...
  X86_STEPS_TAC BIGNUM_MULADD10_EXEC (1--9) THEN
  ...
  ((REWRITE_TAC[memaccess_inbounds; ALL; EX; FST; SND] THEN REPEAT CONJ_TAC THEN MULADD10_CONTAIN_TAC)
   ORELSE
...
let BIGNUM_MULADD10_NOIBT_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e k z d pc stackpointer returnaddress.
        nonoverlapping (word pc,LENGTH bignum_muladd10_tmc) (z,8 * val k) /\
        nonoverlapping (stackpointer,8) (z,8 * val k)
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_muladd10_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [k; z; d] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             MAYCHANGE [memory :> bignum (z,val k)])`,
  X86_PROMOTE_RETURN_NOSTACK_TAC bignum_muladd10_tmc BIGNUM_MULADD10_SAFE THEN DISCHARGE_SAFETY_PROPERTY_TAC);;
...
exists f_events.
    forall e k z d pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_muladd10_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [k; z; d] s /\
            ...
            (MAYCHANGE [RSP] ,,
             MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE [memory :> bignum (z,val k)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MULADD10_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_MULADD10_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e k z d pc stackpointer returnaddress.
        nonoverlapping (word pc,LENGTH bignum_muladd10_windows_tmc)
        (z,8 * val k) /\
        ALL (nonoverlapping (word_sub stackpointer (word 16),24))
        [word pc,LENGTH bignum_muladd10_windows_tmc; z,8 * val k]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_muladd10_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [k; z; d] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,val k);
              memory :> bytes (word_sub stackpointer (word 16),16)])`,
  WINDOWS_X86_WRAP_NOSTACK_TAC bignum_muladd10_windows_tmc bignum_muladd10_tmc BIGNUM_MULADD10_SAFE THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_MULADD10_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e k z d pc stackpointer returnaddress.
        nonoverlapping (word pc,LENGTH bignum_muladd10_windows_mc)
        (z,8 * val k) /\
        ALL (nonoverlapping (word_sub stackpointer (word 16),24))
        [word pc,LENGTH bignum_muladd10_windows_mc; z,8 * val k]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_muladd10_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [k; z; d] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,val k);
              memory :> bytes (word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MULADD10_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
