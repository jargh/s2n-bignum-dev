...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "bignum_cmul_p384" subroutine_signatures)
    BIGNUM_CMUL_P384_CORRECT
    BIGNUM_CMUL_P384_EXEC;;

let BIGNUM_CMUL_P384_SAFE = time prove
 (`exists f_events.
       forall e z c x pc.
           nonoverlapping (word pc,217) (z,8 * 6)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST bignum_cmul_p384_tmc) /\
                    read RIP s = word (pc + 2) /\
                    C_ARGUMENTS [z; c; x] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 214) /\
                    (exists e2.
  ...
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars BIGNUM_CMUL_P384_EXEC);;

let BIGNUM_CMUL_P384_NOIBT_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z c x pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_cmul_p384_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; c; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 6);
              memory :> bytes (word_sub stackpointer (word 8),8)])`,
  X86_PROMOTE_RETURN_STACK_TAC bignum_cmul_p384_tmc BIGNUM_CMUL_P384_SAFE `[R12]` 8 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_CMUL_P384_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z c x pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 8),8) (x,8 * 6) /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_cmul_p384_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; c; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 6);
              memory :> bytes (word_sub stackpointer (word 8),8)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_CMUL_P384_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_CMUL_P384_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z c x pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 24),24) (x,8 * 6) /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_cmul_p384_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; c; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 6);
              memory :> bytes (word_sub stackpointer (word 24),24)])`,
  WINDOWS_X86_WRAP_STACK_TAC bignum_cmul_p384_windows_tmc bignum_cmul_p384_tmc BIGNUM_CMUL_P384_SAFE `[R12]` 8 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_CMUL_P384_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z c x pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 24),24) (x,8 * 6) /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_cmul_p384_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; c; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 6);
              memory :> bytes (word_sub stackpointer (word 24),24)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_CMUL_P384_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
