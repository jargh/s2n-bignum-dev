...
let full_spec,public_vars = mk_safety_spec
    ...
    BIGNUM_SQR_6_12_CORRECT
    BIGNUM_SQR_6_12_EXEC;;

let BIGNUM_SQR_6_12_SAFE = time prove
 (`exists f_events.
       forall e z x pc.
           nonoverlapping (word pc,544) (z,8 * 12) /\
           (x = z \/ nonoverlapping (x,8 * 6) (z,8 * 12))
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST bignum_sqr_6_12_tmc) /\
                    read RIP s = word (pc + 10) /\
                    C_ARGUMENTS [z; x] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 533) /\
                    (exists e2.
                         read events s = APPEND e2 e /\
                         e2 = f_events x z pc /\
                         memaccess_inbounds e2 [x,48; z,96] [z,96]))
               (MAYCHANGE
                [RIP; RAX; RBP; RBX; RCX; RDX; R8; R9; R10; R11; R12; R13;
                 R14; R15] ,,
                MAYCHANGE [memory :> bytes (z,8 * 12)] ,,
                MAYCHANGE SOME_FLAGS ,,
                MAYCHANGE [events])`,
  ASSERT_CONCL_TAC full_spec THEN
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars BIGNUM_SQR_6_12_EXEC);;

let BIGNUM_SQR_6_12_NOIBT_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_sqr_6_12_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      [x,48; z,96; word_sub stackpointer (word 48),56]
                      [z,96; word_sub stackpointer (word 48),48]))
            (MAYCHANGE [RSP] ,,
             MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE
             [memory :> bytes (z,8 * 12);
              memory :> bytes (word_sub stackpointer (word 48),48)])`,
  X86_PROMOTE_RETURN_STACK_TAC bignum_sqr_6_12_tmc BIGNUM_SQR_6_12_SAFE `[RBX; RBP; R12; R13; R14; R15]` 48 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;
...
exists f_events.
    forall e z x pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_sqr_6_12_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 12);
              memory :> bytes (word_sub stackpointer (word 48),48)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_SQR_6_12_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_SQR_6_12_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 64),72) (z,8 * 12) /\
        ALL (nonoverlapping (word_sub stackpointer (word 64),64))
        [word pc,LENGTH bignum_sqr_6_12_windows_tmc; x,8 * 6] /\
        nonoverlapping (word pc,LENGTH bignum_sqr_6_12_windows_tmc)
        (z,8 * 12) /\
        (x = z \/ nonoverlapping (x,8 * 6) (z,8 * 12))
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_sqr_6_12_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      returnaddress /\
                      memaccess_inbounds e2
                      [x,48; z,96; word_sub stackpointer (word 64),72]
                      [z,96; word_sub stackpointer (word 64),64]))
            (MAYCHANGE [RSP] ,,
             WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE
             [memory :> bytes (z,8 * 12);
              memory :> bytes (word_sub stackpointer (word 64),64)])`,
  WINDOWS_X86_WRAP_STACK_TAC bignum_sqr_6_12_windows_tmc bignum_sqr_6_12_tmc BIGNUM_SQR_6_12_SAFE `[RBX; RBP; R12; R13; R14; R15]` 48 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_SQR_6_12_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 64),72) (z,8 * 12) /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_sqr_6_12_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 12);
              memory :> bytes (word_sub stackpointer (word 64),64)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_SQR_6_12_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
