...
let SM2_MONTJMIXADD_ALT_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 240),240))
        [word pc,LENGTH sm2_montjmixadd_alt_tmc; p1,96; p2,64] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH sm2_montjmixadd_alt_tmc;
         word_sub stackpointer (word 240),248]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) sm2_montjmixadd_alt_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,96);
              memory :> bytes (word_sub stackpointer (word 240),240)])`,
  X86_PROMOTE_RETURN_STACK_TAC sm2_montjmixadd_alt_tmc SM2_MONTJMIXADD_ALT_SAFE
    `[RBX; RBP; R12; R13; R14; R15]` 240
...
let SM2_MONTJMIXADD_ALT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ...
        [word pc,LENGTH sm2_montjmixadd_alt_mc; p1,96; p2,64] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH sm2_montjmixadd_alt_mc;
         word_sub stackpointer (word 240),248]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) sm2_montjmixadd_alt_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,96);
              memory :> bytes (word_sub stackpointer (word 240),240)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE SM2_MONTJMIXADD_ALT_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let SM2_MONTJMIXADD_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 256),256))
        [word pc,LENGTH sm2_montjmixadd_alt_windows_tmc; p1,96; p2,64] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH sm2_montjmixadd_alt_windows_tmc;
         word_sub stackpointer (word 256),264]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) sm2_montjmixadd_alt_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,96);
              memory :> bytes (word_sub stackpointer (word 256),256)])`,
  WINDOWS_X86_WRAP_STACK_TAC sm2_montjmixadd_alt_windows_tmc sm2_montjmixadd_alt_tmc
    SM2_MONTJMIXADD_ALT_SAFE `[RBX; RBP; R12; R13; R14; R15]` 240
  THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let SM2_MONTJMIXADD_ALT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 256),256))
        [word pc,LENGTH sm2_montjmixadd_alt_windows_mc; p1,96; p2,64] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH sm2_montjmixadd_alt_windows_mc;
         word_sub stackpointer (word 256),264]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) sm2_montjmixadd_alt_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,96);
              memory :> bytes (word_sub stackpointer (word 256),256)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE SM2_MONTJMIXADD_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
