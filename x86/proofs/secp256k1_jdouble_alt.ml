...
let SECP256K1_JDOUBLE_ALT_SAFE = time prove(full_spec,
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars SECP256K1_JDOUBLE_ALT_EXEC);;

let SECP256K1_JDOUBLE_ALT_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 424),424))
        [word pc,LENGTH secp256k1_jdouble_alt_tmc; p1,96] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH secp256k1_jdouble_alt_tmc;
         word_sub stackpointer (word 424),432]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) secp256k1_jdouble_alt_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [p3; p1] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,96);
              memory :> bytes (word_sub stackpointer (word 424),424)])`,
  X86_PROMOTE_RETURN_STACK_TAC secp256k1_jdouble_alt_tmc SECP256K1_JDOUBLE_ALT_SAFE
    `[RBX; R12; R13; R14; R15]` 424
...
let SECP256K1_JDOUBLE_ALT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 pc stackpointer returnaddress.
        ...
        [word pc,LENGTH secp256k1_jdouble_alt_mc; p1,96] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH secp256k1_jdouble_alt_mc;
         word_sub stackpointer (word 424),432]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) secp256k1_jdouble_alt_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [p3; p1] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,96);
              memory :> bytes (word_sub stackpointer (word 424),424)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE SECP256K1_JDOUBLE_ALT_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let SECP256K1_JDOUBLE_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 440),440))
        [word pc,LENGTH secp256k1_jdouble_alt_windows_tmc; p1,96] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH secp256k1_jdouble_alt_windows_tmc;
         word_sub stackpointer (word 440),448]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) secp256k1_jdouble_alt_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [p3; p1] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,96);
              memory :> bytes (word_sub stackpointer (word 440),440)])`,
  WINDOWS_X86_WRAP_STACK_TAC secp256k1_jdouble_alt_windows_tmc secp256k1_jdouble_alt_tmc
    SECP256K1_JDOUBLE_ALT_SAFE `[RBX; R12; R13; R14; R15]` 424
  THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let SECP256K1_JDOUBLE_ALT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 440),440))
        [word pc,LENGTH secp256k1_jdouble_alt_windows_mc; p1,96] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH secp256k1_jdouble_alt_windows_mc;
         word_sub stackpointer (word 440),448]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) secp256k1_jdouble_alt_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [p3; p1] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,96);
              memory :> bytes (word_sub stackpointer (word 440),440)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE SECP256K1_JDOUBLE_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
