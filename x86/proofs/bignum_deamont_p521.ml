...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "bignum_deamont_p521" subroutine_signatures)
    BIGNUM_DEAMONT_P521_CORRECT
    BIGNUM_DEAMONT_P521_EXEC;;

let BIGNUM_DEAMONT_P521_SAFE = time prove
 (`exists f_events.
       forall e z x pc.
           nonoverlapping (word pc,230) (z,8 * 9)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc)
                    (BUTLAST bignum_deamont_p521_tmc) /\
                    read RIP s = word (pc + 6) /\
                    C_ARGUMENTS [z; x] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 223) /\
                    (exists e2.
                         read events s = APPEND e2 e /\
                         e2 = f_events x z pc /\
                         memaccess_inbounds e2 [x,72; z,72] [z,72]))
               (MAYCHANGE
                [RIP; RAX; RCX; RDX; R8; R9; R10; R11; RBX; RBP; R12; R13] ,,
                MAYCHANGE [memory :> bytes (z,8 * 9)] ,,
                MAYCHANGE SOME_FLAGS ,,
                MAYCHANGE [events])`,
  ASSERT_CONCL_TAC full_spec THEN
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars BIGNUM_DEAMONT_P521_EXEC);;

let BIGNUM_DEAMONT_P521_NOIBT_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_deamont_p521_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 9);
              memory :> bytes (word_sub stackpointer (word 32),32)])`,
  X86_PROMOTE_RETURN_STACK_TAC bignum_deamont_p521_tmc BIGNUM_DEAMONT_P521_SAFE `[RBX; R12; R13; RBP]` 32 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_DEAMONT_P521_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        nonoverlapping (z,8 * 9) (word_sub stackpointer (word 32),40) /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_deamont_p521_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 9);
              memory :> bytes (word_sub stackpointer (word 32),32)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_DEAMONT_P521_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_DEAMONT_P521_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        nonoverlapping (z,8 * 9) (word_sub stackpointer (word 48),56) /\
        ALL (nonoverlapping (word_sub stackpointer (word 48),48))
        [word pc,LENGTH bignum_deamont_p521_windows_tmc; x,8 * 9] /\
        nonoverlapping (word pc,LENGTH bignum_deamont_p521_windows_tmc)
        (z,8 * 9)
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_deamont_p521_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 9);
              memory :> bytes (word_sub stackpointer (word 48),48)])`,
  WINDOWS_X86_WRAP_STACK_TAC bignum_deamont_p521_windows_tmc bignum_deamont_p521_tmc BIGNUM_DEAMONT_P521_SAFE `[RBX; R12; R13; RBP]` 32 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_DEAMONT_P521_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        nonoverlapping (z,8 * 9) (word_sub stackpointer (word 48),56) /\
        ALL (nonoverlapping (word_sub stackpointer (word 48),48))
        [word pc,LENGTH bignum_deamont_p521_windows_mc; x,8 * 9] /\
        nonoverlapping (word pc,LENGTH bignum_deamont_p521_windows_mc)
        (z,8 * 9)
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_deamont_p521_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 9);
              memory :> bytes (word_sub stackpointer (word 48),48)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_DEAMONT_P521_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
