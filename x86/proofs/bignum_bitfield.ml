...
let bitfield_full_spec,bitfield_public_vars = mk_safety_spec
    ~keep_maychanges:true
    (assoc "bignum_bitfield" subroutine_signatures)
    BIGNUM_BITFIELD_CORRECT
    BIGNUM_BITFIELD_EXEC;;

let BIGNUM_BITFIELD_SAFE = prove(bitfield_full_spec,
  ASSERT_CONCL_TAC bitfield_full_spec THEN
  CONCRETIZE_F_EVENTS_TAC
   `\(x:int64) (k:int64) (pc:num).
      if val k = 0 then f_ev_k0 x k pc
      else APPEND (f_ev_post x k pc)
             (APPEND (ENUMERATEL (val k) (f_ev_loop x k pc))
                     (f_ev_pre x k pc)):(uarch_event) list` THEN
  REPEAT META_EXISTS_TAC THEN STRIP_TAC THEN
  W64_GEN_TAC `k:num` THEN
  MAP_EVERY X_GEN_TAC [`x:int64`; `n:int64`; `l:int64`; `pc:num`] THEN
  REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS] THEN
  ASM_CASES_TAC `k = 0` THENL
  ...
  ENSURES_EVENTS_WHILE_UP2_TAC `k:num` `pc + 0x1b` `pc + 0x32`
   `\i s. read RDI s = word k /\ read RSI s = x /\ read R9 s = word i` THEN
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [ ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC BIGNUM_BITFIELD_EXEC (1--8) THEN
     ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN DISCHARGE_SAFETY_PROPERTY_TAC;
     ALL_TAC;
     ENSURES_INIT_TAC "s0" THEN STRIP_EXISTS_ASSUM_TAC THEN
     X86_STEPS_TAC BIGNUM_BITFIELD_EXEC (1--15) THEN
     ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN DISCHARGE_SAFETY_PROPERTY_TAC ] THEN
  X_GEN_TAC `i:num` THEN STRIP_TAC THEN VAL_INT64_TAC `i:num` THEN
  ENSURES_INIT_TAC "s0" THEN STRIP_EXISTS_ASSUM_TAC THEN
  X86_STEPS_TAC BIGNUM_BITFIELD_EXEC (1--7) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
...
let BIGNUM_BITFIELD_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e k x n l pc stackpointer returnaddress.
        ensures x86
            (\s. bytes_loaded s (word pc) bignum_bitfield_tmc /\
                 read RIP s = word pc /\ read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [k; x; n; l] s /\ read events s = e)
            (\s. read RIP s = returnaddress /\
                 read RSP s = word_add stackpointer (word 8) /\
            ...
            (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI)`,
  X86_PROMOTE_RETURN_NOSTACK_TAC bignum_bitfield_tmc BIGNUM_BITFIELD_SAFE THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_BITFIELD_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e k x n l pc stackpointer returnaddress.
        ensures x86
            (\s. bytes_loaded s (word pc) bignum_bitfield_mc /\
            ...
            (\s. read RIP s = returnaddress /\
                 ...
                 (exists e2. read events s = APPEND e2 e /\
                      ...
                      memaccess_inbounds e2 [x,val k * 8; stackpointer,8] []))
            (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI)`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_BITFIELD_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_BITFIELD_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e k x n l pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
        [word pc,LENGTH bignum_bitfield_windows_tmc; x,8 * val k]
        ==> ensures x86
            (\s. bytes_loaded s (word pc) bignum_bitfield_windows_tmc /\
                 read RIP s = word pc /\ read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [k; x; n; l] s /\ read events s = e)
            (\s. read RIP s = returnaddress /\
                 read RSP s = word_add stackpointer (word 8) /\
                 (exists e2. read events s = APPEND e2 e /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             MAYCHANGE [memory :> bytes (word_sub stackpointer (word 16),16)])`,
  WINDOWS_X86_WRAP_NOSTACK_TAC bignum_bitfield_windows_tmc bignum_bitfield_tmc BIGNUM_BITFIELD_SAFE THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_BITFIELD_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e k x n l pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
        [word pc,LENGTH bignum_bitfield_windows_mc; x,8 * val k]
        ==> ensures x86
            ...
            (MAYCHANGE [RSP] ,,
             WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE [memory :> bytes (word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_BITFIELD_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
