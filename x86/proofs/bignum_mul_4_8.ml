...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "bignum_mul_4_8" subroutine_signatures)
    BIGNUM_MUL_4_8_CORRECT
    BIGNUM_MUL_4_8_EXEC;;

let BIGNUM_MUL_4_8_SAFE = time prove
 (`exists f_events.
       forall e z x y pc.
           nonoverlapping (word pc,340) (z,8 * 8) /\
           (y = z \/ nonoverlapping (y,8 * 4) (z,8 * 8)) /\
           nonoverlapping (x,8 * 4) (z,8 * 8)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST bignum_mul_4_8_tmc) /\
                    read RIP s = word (pc + 2) /\
                    C_ARGUMENTS [z; x; y] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 337) /\
                    (exists e2.
  ...
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars BIGNUM_MUL_4_8_EXEC);;

let BIGNUM_MUL_4_8_NOIBT_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x y pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_mul_4_8_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x; y] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 8);
              memory :> bytes (word_sub stackpointer (word 16),16)])`,
  X86_PROMOTE_RETURN_STACK_TAC bignum_mul_4_8_tmc BIGNUM_MUL_4_8_SAFE `[RBX; RBP]` 16 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_MUL_4_8_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x y pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 16),24) (z,8 * 8) /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_mul_4_8_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x; y] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 8);
              memory :> bytes (word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MUL_4_8_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_MUL_4_8_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x y pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 32),40) (z,8 * 8) /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_mul_4_8_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x; y] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      returnaddress /\
                      memaccess_inbounds e2
                      [x,32; y,32; z,64; word_sub stackpointer (word 32),40]
                      [z,64; word_sub stackpointer (word 32),32]))
            (MAYCHANGE [RSP] ,,
             WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE
             [memory :> bytes (z,8 * 8);
              memory :> bytes (word_sub stackpointer (word 32),32)])`,
  WINDOWS_X86_WRAP_STACK_TAC bignum_mul_4_8_windows_tmc bignum_mul_4_8_tmc BIGNUM_MUL_4_8_SAFE `[RBX; RBP]` 16 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_MUL_4_8_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x y pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 32),40) (z,8 * 8) /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_mul_4_8_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x; y] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 8);
              memory :> bytes (word_sub stackpointer (word 32),32)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MUL_4_8_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
