...
(**** print_literal_from_elf "x86/secp256k1/bignum_mod_p256k1.o";;
 ****)

let bignum_mod_p256k1_mc =
  define_assert_from_elf "bignum_mod_p256k1_mc" "x86/secp256k1/bignum_mod_p256k1.o"
[
  0xf3; 0x0f; 0x1e; 0xfa;  (* ENDBR64 *)
  0x53;                    (* PUSH (% rbx) *)
  0x41; 0x54;              (* PUSH (% r12) *)
  0x48; 0x83; 0xfe; 0x04;  (* CMP (% rsi) (Imm8 (word 4)) *)
  0x0f; 0x82; 0xb3; 0x00; 0x00; 0x00;
                           (* JB (Imm32 (word 179)) *)
...
let bignum_mod_p256k1_tmc = define_trimmed "bignum_mod_p256k1_tmc" bignum_mod_p256k1_mc;;

let BIGNUM_MOD_P256K1_EXEC = X86_MK_EXEC_RULE bignum_mod_p256k1_tmc;;
...
let tac execth offset =
  ...
  ENSURES_SEQUENCE_TAC (offset 0x58)
   `\s. bignum_from_memory(x,k) s = n /\
        read RDI s = z /\
        read RCX s = x /\
        read RSI s = word(k - 4) /\
        read RBX s = word 0x1000003D1 /\
  ...
  CONJ_TAC THENL
   [ABBREV_TAC `j = k - 4` THEN VAL_INT64_TAC `j:num` THEN
    ...
    X86_ACCSTEPS_TAC execth [10;11;12;13] (1--13) THEN
    SUBGOAL_THEN `carry_s13 <=> p_256k1 <= m` SUBST_ALL_TAC THENL
     [MATCH_MP_TAC FLAG_FROM_CARRY_LE THEN EXISTS_TAC `256` THEN
      ...
      ALL_TAC] THEN
    X86_ACCSTEPS_TAC execth [17;18;19;20] (14--20) THEN
    ...
    REWRITE_TAC[REAL_BITVAL_NOT; p_256k1] THEN
  ...
  ENSURES_WHILE_PDOWN_TAC `k - 4` (offset 0x5d) (offset 0xab)
  ...
  X86_ACCSTEPS_TAC execth [6;7;8;9;10;11;12] (5--12) THEN
  ...
  X86_ACCSTEPS_TAC execth [15;16;17;18] (13--23) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
...
let BIGNUM_MOD_P256K1_CORRECT = time prove
 (`!z k x n pc.
  ...
  tac BIGNUM_MOD_P256K1_EXEC (curry mk_comb `(+) (pc:num)` o mk_small_numeral));;

let BIGNUM_MOD_P256K1_NOIBT_SUBROUTINE_CORRECT = time prove
 (`!z k x n pc stackpointer returnaddress.
      nonoverlapping (word_sub stackpointer (word 16),24) (z,32) /\
      ALL (nonoverlapping (word_sub stackpointer (word 16),16))
          [(word pc,LENGTH bignum_mod_p256k1_tmc); (x, 8 * val k)] /\
      nonoverlapping (word pc,LENGTH bignum_mod_p256k1_tmc) (z,32)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) bignum_mod_p256k1_tmc /\
                read RIP s = word pc /\
  ...
  X86_ADD_RETURN_STACK_TAC BIGNUM_MOD_P256K1_EXEC BIGNUM_MOD_P256K1_CORRECT
   `[RBX; R12]` 16);;
...
let BIGNUM_MOD_P256K1_SUBROUTINE_CORRECT = time prove
 (`!z k x n pc stackpointer returnaddress.
      nonoverlapping (word_sub stackpointer (word 16),24) (z,32) /\
      ALL (nonoverlapping (word_sub stackpointer (word 16),16))
          [(word pc,LENGTH bignum_mod_p256k1_mc); (x, 8 * val k)] /\
      nonoverlapping (word pc,LENGTH bignum_mod_p256k1_mc) (z,32)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) bignum_mod_p256k1_mc /\
                read RIP s = word pc /\
                read RSP s = stackpointer /\
                read (memory :> bytes64 stackpointer) s = returnaddress /\
                C_ARGUMENTS [z; k; x] s /\
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bignum(z,4);
                 memory :> bytes(word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MOD_P256K1_NOIBT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let bignum_mod_p256k1_windows_mc = define_from_elf
   "bignum_mod_p256k1_windows_mc" "x86/secp256k1/bignum_mod_p256k1.obj";;

let bignum_mod_p256k1_windows_tmc = define_trimmed "bignum_mod_p256k1_windows_tmc" bignum_mod_p256k1_windows_mc;;
 ...
 (`!z k x n pc.
      ...
      ==> ensures x86
          ...
          (MAYCHANGE [RIP; RSI; RAX; RDX; RCX; RBX; R8; R9; R10; R11; R12] ,,
           MAYCHANGE SOME_FLAGS ,, MAYCHANGE [events] ,,
           MAYCHANGE [memory :> bignum(z,4)])`,
  tac (X86_MK_EXEC_RULE bignum_mod_p256k1_windows_tmc)
      (curry mk_comb `(+) (pc:num)` o mk_small_numeral o (fun n -> n+11)));;

let BIGNUM_MOD_P256K1_NOIBT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z k x n pc stackpointer returnaddress.
      nonoverlapping (word_sub stackpointer (word 32),40) (z,32) /\
      ALL (nonoverlapping (word_sub stackpointer (word 32),32))
          [(word pc,LENGTH bignum_mod_p256k1_windows_tmc); (x, 8 * val k)] /\
      nonoverlapping (word pc,LENGTH bignum_mod_p256k1_windows_tmc) (z,32)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) bignum_mod_p256k1_windows_tmc /\
                read RIP s = word pc /\
                read RSP s = stackpointer /\
  ...
  GEN_X86_ADD_RETURN_STACK_TAC (X86_MK_EXEC_RULE bignum_mod_p256k1_windows_tmc)
    BIGNUM_MOD_P256K1_WINDOWS_CORRECT
    `[RDI; RSI; RBX; R12]` 32 (7,5));;

let BIGNUM_MOD_P256K1_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z k x n pc stackpointer returnaddress.
      nonoverlapping (word_sub stackpointer (word 32),40) (z,32) /\
      ALL (nonoverlapping (word_sub stackpointer (word 32),32))
          [(word pc,LENGTH bignum_mod_p256k1_windows_mc); (x, 8 * val k)] /\
      nonoverlapping (word pc,LENGTH bignum_mod_p256k1_windows_mc) (z,32)
      ==> ensures x86
           (\s. bytes_loaded s (word pc) bignum_mod_p256k1_windows_mc /\
                read RIP s = word pc /\
                read RSP s = stackpointer /\
                read (memory :> bytes64 stackpointer) s = returnaddress /\
                WINDOWS_C_ARGUMENTS [z; k; x] s /\
          ...
          (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bignum(z,4);
                 memory :> bytes(word_sub stackpointer (word 32),32)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MOD_P256K1_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;
...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "bignum_mod_p256k1" subroutine_signatures)
    BIGNUM_MOD_P256K1_CORRECT
    BIGNUM_MOD_P256K1_EXEC;;
...
let MODP_SAFE_TAC spec execth off =
  ...
  FIRST_ASSUM(STRIP_ASSUME_TAC o MATCH_MP (ARITH_RULE
    `~(k < 4) ==> ~(k = 0) /\ ~(k = 1) /\ ~(k = 2) /\ ~(k = 3)`)) THEN
  ASM_REWRITE_TAC[] THEN
  ENSURES_EVENTS_SEQUENCE_TAC (off 0x58)
  ...
  FIRST_ASSUM(STRIP_ASSUME_TAC o MATCH_MP (ARITH_RULE
   ...
   [ ASM_REWRITE_TAC[SUB_REFL; MULT_0] THEN
     ...
     ALL_TAC ] THEN
  ASM_REWRITE_TAC[] THEN
  ENSURES_EVENTS_WHILE_UP2_TAC `k' - 4` (off 0x5d) (off 0xad)
  ...
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [ SIMPLE_ARITH_TAC;
     ...
     ENSURES_INIT_TAC "s0" THEN STRIP_EXISTS_ASSUM_TAC THEN
     X86_STEPS_TAC execth (1--24) THEN
     ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
...
let BIGNUM_MOD_P256K1_SAFE = prove(full_spec,
  MODP_SAFE_TAC full_spec BIGNUM_MOD_P256K1_EXEC
    (fun n -> mk_comb(`(+) (pc:num)`, mk_small_numeral n)));;

let BIGNUM_MOD_P256K1_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e z k x pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_mod_p256k1_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; k; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,4);
              memory :> bytes (word_sub stackpointer (word 16),16)])`,
  X86_ADD_RETURN_STACK_TAC BIGNUM_MOD_P256K1_EXEC BIGNUM_MOD_P256K1_SAFE
    `[RBX; R12]` 16 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_MOD_P256K1_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e z k x pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 16),24) (z,32) /\
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_mod_p256k1_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; k; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,4);
              memory :> bytes (word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MOD_P256K1_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_MOD_P256K1_WINDOWS_SAFE = prove
 (`exists f_events.
     forall e z k x pc.
         nonoverlapping (word pc,245) (z,32)
         ==> ensures x86
             (\s.
                  bytes_loaded s (word pc) bignum_mod_p256k1_windows_tmc /\
                  read RIP s = word (pc + 14) /\
                  C_ARGUMENTS [z; k; x] s /\
                  read events s = e)
             (\s.
                  read RIP s = word (pc + 199) /\
                  (exists e2.
                       read events s = APPEND e2 e /\
       ...
       forall e z k x pc.
           ...
           ==> ensures x86
               ...
               (MAYCHANGE [RIP; RSI; RAX; RDX; RCX; RBX; R8; R9; R10; R11; R12] ,,
                ...
                MAYCHANGE [events] ,,
                MAYCHANGE [memory :> bignum (z,4)])`
    (X86_MK_EXEC_RULE bignum_mod_p256k1_windows_tmc)
    (fun n -> mk_comb(`(+) (pc:num)`, mk_small_numeral (n + 11))));;

let BIGNUM_MOD_P256K1_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e z k x pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 32),40) (z,32) /\
        ALL (nonoverlapping (word_sub stackpointer (word 32),32))
        ...
        ==> ensures x86
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,4);
              memory :> bytes (word_sub stackpointer (word 32),32)])`,
  GEN_X86_ADD_RETURN_STACK_TAC (X86_MK_EXEC_RULE bignum_mod_p256k1_windows_tmc)
    BIGNUM_MOD_P256K1_WINDOWS_SAFE `[RDI; RSI; RBX; R12]` 32 (7,5) THEN
  DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_MOD_P256K1_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e z k x pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 32),40) (z,32) /\
        ALL (nonoverlapping (word_sub stackpointer (word 32),32))
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_mod_p256k1_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; k; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,4);
              memory :> bytes (word_sub stackpointer (word 32),32)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MOD_P256K1_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
