...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "sm2_montjdouble" subroutine_signatures)
    SM2_MONTJDOUBLE_CORRECT
    SM2_MONTJDOUBLE_EXEC;;

let SM2_MONTJDOUBLE_SAFE = time prove
 (`exists f_events.
       forall e p3 p1 pc stackpointer.
           ...
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST sm2_montjdouble_tmc) /\
                    read RIP s = word (pc + 0x10) /\
                    read RSP s = stackpointer /\
                    C_ARGUMENTS [p3; p1] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 0x14b1) /\
                    (exists e2.
  ...
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars SM2_MONTJDOUBLE_EXEC);;

let SM2_MONTJDOUBLE_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 232),232))
        [word pc,LENGTH sm2_montjdouble_tmc; p1,96] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH sm2_montjdouble_tmc;
         word_sub stackpointer (word 232),240]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) sm2_montjdouble_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [p3; p1] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,96);
              memory :> bytes (word_sub stackpointer (word 232),232)])`,
  X86_PROMOTE_RETURN_STACK_TAC sm2_montjdouble_tmc SM2_MONTJDOUBLE_SAFE
    `[RBX; R12; R13; R14; R15]` 232
  THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let SM2_MONTJDOUBLE_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 232),232))
        [word pc,LENGTH sm2_montjdouble_mc; p1,96] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH sm2_montjdouble_mc;
         word_sub stackpointer (word 232),240]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) sm2_montjdouble_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [p3; p1] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,96);
              memory :> bytes (word_sub stackpointer (word 232),232)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE SM2_MONTJDOUBLE_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let SM2_MONTJDOUBLE_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 248),248))
        [word pc,LENGTH sm2_montjdouble_windows_tmc; p1,96] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH sm2_montjdouble_windows_tmc;
         word_sub stackpointer (word 248),256]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) sm2_montjdouble_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [p3; p1] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,96);
              memory :> bytes (word_sub stackpointer (word 248),248)])`,
  WINDOWS_X86_WRAP_STACK_TAC sm2_montjdouble_windows_tmc sm2_montjdouble_tmc
    SM2_MONTJDOUBLE_SAFE `[RBX; R12; R13; R14; R15]` 232
  THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let SM2_MONTJDOUBLE_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 248),248))
        [word pc,LENGTH sm2_montjdouble_windows_mc; p1,96] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH sm2_montjdouble_windows_mc;
         word_sub stackpointer (word 248),256]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) sm2_montjdouble_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [p3; p1] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,96);
              memory :> bytes (word_sub stackpointer (word 248),248)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE SM2_MONTJDOUBLE_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
