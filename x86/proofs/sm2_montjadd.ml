...
let SM2_MONTJADD_SAFE = time prove
 (`exists f_events.
       forall e p3 p1 p2 pc stackpointer.
           ...
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST sm2_montjadd_tmc) /\
                    read RIP s = word (pc + 0x11) /\
                    read RSP s = stackpointer /\
                    C_ARGUMENTS [p3; p1; p2] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 0x26be) /\
                    (exists e2.
...
let SM2_MONTJADD_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 272),272))
        [word pc,LENGTH sm2_montjadd_tmc; p1,96; p2,96] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH sm2_montjadd_tmc;
         word_sub stackpointer (word 272),280]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) sm2_montjadd_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,96);
              memory :> bytes (word_sub stackpointer (word 272),272)])`,
  X86_PROMOTE_RETURN_STACK_TAC sm2_montjadd_tmc SM2_MONTJADD_SAFE
    `[RBX; RBP; R12; R13; R14; R15]` 272
...
let SM2_MONTJADD_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ...
        [word pc,LENGTH sm2_montjadd_mc; p1,96; p2,96] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH sm2_montjadd_mc;
         word_sub stackpointer (word 272),280]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) sm2_montjadd_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,96);
              memory :> bytes (word_sub stackpointer (word 272),272)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE SM2_MONTJADD_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let SM2_MONTJADD_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 288),288))
        [word pc,LENGTH sm2_montjadd_windows_tmc; p1,96; p2,96] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH sm2_montjadd_windows_tmc;
         word_sub stackpointer (word 288),296]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) sm2_montjadd_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,96);
              memory :> bytes (word_sub stackpointer (word 288),288)])`,
  WINDOWS_X86_WRAP_STACK_TAC sm2_montjadd_windows_tmc sm2_montjadd_tmc
    SM2_MONTJADD_SAFE `[RBX; RBP; R12; R13; R14; R15]` 272
  THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let SM2_MONTJADD_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 288),288))
        [word pc,LENGTH sm2_montjadd_windows_mc; p1,96; p2,96] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH sm2_montjadd_windows_mc;
         word_sub stackpointer (word 288),296]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) sm2_montjadd_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,96);
              memory :> bytes (word_sub stackpointer (word 288),288)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE SM2_MONTJADD_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
