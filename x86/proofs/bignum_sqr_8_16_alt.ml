...
let bignum_sqr_8_16_alt_mc =
  define_assert_from_elf "bignum_sqr_8_16_alt_mc" "x86/fastmul/bignum_sqr_8_16_alt.o"
[
...
let bignum_sqr_8_16_alt_tmc = define_trimmed "bignum_sqr_8_16_alt_tmc" bignum_sqr_8_16_alt_mc;;

let BIGNUM_SQR_8_16_ALT_EXEC = X86_MK_CORE_EXEC_RULE bignum_sqr_8_16_alt_tmc;;
...
let BIGNUM_SQR_8_16_ALT_CORRECT = time prove
 (`!z x a pc.
     ALL (nonoverlapping (z,8 * 16)) [(word pc,0x386); (x,8 * 8)]
     ==> ensures x86
          (\s. bytes_loaded s (word pc) (BUTLAST bignum_sqr_8_16_alt_tmc) /\
               read RIP s = word pc /\
               C_ARGUMENTS [z; x] s /\
               bignum_from_memory (x,8) s = a)
          (\s. read RIP s = word (pc + 0x385) /\
  ...
  X86_ACCSTEPS_TAC BIGNUM_SQR_8_16_ALT_EXEC (1--264) (1--264) THEN
...
let BIGNUM_SQR_8_16_ALT_NOIBT_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
     ALL (nonoverlapping (z,8 * 16))
         [(word pc,LENGTH bignum_sqr_8_16_alt_tmc); (x,8 * 8); (stackpointer,8)]
     ==> ensures x86
          (\s. bytes_loaded s (word pc) bignum_sqr_8_16_alt_tmc /\
               read RIP s = word pc /\
  ...
  X86_PROMOTE_RETURN_NOSTACK_TAC
   bignum_sqr_8_16_alt_tmc BIGNUM_SQR_8_16_ALT_CORRECT);;

let BIGNUM_SQR_8_16_ALT_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
     ALL (nonoverlapping (z,8 * 16))
         [(word pc,LENGTH bignum_sqr_8_16_alt_mc); (x,8 * 8); (stackpointer,8)]
     ==> ensures x86
          (\s. bytes_loaded s (word pc) bignum_sqr_8_16_alt_mc /\
               read RIP s = word pc /\
               read RSP s = stackpointer /\
               read (memory :> bytes64 stackpointer) s = returnaddress /\
               C_ARGUMENTS [z; x] s /\
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(z,8 * 16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_SQR_8_16_ALT_NOIBT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let bignum_sqr_8_16_alt_windows_mc = define_from_elf
   "bignum_sqr_8_16_alt_windows_mc" "x86/fastmul/bignum_sqr_8_16_alt.obj";;

let bignum_sqr_8_16_alt_windows_tmc = define_trimmed "bignum_sqr_8_16_alt_windows_tmc" bignum_sqr_8_16_alt_windows_mc;;

let BIGNUM_SQR_8_16_ALT_NOIBT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,LENGTH bignum_sqr_8_16_alt_windows_tmc); (x,8 * 8)] /\
     ALL (nonoverlapping (z,8 * 16))
         [(word pc,LENGTH bignum_sqr_8_16_alt_windows_tmc); (x,8 * 8); (word_sub stackpointer (word 16),24)]
     ==> ensures x86
          (\s. bytes_loaded s (word pc) bignum_sqr_8_16_alt_windows_tmc /\
               read RIP s = word pc /\
               read RSP s = stackpointer /\
               read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_NOSTACK_TAC
   bignum_sqr_8_16_alt_windows_tmc bignum_sqr_8_16_alt_tmc
   BIGNUM_SQR_8_16_ALT_CORRECT);;

let BIGNUM_SQR_8_16_ALT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,LENGTH bignum_sqr_8_16_alt_windows_mc); (x,8 * 8)] /\
     ALL (nonoverlapping (z,8 * 16))
         [(word pc,LENGTH bignum_sqr_8_16_alt_windows_mc); (x,8 * 8); (word_sub stackpointer (word 16),24)]
     ==> ensures x86
          (\s. bytes_loaded s (word pc) bignum_sqr_8_16_alt_windows_mc /\
               read RIP s = word pc /\
               read RSP s = stackpointer /\
               read (memory :> bytes64 stackpointer) s = returnaddress /\
               WINDOWS_C_ARGUMENTS [z; x] s /\
          ...
          (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(z,8 * 16);
                      memory :> bytes(word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_SQR_8_16_ALT_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;
...
let BIGNUM_SQR_8_16_ALT_SAFE = time prove
 (`exists f_events.
       forall e z x pc.
           ALL (nonoverlapping (z,8 * 16)) [word pc,902; x,8 * 8]
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc)
                    (BUTLAST bignum_sqr_8_16_alt_tmc) /\
                    read RIP s = word pc /\
                    C_ARGUMENTS [z; x] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 901) /\
                    (exists e2.
                         read events s = APPEND e2 e /\
                         e2 = f_events x z pc /\
                         memaccess_inbounds e2 [x,64; z,128] [z,128]))
               (MAYCHANGE [RIP; RAX; RCX; RDX; R8; R9; R10; R11] ,,
                MAYCHANGE [memory :> bytes (z,8 * 16)] ,,
                MAYCHANGE SOME_FLAGS ,,
                MAYCHANGE [events])`,
  ASSERT_CONCL_TAC full_spec THEN
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars BIGNUM_SQR_8_16_ALT_EXEC);;

(* Has no "word_sub stackpointer (word ..)"; stackofs is None *)
let BIGNUM_SQR_8_16_ALT_NOIBT_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_sqr_8_16_alt_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      memaccess_inbounds e2 [x,64; z,128; stackpointer,8]
                      [z,128; stackpointer,0]))
            (MAYCHANGE [RSP] ,,
             MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE [memory :> bytes (z,8 * 16)])`,
  X86_PROMOTE_RETURN_NOSTACK_TAC bignum_sqr_8_16_alt_tmc BIGNUM_SQR_8_16_ALT_SAFE THEN DISCHARGE_SAFETY_PROPERTY_TAC);;
...
exists f_events.
    forall e z x pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_sqr_8_16_alt_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE [memory :> bytes (z,8 * 16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_SQR_8_16_ALT_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_SQR_8_16_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_sqr_8_16_alt_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      [x,64; z,128; word_sub stackpointer (word 16),24]
                      [z,128; word_sub stackpointer (word 16),16]))
            (MAYCHANGE [RSP] ,,
             WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE
             [memory :> bytes (z,8 * 16);
              memory :> bytes (word_sub stackpointer (word 16),16)])`,
  WINDOWS_X86_WRAP_NOSTACK_TAC bignum_sqr_8_16_alt_windows_tmc bignum_sqr_8_16_alt_tmc BIGNUM_SQR_8_16_ALT_SAFE THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_SQR_8_16_ALT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_sqr_8_16_alt_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 16);
              memory :> bytes (word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_SQR_8_16_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
