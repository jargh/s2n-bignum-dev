...
let bignum_demont_p521_mc = define_assert_from_elf "bignum_demont_p521_mc" "x86/p521/bignum_demont_p521.o"
[
  ...
  0x48; 0x0f; 0xac; 0xd1; 0x37;
                           (* SHRD (% rcx) (% rdx) (Imm8 (word 55)) *)
...
let bignum_demont_p521_tmc = define_trimmed "bignum_demont_p521_tmc" bignum_demont_p521_mc;;

let BIGNUM_DEMONT_P521_EXEC = X86_MK_CORE_EXEC_RULE bignum_demont_p521_tmc;;
...
let BIGNUM_DEMONT_P521_CORRECT = time prove
 (`!z x n pc.
        nonoverlapping (word pc,0x7d) (z,8 * 9) /\
        (x = z \/ nonoverlapping(x,8 * 9) (z,8 * 9))
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST bignum_demont_p521_tmc) /\
                  read RIP s = word pc /\
                  C_ARGUMENTS [z; x] s /\
                  bignum_from_memory (x,9) s = n)
             (\s. read RIP s = word (pc + 0x7c) /\
                  (n < p_521
  ...
  X86_STEPS_TAC BIGNUM_DEMONT_P521_EXEC (1--30) THEN
  ...
  REWRITE_TAC[BIT_WORD_SUBWORD; BIT_WORD_JOIN; BIT_WORD_OR; BIT_WORD_1;
...
let BIGNUM_DEMONT_P521_NOIBT_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        nonoverlapping (word pc,LENGTH bignum_demont_p521_tmc) (z,8 * 9) /\
        nonoverlapping (stackpointer,8) (z,8 * 9) /\
        (x = z \/ nonoverlapping(x,8 * 9) (z,8 * 9))
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_demont_p521_tmc /\
                  read RIP s = word pc /\
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bignum(z,9)])`,
  X86_PROMOTE_RETURN_NOSTACK_TAC bignum_demont_p521_tmc
    BIGNUM_DEMONT_P521_CORRECT);;

let BIGNUM_DEMONT_P521_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        nonoverlapping (word pc,LENGTH bignum_demont_p521_mc) (z,8 * 9) /\
        nonoverlapping (stackpointer,8) (z,8 * 9) /\
        (x = z \/ nonoverlapping(x,8 * 9) (z,8 * 9))
        ==> ensures x86
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bignum(z,9)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_DEMONT_P521_NOIBT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let bignum_demont_p521_windows_mc = define_from_elf
   "bignum_demont_p521_windows_mc" "x86/p521/bignum_demont_p521.obj";;

let bignum_demont_p521_windows_tmc = define_trimmed "bignum_demont_p521_windows_tmc" bignum_demont_p521_windows_mc;;

let BIGNUM_DEMONT_P521_NOIBT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,LENGTH bignum_demont_p521_windows_tmc); (x,8 * 9)] /\
        nonoverlapping (word pc,LENGTH bignum_demont_p521_windows_tmc) (z,8 * 9) /\
        nonoverlapping (word_sub stackpointer (word 16),24) (z,8 * 9) /\
        (x = z \/ nonoverlapping(x,8 * 9) (z,8 * 9))
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_demont_p521_windows_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_NOSTACK_TAC bignum_demont_p521_windows_tmc
    bignum_demont_p521_tmc BIGNUM_DEMONT_P521_CORRECT);;

let BIGNUM_DEMONT_P521_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,LENGTH bignum_demont_p521_windows_mc); (x,8 * 9)] /\
        nonoverlapping (word pc,LENGTH bignum_demont_p521_windows_mc) (z,8 * 9) /\
        nonoverlapping (word_sub stackpointer (word 16),24) (z,8 * 9) /\
        (x = z \/ nonoverlapping(x,8 * 9) (z,8 * 9))
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_demont_p521_windows_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [z; x] s /\
          ...
          (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bignum(z,9);
                      memory :> bytes(word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_DEMONT_P521_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;
...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "bignum_demont_p521" subroutine_signatures)
    BIGNUM_DEMONT_P521_CORRECT
    BIGNUM_DEMONT_P521_EXEC;;

let BIGNUM_DEMONT_P521_SAFE = time prove
 (`exists f_events.
       forall e z x pc.
           nonoverlapping (word pc,125) (z,8 * 9) /\
           (x = z \/ nonoverlapping (x,8 * 9) (z,8 * 9))
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST bignum_demont_p521_tmc) /\
                    read RIP s = word pc /\
                    C_ARGUMENTS [z; x] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 124) /\
                    (exists e2.
  ...
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars BIGNUM_DEMONT_P521_EXEC);;

(* Has no "word_sub stackpointer (word ..)"; stackofs is None *)
let BIGNUM_DEMONT_P521_NOIBT_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_demont_p521_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      memaccess_inbounds e2 [x,72; z,72; stackpointer,8]
                      [z,72; stackpointer,0]))
            (MAYCHANGE [RSP] ,,
             MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE [memory :> bignum (z,9)])`,
  X86_PROMOTE_RETURN_NOSTACK_TAC bignum_demont_p521_tmc BIGNUM_DEMONT_P521_SAFE THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_DEMONT_P521_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        nonoverlapping (word pc,LENGTH bignum_demont_p521_mc) (z,8 * 9) /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_demont_p521_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE [memory :> bignum (z,9)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_DEMONT_P521_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_DEMONT_P521_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
        [word pc,LENGTH bignum_demont_p521_windows_tmc; x,8 * 9] /\
        nonoverlapping (word pc,LENGTH bignum_demont_p521_windows_tmc)
        (z,8 * 9) /\
        nonoverlapping (word_sub stackpointer (word 16),24) (z,8 * 9) /\
        (x = z \/ nonoverlapping (x,8 * 9) (z,8 * 9))
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_demont_p521_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      [x,72; z,72; word_sub stackpointer (word 16),24]
                      [z,72; word_sub stackpointer (word 16),16]))
            (MAYCHANGE [RSP] ,,
             WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE
             [memory :> bignum (z,9);
              memory :> bytes (word_sub stackpointer (word 16),16)])`,
  WINDOWS_X86_WRAP_NOSTACK_TAC bignum_demont_p521_windows_tmc bignum_demont_p521_tmc BIGNUM_DEMONT_P521_SAFE THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_DEMONT_P521_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
        [word pc,LENGTH bignum_demont_p521_windows_mc; x,8 * 9] /\
        nonoverlapping (word pc,LENGTH bignum_demont_p521_windows_mc)
        (z,8 * 9) /\
        nonoverlapping (word_sub stackpointer (word 16),24) (z,8 * 9) /\
        (x = z \/ nonoverlapping (x,8 * 9) (z,8 * 9))
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_demont_p521_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,9);
              memory :> bytes (word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_DEMONT_P521_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
