...
let P384_MONTJMIXADD_ALT_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 352),352))
        [word pc,LENGTH p384_montjmixadd_alt_tmc; p1,144; p2,96] /\
        ALL (nonoverlapping (p3,144))
        [word pc,LENGTH p384_montjmixadd_alt_tmc;
         word_sub stackpointer (word 352),360]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p384_montjmixadd_alt_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,144);
              memory :> bytes (word_sub stackpointer (word 352),352)])`,
  X86_PROMOTE_RETURN_STACK_TAC p384_montjmixadd_alt_tmc P384_MONTJMIXADD_ALT_SAFE
    `[RBX; RBP; R12; R13; R14; R15]` 352
...
let P384_MONTJMIXADD_ALT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ...
        [word pc,LENGTH p384_montjmixadd_alt_mc; p1,144; p2,96] /\
        ALL (nonoverlapping (p3,144))
        [word pc,LENGTH p384_montjmixadd_alt_mc;
         word_sub stackpointer (word 352),360]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p384_montjmixadd_alt_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,144);
              memory :> bytes (word_sub stackpointer (word 352),352)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P384_MONTJMIXADD_ALT_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let P384_MONTJMIXADD_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 368),368))
        [word pc,LENGTH p384_montjmixadd_alt_windows_tmc; p1,144; p2,96] /\
        ALL (nonoverlapping (p3,144))
        [word pc,LENGTH p384_montjmixadd_alt_windows_tmc;
         word_sub stackpointer (word 368),376]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p384_montjmixadd_alt_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,144);
              memory :> bytes (word_sub stackpointer (word 368),368)])`,
  WINDOWS_X86_WRAP_STACK_TAC p384_montjmixadd_alt_windows_tmc p384_montjmixadd_alt_tmc
    P384_MONTJMIXADD_ALT_SAFE `[RBX; RBP; R12; R13; R14; R15]` 352
  THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let P384_MONTJMIXADD_ALT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 368),368))
        [word pc,LENGTH p384_montjmixadd_alt_windows_mc; p1,144; p2,96] /\
        ALL (nonoverlapping (p3,144))
        [word pc,LENGTH p384_montjmixadd_alt_windows_mc;
         word_sub stackpointer (word 368),376]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p384_montjmixadd_alt_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,144);
              memory :> bytes (word_sub stackpointer (word 368),368)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P384_MONTJMIXADD_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
