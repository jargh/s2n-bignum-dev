...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "bignum_mod_sm2" subroutine_signatures)
    BIGNUM_MOD_SM2_CORRECT
    BIGNUM_MOD_SM2_EXEC;;
...
let MODP_SAFE_TAC spec execth off =
  ...
  FIRST_ASSUM(STRIP_ASSUME_TAC o MATCH_MP (ARITH_RULE
    `~(k < 4) ==> ~(k = 0) /\ ~(k = 1) /\ ~(k = 2) /\ ~(k = 3)`)) THEN
  ASM_REWRITE_TAC[] THEN
  ENSURES_EVENTS_SEQUENCE_TAC (off 0x5b)
   `\s. read RDI s = z /\ read RDX s = x /\ read RSI s = word(k' - 4)` THEN
  CONJ_TAC THENL
   [ ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC execth (1--20) THEN
     ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  ...
  FIRST_ASSUM(STRIP_ASSUME_TAC o MATCH_MP (ARITH_RULE
   ...
   [ ASM_REWRITE_TAC[SUB_REFL; MULT_0] THEN
     ...
     ALL_TAC ] THEN
  ASM_REWRITE_TAC[] THEN
  ENSURES_EVENTS_WHILE_UP2_TAC `k' - 4` (off 0x64) (off 0xf3)
   `\i s. read RDI s = z /\ read RDX s = x /\ read RSI s = word((k' - 4) - i)` THEN
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [ SIMPLE_ARITH_TAC;
     ENSURES_INIT_TAC "s0" THEN STRIP_EXISTS_ASSUM_TAC THEN
...
let BIGNUM_MOD_SM2_SAFE = prove(full_spec,
  MODP_SAFE_TAC full_spec BIGNUM_MOD_SM2_EXEC
    (fun n -> mk_comb(`(+) (pc:num)`, mk_small_numeral n)));;

let BIGNUM_MOD_SM2_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e z k x pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_mod_sm2_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; k; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,4);
              memory :> bytes (word_sub stackpointer (word 16),16)])`,
  X86_ADD_RETURN_STACK_TAC BIGNUM_MOD_SM2_EXEC BIGNUM_MOD_SM2_SAFE
    `[RBX; R12]` 16 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_MOD_SM2_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e z k x pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 16),24) (z,32) /\
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_mod_sm2_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; k; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,4);
              memory :> bytes (word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MOD_SM2_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_MOD_SM2_WINDOWS_SAFE = prove
 (`exists f_events.
     forall e z k x pc.
         nonoverlapping (word pc,315) (z,32)
         ==> ensures x86
             (\s.
                  bytes_loaded s (word pc) bignum_mod_sm2_windows_tmc /\
       ...
       forall e z k x pc.
           ...
           ==> ensures x86
               ...
               (MAYCHANGE [RIP; RSI; RAX; RDX; RCX; RBX; R8; R9; R10; R11; R12] ,,
                ...
                MAYCHANGE [events] ,,
                MAYCHANGE [memory :> bignum (z,4)])`
    (X86_MK_EXEC_RULE bignum_mod_sm2_windows_tmc)
    (fun n -> mk_comb(`(+) (pc:num)`, mk_small_numeral (n + 11))));;

let BIGNUM_MOD_SM2_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e z k x pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 32),40) (z,32) /\
        ALL (nonoverlapping (word_sub stackpointer (word 32),32))
        ...
        ==> ensures x86
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,4);
              memory :> bytes (word_sub stackpointer (word 32),32)])`,
  GEN_X86_ADD_RETURN_STACK_TAC (X86_MK_EXEC_RULE bignum_mod_sm2_windows_tmc)
    BIGNUM_MOD_SM2_WINDOWS_SAFE `[RDI; RSI; RBX; R12]` 32 (7,5) THEN
  DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_MOD_SM2_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e z k x pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 32),40) (z,32) /\
        ALL (nonoverlapping (word_sub stackpointer (word 32),32))
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_mod_sm2_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; k; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,4);
              memory :> bytes (word_sub stackpointer (word 32),32)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MOD_SM2_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
