...
let bignum_montmul_p256k1_mc =
  define_assert_from_elf "bignum_montmul_p256k1_mc" "x86/secp256k1/bignum_montmul_p256k1.o"
[
...
let bignum_montmul_p256k1_tmc = define_trimmed "bignum_montmul_p256k1_tmc" bignum_montmul_p256k1_mc;;

let BIGNUM_MONTMUL_P256K1_EXEC = X86_MK_CORE_EXEC_RULE bignum_montmul_p256k1_tmc;;
...
let BIGNUM_MONTMUL_P256K1_CORRECT = time prove
 (`!z x y a b pc.
        ...
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST bignum_montmul_p256k1_tmc) /\
                  read RIP s = word(pc + 0xa) /\
                  C_ARGUMENTS [z; x; y] s /\
                  bignum_from_memory (x,4) s = a /\
                  bignum_from_memory (y,4) s = b)
             (\s. read RIP s = word (pc + 0x1e1) /\
  ...
  ASM_CASES_TAC `a * b <= 2 EXP 256 * p_256k1` THENL
   [ASM_REWRITE_TAC[]; X86_SIM_TAC BIGNUM_MONTMUL_P256K1_EXEC (1--105)] THEN
  ...
  X86_ACCSTEPS_TAC BIGNUM_MONTMUL_P256K1_EXEC (1--56) (1--56) THEN
  ABBREV_TAC
   `l = bignum_of_wordlist
     [mullo_s4; sum_s15; sum_s30; sum_s45;
      sum_s48; sum_s51; sum_s54; sum_s56]` THEN
  ...
  X86_ACCSTEPS_TAC BIGNUM_MONTMUL_P256K1_EXEC
   [61;62;63;66;68;72;74;78;80;81;82;83;84;85;86;87]
   (57--88) THEN
  ...
  ABBREV_TAC
   `t = bignum_of_wordlist[sum_s84; sum_s85; sum_s86; sum_s87;
                           word(bitval(carry_s87))]` THEN
  SUBGOAL_THEN
  ...
  X86_ACCSTEPS_TAC BIGNUM_MONTMUL_P256K1_EXEC [90;92;94;96] (89--105) THEN
  ...
  SUBGOAL_THEN `2 EXP 64 <= val(word_neg(word(bitval carry_s87):int64)) +
                            18446744073709551615 + bitval carry_s96 <=>
                p_256k1 <= t` SUBST_ALL_TAC THENL
   [EXPAND_TAC "t" THEN BOOL_CASES_TAC `carry_s87:bool` THEN
    ASM_REWRITE_TAC[BITVAL_CLAUSES; p_256k1; bignum_of_wordlist] THEN
    CONV_TAC(DEPTH_CONV WORD_NUM_RED_CONV) THENL
     [ARITH_TAC; ALL_TAC] THEN
    TRANS_TAC EQ_TRANS `carry_s96:bool` THEN CONJ_TAC THENL
...
let BIGNUM_MONTMUL_P256K1_NOIBT_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
        ...
        ALL (nonoverlapping (word_sub stackpointer (word 48),48))
            [(word pc,LENGTH bignum_montmul_p256k1_tmc); (x,8 * 4); (y,8 * 4)] /\
        nonoverlapping (word pc,LENGTH bignum_montmul_p256k1_tmc) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_montmul_p256k1_tmc /\
                  read RIP s = word pc /\
  ...
  X86_PROMOTE_RETURN_STACK_TAC
   bignum_montmul_p256k1_tmc BIGNUM_MONTMUL_P256K1_CORRECT
...
let BIGNUM_MONTMUL_P256K1_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 48),56) /\
        ALL (nonoverlapping (word_sub stackpointer (word 48),48))
            [(word pc,LENGTH bignum_montmul_p256k1_mc); (x,8 * 4); (y,8 * 4)] /\
        nonoverlapping (word pc,LENGTH bignum_montmul_p256k1_mc) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_montmul_p256k1_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  C_ARGUMENTS [z; x; y] s /\
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                         memory :> bytes(word_sub stackpointer (word 48),48)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MONTMUL_P256K1_NOIBT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let bignum_montmul_p256k1_windows_mc = define_from_elf
   "bignum_montmul_p256k1_windows_mc" "x86/secp256k1/bignum_montmul_p256k1.obj";;

let bignum_montmul_p256k1_windows_tmc = define_trimmed "bignum_montmul_p256k1_windows_tmc" bignum_montmul_p256k1_windows_mc;;

let BIGNUM_MONTMUL_P256K1_NOIBT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 64),72) /\
        ALL (nonoverlapping (word_sub stackpointer (word 64),64))
            [(word pc,LENGTH bignum_montmul_p256k1_windows_tmc); (x,8 * 4); (y,8 * 4)] /\
        nonoverlapping (word pc,LENGTH bignum_montmul_p256k1_windows_tmc) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_montmul_p256k1_windows_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
   bignum_montmul_p256k1_windows_tmc bignum_montmul_p256k1_tmc
   BIGNUM_MONTMUL_P256K1_CORRECT `[RBX; RBP; R12; R13; R14; R15]` 48);;

let BIGNUM_MONTMUL_P256K1_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 64),72) /\
        ALL (nonoverlapping (word_sub stackpointer (word 64),64))
            [(word pc,LENGTH bignum_montmul_p256k1_windows_mc); (x,8 * 4); (y,8 * 4)] /\
        nonoverlapping (word pc,LENGTH bignum_montmul_p256k1_windows_mc) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_montmul_p256k1_windows_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [z; x; y] s /\
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                         memory :> bytes(word_sub stackpointer (word 64),64)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MONTMUL_P256K1_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;
...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "bignum_montmul_p256k1" subroutine_signatures)
    BIGNUM_MONTMUL_P256K1_CORRECT
    BIGNUM_MONTMUL_P256K1_EXEC;;

let BIGNUM_MONTMUL_P256K1_SAFE = time prove
 (`exists f_events.
       forall e z x y pc.
           nonoverlapping (word pc,492) (z,8 * 4)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc)
                    (BUTLAST bignum_montmul_p256k1_tmc) /\
                    read RIP s = word (pc + 10) /\
                    C_ARGUMENTS [z; x; y] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 481) /\
                    (exists e2.
  ...
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars BIGNUM_MONTMUL_P256K1_EXEC);;

let BIGNUM_MONTMUL_P256K1_NOIBT_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x y pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_montmul_p256k1_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x; y] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      [x,32; y,32; z,32; word_sub stackpointer (word 48),56]
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 4);
              memory :> bytes (word_sub stackpointer (word 48),48)])`,
  X86_PROMOTE_RETURN_STACK_TAC bignum_montmul_p256k1_tmc BIGNUM_MONTMUL_P256K1_SAFE `[RBX; RBP; R12; R13; R14; R15]` 48 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_MONTMUL_P256K1_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x y pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 48),56) /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_montmul_p256k1_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x; y] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 4);
              memory :> bytes (word_sub stackpointer (word 48),48)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MONTMUL_P256K1_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_MONTMUL_P256K1_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x y pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 64),72) /\
        ALL (nonoverlapping (word_sub stackpointer (word 64),64))
        [word pc,LENGTH bignum_montmul_p256k1_windows_tmc; x,8 * 4; y,8 * 4] /\
        nonoverlapping (word pc,LENGTH bignum_montmul_p256k1_windows_tmc)
        (z,8 * 4)
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_montmul_p256k1_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x; y] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      read events s = APPEND e2 e /\
                      e2 =
                      f_events x y z pc (word_sub stackpointer (word 64))
                      returnaddress /\
                      memaccess_inbounds e2
                      [x,32; y,32; z,32; word_sub stackpointer (word 64),72]
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 4);
              memory :> bytes (word_sub stackpointer (word 64),64)])`,
  WINDOWS_X86_WRAP_STACK_TAC bignum_montmul_p256k1_windows_tmc bignum_montmul_p256k1_tmc BIGNUM_MONTMUL_P256K1_SAFE `[RBX; RBP; R12; R13; R14; R15]` 48 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_MONTMUL_P256K1_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x y pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 64),72) /\
        ALL (nonoverlapping (word_sub stackpointer (word 64),64))
        [word pc,LENGTH bignum_montmul_p256k1_windows_mc; x,8 * 4; y,8 * 4] /\
        nonoverlapping (word pc,LENGTH bignum_montmul_p256k1_windows_mc)
        (z,8 * 4)
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_montmul_p256k1_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x; y] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 4);
              memory :> bytes (word_sub stackpointer (word 64),64)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MONTMUL_P256K1_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
