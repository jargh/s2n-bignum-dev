...
let WORD_NEGMODINV_SAFE = time prove
 (`exists f_events.
       forall e a pc.
           ensures x86
           (\s.
                bytes_loaded s (word pc) (BUTLAST word_negmodinv_tmc) /\
                read RIP s = word pc /\
                C_ARGUMENTS [a] s /\
                read events s = e)
           (\s.
  ...
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars WORD_NEGMODINV_EXEC);;

(* Has no "word_sub stackpointer (word ..)"; stackofs is None *)
let WORD_NEGMODINV_NOIBT_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e a pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) word_negmodinv_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [a] s /\
            ...
            (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI)`,
  X86_PROMOTE_RETURN_NOSTACK_TAC word_negmodinv_tmc WORD_NEGMODINV_SAFE THEN DISCHARGE_SAFETY_PROPERTY_TAC);;
...
exists f_events.
    forall e a pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) word_negmodinv_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [a] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      memaccess_inbounds e2 [stackpointer,8] [stackpointer,0]))
            (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI)`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WORD_NEGMODINV_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let WORD_NEGMODINV_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e a pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 16),16)
        (word pc,LENGTH word_negmodinv_windows_tmc)
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) word_negmodinv_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [a] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             MAYCHANGE [memory :> bytes (word_sub stackpointer (word 16),16)])`,
  WINDOWS_X86_WRAP_NOSTACK_TAC word_negmodinv_windows_tmc word_negmodinv_tmc WORD_NEGMODINV_SAFE THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let WORD_NEGMODINV_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e a pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 16),16)
        (word pc,LENGTH word_negmodinv_windows_mc)
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) word_negmodinv_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [a] s /\
            ...
            (MAYCHANGE [RSP] ,,
             WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE [memory :> bytes (word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE WORD_NEGMODINV_NOIBT_WINDOWS_SUBROUTINE_SAFE));;