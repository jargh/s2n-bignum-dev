...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "bignum_sqr_4_8" subroutine_signatures)
    BIGNUM_SQR_4_8_CORRECT
    BIGNUM_SQR_4_8_EXEC;;

let BIGNUM_SQR_4_8_SAFE = time prove
 (`exists f_events.
       forall e z x pc.
           nonoverlapping (word pc,265) (z,8 * 8) /\
           (x = z \/ nonoverlapping (x,8 * 4) (z,8 * 8))
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST bignum_sqr_4_8_tmc) /\
                    read RIP s = word (pc + 5) /\
                    C_ARGUMENTS [z; x] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 259) /\
                    (exists e2.
                         read events s = APPEND e2 e /\
                         e2 = f_events x z pc /\
                         memaccess_inbounds e2 [x,32; z,64] [z,64]))
               (MAYCHANGE
                [RIP; RAX; RBP; RCX; RDX; R8; R9; R10; R11; R12; R13] ,,
                MAYCHANGE [memory :> bytes (z,8 * 8)] ,,
                MAYCHANGE SOME_FLAGS ,,
                MAYCHANGE [events])`,
  ASSERT_CONCL_TAC full_spec THEN
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars BIGNUM_SQR_4_8_EXEC);;

let BIGNUM_SQR_4_8_NOIBT_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_sqr_4_8_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      [x,32; z,64; word_sub stackpointer (word 24),32]
                      [z,64; word_sub stackpointer (word 24),24]))
            (MAYCHANGE [RSP] ,,
             MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE
             [memory :> bytes (z,8 * 8);
              memory :> bytes (word_sub stackpointer (word 24),24)])`,
  X86_PROMOTE_RETURN_STACK_TAC bignum_sqr_4_8_tmc BIGNUM_SQR_4_8_SAFE `[RBP; R12; R13]` 24 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_SQR_4_8_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 24),32) (z,8 * 8) /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_sqr_4_8_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 8);
              memory :> bytes (word_sub stackpointer (word 24),24)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_SQR_4_8_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_SQR_4_8_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 40),48) (z,8 * 8) /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_sqr_4_8_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      returnaddress /\
                      memaccess_inbounds e2
                      [x,32; z,64; word_sub stackpointer (word 40),48]
                      [z,64; word_sub stackpointer (word 40),40]))
            (MAYCHANGE [RSP] ,,
             WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE
             [memory :> bytes (z,8 * 8);
              memory :> bytes (word_sub stackpointer (word 40),40)])`,
  WINDOWS_X86_WRAP_STACK_TAC bignum_sqr_4_8_windows_tmc bignum_sqr_4_8_tmc BIGNUM_SQR_4_8_SAFE `[RBP; R12; R13]` 24 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_SQR_4_8_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 40),48) (z,8 * 8) /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_sqr_4_8_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 8);
              memory :> bytes (word_sub stackpointer (word 40),40)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_SQR_4_8_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
