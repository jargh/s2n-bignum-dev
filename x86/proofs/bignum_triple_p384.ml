...
let bignum_triple_p384_tmc = define_trimmed "bignum_triple_p384_tmc" bignum_triple_p384_mc;;

let BIGNUM_TRIPLE_P384_EXEC = X86_MK_CORE_EXEC_RULE bignum_triple_p384_tmc;;
...
let BIGNUM_TRIPLE_P384_CORRECT = time prove
 (`!z x n pc.
        ...
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST bignum_triple_p384_tmc) /\
...
let BIGNUM_TRIPLE_P384_NOIBT_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 8),8) (x,8 * 6) /\
        nonoverlapping (z,8 * 6) (word_sub stackpointer (word 8),16) /\
        ALL (nonoverlapping (word pc,LENGTH bignum_triple_p384_tmc))
            [(z,8 * 6); (word_sub stackpointer (word 8),8)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_triple_p384_tmc /\
                  read RIP s = word pc /\
  ...
  X86_PROMOTE_RETURN_STACK_TAC
    bignum_triple_p384_tmc BIGNUM_TRIPLE_P384_CORRECT
    `[RBX]` 8);;
...
let BIGNUM_TRIPLE_P384_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 8),8) (x,8 * 6) /\
        nonoverlapping (z,8 * 6) (word_sub stackpointer (word 8),16) /\
        ALL (nonoverlapping (word pc,LENGTH bignum_triple_p384_mc))
            [(z,8 * 6); (word_sub stackpointer (word 8),8)]
        ==> ensures x86
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bignum(z,6);
                         memory :> bytes(word_sub stackpointer (word 8),8)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_TRIPLE_P384_NOIBT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let bignum_triple_p384_windows_mc = define_from_elf
   "bignum_triple_p384_windows_mc" "x86/p384/bignum_triple_p384.obj";;

let bignum_triple_p384_windows_tmc = define_trimmed "bignum_triple_p384_windows_tmc" bignum_triple_p384_windows_mc;;

let BIGNUM_TRIPLE_P384_NOIBT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 24),24) (x,8 * 6) /\
        nonoverlapping (z,8 * 6) (word_sub stackpointer (word 24),32) /\
        ALL (nonoverlapping (word pc,LENGTH bignum_triple_p384_windows_tmc))
            [(z,8 * 6); (word_sub stackpointer (word 24),24)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_triple_p384_windows_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
    bignum_triple_p384_windows_tmc bignum_triple_p384_tmc
    BIGNUM_TRIPLE_P384_CORRECT `[RBX]` 8);;

let BIGNUM_TRIPLE_P384_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 24),24) (x,8 * 6) /\
        nonoverlapping (z,8 * 6) (word_sub stackpointer (word 24),32) /\
        ALL (nonoverlapping (word pc,LENGTH bignum_triple_p384_windows_mc))
            [(z,8 * 6); (word_sub stackpointer (word 24),24)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_triple_p384_windows_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [z; x] s /\
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bignum(z,6);
                         memory :> bytes(word_sub stackpointer (word 24),24)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_TRIPLE_P384_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;
...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "bignum_triple_p384" subroutine_signatures)
    BIGNUM_TRIPLE_P384_CORRECT
    BIGNUM_TRIPLE_P384_EXEC;;

let BIGNUM_TRIPLE_P384_SAFE = time prove
 (`exists f_events.
       forall e z x pc.
           nonoverlapping (word pc,239) (z,8 * 6)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST bignum_triple_p384_tmc) /\
                    read RIP s = word (pc + 1) /\
                    C_ARGUMENTS [z; x] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 237) /\
                    (exists e2.
  ...
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars BIGNUM_TRIPLE_P384_EXEC);;

let BIGNUM_TRIPLE_P384_NOIBT_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_triple_p384_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      read events s = APPEND e2 e /\
                      e2 =
                      f_events x z pc (word_sub stackpointer (word 8))
                      returnaddress /\
                      memaccess_inbounds e2
                      [x,48; z,48; word_sub stackpointer (word 8),16]
                      [z,48; word_sub stackpointer (word 8),8]))
            (MAYCHANGE [RSP] ,,
             MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE
             [memory :> bignum (z,6);
              memory :> bytes (word_sub stackpointer (word 8),8)])`,
  X86_PROMOTE_RETURN_STACK_TAC bignum_triple_p384_tmc BIGNUM_TRIPLE_P384_SAFE `[RBX]` 8 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_TRIPLE_P384_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 8),8) (x,8 * 6) /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_triple_p384_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,6);
              memory :> bytes (word_sub stackpointer (word 8),8)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_TRIPLE_P384_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_TRIPLE_P384_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 24),24) (x,8 * 6) /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_triple_p384_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      read events s = APPEND e2 e /\
                      e2 =
                      f_events x z pc (word_sub stackpointer (word 24))
                      returnaddress /\
                      memaccess_inbounds e2
                      [x,48; z,48; word_sub stackpointer (word 24),32]
                      [z,48; word_sub stackpointer (word 24),24]))
            (MAYCHANGE [RSP] ,,
             WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE
             [memory :> bignum (z,6);
              memory :> bytes (word_sub stackpointer (word 24),24)])`,
  WINDOWS_X86_WRAP_STACK_TAC bignum_triple_p384_windows_tmc bignum_triple_p384_tmc BIGNUM_TRIPLE_P384_SAFE `[RBX]` 8 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_TRIPLE_P384_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 24),24) (x,8 * 6) /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_triple_p384_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,6);
              memory :> bytes (word_sub stackpointer (word 24),24)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_TRIPLE_P384_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
