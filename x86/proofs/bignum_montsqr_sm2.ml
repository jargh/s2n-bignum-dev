...
let bignum_montsqr_sm2_mc =
  define_assert_from_elf "bignum_montsqr_sm2_mc" "x86/sm2/bignum_montsqr_sm2.o"
[
...
let bignum_montsqr_sm2_tmc = define_trimmed "bignum_montsqr_sm2_tmc" bignum_montsqr_sm2_mc;;

let BIGNUM_MONTSQR_SM2_EXEC = X86_MK_CORE_EXEC_RULE bignum_montsqr_sm2_tmc;;
...
let BIGNUM_MONTSQR_SM2_CORRECT = time prove
 (`!z x a pc.
        ...
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST bignum_montsqr_sm2_tmc) /\
                  read RIP s = word(pc + 0x0a) /\
                  C_ARGUMENTS [z; x] s /\
                  bignum_from_memory (x,4) s = a)
             (\s. read RIP s = word (pc + 0x1d2) /\
  ...
  ASM_CASES_TAC `a EXP 2 <= 2 EXP 256 * p_sm2` THENL
   [ASM_REWRITE_TAC[]; X86_SIM_TAC BIGNUM_MONTSQR_SM2_EXEC (1--113)] THEN
  ENSURES_INIT_TAC "s0" THEN
  BIGNUM_DIGITIZE_TAC "x_" `bignum_from_memory (x,4) s0` THEN

  X86_ACCSTEPS_TAC BIGNUM_MONTSQR_SM2_EXEC (1--94) (1--95) THEN
  ABBREV_TAC
  ...
  X86_ACCSTEPS_TAC BIGNUM_MONTSQR_SM2_EXEC [99;101;103;104;105] (96--113) THEN
...
let BIGNUM_MONTSQR_SM2_NOIBT_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 48),56) /\
        ALL (nonoverlapping (word_sub stackpointer (word 48),48))
            [(word pc,LENGTH bignum_montsqr_sm2_tmc); (x,8 * 4)] /\
        nonoverlapping (word pc,LENGTH bignum_montsqr_sm2_tmc) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_montsqr_sm2_tmc /\
                  read RIP s = word pc /\
  ...
  X86_PROMOTE_RETURN_STACK_TAC
   bignum_montsqr_sm2_tmc BIGNUM_MONTSQR_SM2_CORRECT
 ...
 (`!z x a pc stackpointer returnaddress.
        ...
        ==> ensures x86
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                       memory :> bytes(word_sub stackpointer (word 48),48)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MONTSQR_SM2_NOIBT_SUBROUTINE_CORRECT));;
...
let BIGNUM_AMONTSQR_SM2_NOIBT_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 48),56) /\
        ALL (nonoverlapping (word_sub stackpointer (word 48),48))
            [(word pc,LENGTH bignum_montsqr_sm2_tmc); (x,8 * 4)] /\
        ...
        ==> ensures x86
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                       memory :> bytes(word_sub stackpointer (word 48),48)])`,
  X86_PROMOTE_RETURN_STACK_TAC
   bignum_montsqr_sm2_tmc BIGNUM_AMONTSQR_SM2_CORRECT
   `[RBX; RBP; R12; R13; R14; R15]` 48);;
 ...
 (`!z x a pc stackpointer returnaddress.
        ...
        ==> ensures x86
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                       memory :> bytes(word_sub stackpointer (word 48),48)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_AMONTSQR_SM2_NOIBT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let bignum_montsqr_sm2_windows_mc = define_from_elf
   "bignum_montsqr_sm2_windows_mc" "x86/sm2/bignum_montsqr_sm2.obj";;

let bignum_montsqr_sm2_windows_tmc = define_trimmed "bignum_montsqr_sm2_windows_tmc" bignum_montsqr_sm2_windows_mc;;

let BIGNUM_MONTSQR_SM2_NOIBT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 64),72) /\
        ALL (nonoverlapping (word_sub stackpointer (word 64),64))
            [(word pc,LENGTH bignum_montsqr_sm2_windows_tmc); (x,8 * 4)] /\
        nonoverlapping (word pc,LENGTH bignum_montsqr_sm2_windows_tmc) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_montsqr_sm2_windows_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
   bignum_montsqr_sm2_windows_tmc bignum_montsqr_sm2_tmc
   BIGNUM_MONTSQR_SM2_CORRECT `[RBX; RBP; R12; R13; R14; R15]` 48);;

let BIGNUM_MONTSQR_SM2_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 64),72) /\
        ALL (nonoverlapping (word_sub stackpointer (word 64),64))
            [(word pc,LENGTH bignum_montsqr_sm2_windows_mc); (x,8 * 4)] /\
        nonoverlapping (word pc,LENGTH bignum_montsqr_sm2_windows_mc) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_montsqr_sm2_windows_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [z; x] s /\
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                       memory :> bytes(word_sub stackpointer (word 64),64)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MONTSQR_SM2_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;

let BIGNUM_AMONTSQR_SM2_NOIBT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 64),72) /\
        ALL (nonoverlapping (word_sub stackpointer (word 64),64))
            [(word pc,LENGTH bignum_montsqr_sm2_windows_tmc); (x,8 * 4)] /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
   bignum_montsqr_sm2_windows_tmc bignum_montsqr_sm2_tmc
   BIGNUM_AMONTSQR_SM2_CORRECT `[RBX; RBP; R12; R13; R14; R15]` 48);;

let BIGNUM_AMONTSQR_SM2_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z x a pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 64),72) /\
        ALL (nonoverlapping (word_sub stackpointer (word 64),64))
            [(word pc,LENGTH bignum_montsqr_sm2_windows_mc); (x,8 * 4)] /\
        ...
        ==> ensures x86
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                       memory :> bytes(word_sub stackpointer (word 64),64)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_AMONTSQR_SM2_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;
...
let BIGNUM_MONTSQR_SM2_SAFE = time prove
 (`exists f_events.
       forall e z x pc.
           nonoverlapping (word pc,477) (z,8 * 4)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST bignum_montsqr_sm2_tmc) /\
                    read RIP s = word (pc + 10) /\
                    C_ARGUMENTS [z; x] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 466) /\
                    (exists e2.
  ...
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars BIGNUM_MONTSQR_SM2_EXEC);;

let BIGNUM_MONTSQR_SM2_NOIBT_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_montsqr_sm2_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      [x,32; z,32; word_sub stackpointer (word 48),56]
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 4);
              memory :> bytes (word_sub stackpointer (word 48),48)])`,
  X86_PROMOTE_RETURN_STACK_TAC bignum_montsqr_sm2_tmc BIGNUM_MONTSQR_SM2_SAFE `[RBX; RBP; R12; R13; R14; R15]` 48 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;
...
exists f_events.
    forall e z x pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_montsqr_sm2_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 4);
              memory :> bytes (word_sub stackpointer (word 48),48)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MONTSQR_SM2_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_MONTSQR_SM2_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 64),72) /\
        ALL (nonoverlapping (word_sub stackpointer (word 64),64))
        [word pc,LENGTH bignum_montsqr_sm2_windows_tmc; x,8 * 4] /\
        nonoverlapping (word pc,LENGTH bignum_montsqr_sm2_windows_tmc)
        (z,8 * 4)
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_montsqr_sm2_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      returnaddress /\
                      memaccess_inbounds e2
                      [x,32; z,32; word_sub stackpointer (word 64),72]
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 4);
              memory :> bytes (word_sub stackpointer (word 64),64)])`,
  WINDOWS_X86_WRAP_STACK_TAC bignum_montsqr_sm2_windows_tmc bignum_montsqr_sm2_tmc BIGNUM_MONTSQR_SM2_SAFE `[RBX; RBP; R12; R13; R14; R15]` 48 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_MONTSQR_SM2_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 64),72) /\
        ALL (nonoverlapping (word_sub stackpointer (word 64),64))
        [word pc,LENGTH bignum_montsqr_sm2_windows_mc; x,8 * 4] /\
        nonoverlapping (word pc,LENGTH bignum_montsqr_sm2_windows_mc)
        (z,8 * 4)
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_montsqr_sm2_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 4);
              memory :> bytes (word_sub stackpointer (word 64),64)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MONTSQR_SM2_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
