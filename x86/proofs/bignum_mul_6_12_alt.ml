...
let bignum_mul_6_12_alt_tmc = define_trimmed "bignum_mul_6_12_alt_tmc" bignum_mul_6_12_alt_mc;;

let BIGNUM_MUL_6_12_ALT_EXEC = X86_MK_CORE_EXEC_RULE bignum_mul_6_12_alt_tmc;;
...
let BIGNUM_MUL_6_12_ALT_CORRECT = time prove
 (`!z x y a b pc.
     ...
     ==> ensures x86
          (\s. bytes_loaded s (word pc) (BUTLAST bignum_mul_6_12_alt_tmc) /\
               read RIP s = word pc /\
               C_ARGUMENTS [z; x; y] s /\
...
let BIGNUM_MUL_6_12_ALT_NOIBT_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
     ALL (nonoverlapping (z,8 * 12))
         [(word pc,LENGTH bignum_mul_6_12_alt_tmc); (x,8 * 6); (y,8 * 6); (stackpointer,8)]
     ==> ensures x86
          (\s. bytes_loaded s (word pc) bignum_mul_6_12_alt_tmc /\
               read RIP s = word pc /\
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(z,8 * 12)])`,
  X86_PROMOTE_RETURN_NOSTACK_TAC bignum_mul_6_12_alt_tmc
     BIGNUM_MUL_6_12_ALT_CORRECT);;

let BIGNUM_MUL_6_12_ALT_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
     ALL (nonoverlapping (z,8 * 12))
         [(word pc,LENGTH bignum_mul_6_12_alt_mc); (x,8 * 6); (y,8 * 6); (stackpointer,8)]
     ==> ensures x86
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(z,8 * 12)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MUL_6_12_ALT_NOIBT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let bignum_mul_6_12_alt_windows_mc = define_from_elf
   "bignum_mul_6_12_alt_windows_mc" "x86/fastmul/bignum_mul_6_12_alt.obj";;

let bignum_mul_6_12_alt_windows_tmc = define_trimmed "bignum_mul_6_12_alt_windows_tmc" bignum_mul_6_12_alt_windows_mc;;

let BIGNUM_MUL_6_12_ALT_NOIBT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
     ALL (nonoverlapping (word_sub stackpointer (word 16),16))
         [(word pc,LENGTH bignum_mul_6_12_alt_windows_tmc); (x,8 * 6); (y,8 * 6)] /\
     ALL (nonoverlapping (z,8 * 12))
         [(word pc,LENGTH bignum_mul_6_12_alt_windows_tmc); (x,8 * 6); (y,8 * 6);
          (word_sub stackpointer (word 16),24)]
     ==> ensures x86
          (\s. bytes_loaded s (word pc) bignum_mul_6_12_alt_windows_tmc /\
               read RIP s = word pc /\
               read RSP s = stackpointer /\
               read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_NOSTACK_TAC
    bignum_mul_6_12_alt_windows_tmc bignum_mul_6_12_alt_tmc
    BIGNUM_MUL_6_12_ALT_CORRECT);;

let BIGNUM_MUL_6_12_ALT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z x y a b pc stackpointer returnaddress.
     ALL (nonoverlapping (word_sub stackpointer (word 16),16))
         [(word pc,LENGTH bignum_mul_6_12_alt_windows_mc); (x,8 * 6); (y,8 * 6)] /\
     ALL (nonoverlapping (z,8 * 12))
         [(word pc,LENGTH bignum_mul_6_12_alt_windows_mc); (x,8 * 6); (y,8 * 6);
          (word_sub stackpointer (word 16),24)]
     ==> ensures x86
          (\s. bytes_loaded s (word pc) bignum_mul_6_12_alt_windows_mc /\
               read RIP s = word pc /\
               read RSP s = stackpointer /\
               read (memory :> bytes64 stackpointer) s = returnaddress /\
               WINDOWS_C_ARGUMENTS [z; x; y] s /\
          ...
          (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(z,8 * 12);
                      memory :> bytes(word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MUL_6_12_ALT_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;
...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "bignum_mul_6_12_alt" subroutine_signatures)
    BIGNUM_MUL_6_12_ALT_CORRECT
    BIGNUM_MUL_6_12_ALT_EXEC;;

let BIGNUM_MUL_6_12_ALT_SAFE = time prove
 (`exists f_events.
       forall e z x y pc.
           ALL (nonoverlapping (z,8 * 12)) [word pc,693; x,8 * 6; y,8 * 6]
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc)
                    (BUTLAST bignum_mul_6_12_alt_tmc) /\
                    read RIP s = word pc /\
                    C_ARGUMENTS [z; x; y] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 692) /\
                    (exists e2.
                         read events s = APPEND e2 e /\
                         e2 = f_events x y z pc /\
                         memaccess_inbounds e2 [x,48; y,48; z,96] [z,96]))
               (MAYCHANGE [RIP; RAX; RCX; RDX; R8; R9; R10] ,,
                MAYCHANGE [memory :> bytes (z,8 * 12)] ,,
                MAYCHANGE SOME_FLAGS ,,
                MAYCHANGE [events])`,
  ASSERT_CONCL_TAC full_spec THEN
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars BIGNUM_MUL_6_12_ALT_EXEC);;

(* Has no "word_sub stackpointer (word ..)"; stackofs is None *)
let BIGNUM_MUL_6_12_ALT_NOIBT_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x y pc stackpointer returnaddress.
        ALL (nonoverlapping (z,8 * 12))
        [word pc,LENGTH bignum_mul_6_12_alt_tmc; x,8 * 6; y,8 * 6;
         stackpointer,8]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_mul_6_12_alt_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x; y] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      [x,48; y,48; z,96; stackpointer,8]
                      [z,96; stackpointer,0]))
            (MAYCHANGE [RSP] ,,
             MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE [memory :> bytes (z,8 * 12)])`,
  X86_PROMOTE_RETURN_NOSTACK_TAC bignum_mul_6_12_alt_tmc BIGNUM_MUL_6_12_ALT_SAFE THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_MUL_6_12_ALT_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x y pc stackpointer returnaddress.
        ALL (nonoverlapping (z,8 * 12))
        [word pc,LENGTH bignum_mul_6_12_alt_mc; x,8 * 6; y,8 * 6;
         stackpointer,8]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_mul_6_12_alt_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x; y] s /\
            ...
            (MAYCHANGE [RSP] ,,
             MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE [memory :> bytes (z,8 * 12)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MUL_6_12_ALT_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_MUL_6_12_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x y pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_mul_6_12_alt_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x; y] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      returnaddress /\
                      memaccess_inbounds e2
                      [x,48; y,48; z,96; word_sub stackpointer (word 16),24]
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 12);
              memory :> bytes (word_sub stackpointer (word 16),16)])`,
  WINDOWS_X86_WRAP_NOSTACK_TAC bignum_mul_6_12_alt_windows_tmc bignum_mul_6_12_alt_tmc BIGNUM_MUL_6_12_ALT_SAFE THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_MUL_6_12_ALT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x y pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_mul_6_12_alt_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x; y] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 12);
              memory :> bytes (word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MUL_6_12_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
