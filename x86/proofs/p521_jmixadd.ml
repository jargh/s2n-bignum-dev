...
(**** print_literal_from_elf "x86/p521/p521_jmixadd.o";;
 ****)

let p521_jmixadd_mc = define_assert_from_elf
  "p521_jmixadd_mc" "x86/p521/p521_jmixadd.o"
[
  ...
  0x48; 0x81; 0xc4; 0xf0; 0x01; 0x00; 0x00;
                           (* ADD (% rsp) (Imm32 (word 496)) *)
  0x41; 0x5f;              (* POP (% r15) *)
  0x41; 0x5e;              (* POP (% r14) *)
  0x41; 0x5d;              (* POP (% r13) *)
  0x41; 0x5c;              (* POP (% r12) *)
...
let p521_jmixadd_tmc = define_trimmed "p521_jmixadd_tmc" p521_jmixadd_mc;;

let P521_JMIXADD_EXEC = X86_MK_CORE_EXEC_RULE p521_jmixadd_tmc;;
...
let LOCAL_SQR_P521_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE p521_jmixadd_tmc) 231 lvs
  `!(t:x86state) pcin pcout p3 n3 p1 n1.
    !n. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 9)) t = n
    ==>
    nonoverlapping (word pc,0x5941) (stackpointer,496) /\
    nonoverlapping (read RDI t,216) (stackpointer,496) /\
    nonoverlapping (read RSI t,216) (stackpointer,496) /\
    nonoverlapping (read RCX t,144) (stackpointer,496) /\
    nonoverlapping (word pc,0x5941) (read RDI t,216)
    ==> ensures x86
         (\s. bytes_loaded s (word pc) (BUTLAST p521_jmixadd_tmc) /\
              read RIP s = pcin /\
           ...
           (MAYCHANGE [RIP; RAX; RBX; RDX; RBP;
            ...
            MAYCHANGE
             [memory :> bytes(word_add (read p3 t) (word n3),8 * 9);
              memory :> bytes(word_add stackpointer (word 432),64)] ,,
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ASM_CASES_TAC `n < p_521` THENL
   [ASM_REWRITE_TAC[]; X86_SIM_TAC P521_JMIXADD_EXEC (1--231)] THEN
  ...
  X86_ACCSTEPS_TAC P521_JMIXADD_EXEC (1--160) (1--160) THEN
  X86_ACCSTEPS_TAC P521_JMIXADD_EXEC (165--189) (161--189) THEN
  SUBGOAL_THEN
  ...
  SUBGOAL_THEN
   `2 EXP 512 * bignum_of_wordlist[h0;h1;h2;h3;h4;h5;h6;h7;h8] +
    bignum_from_memory(word_add stackpointer (word 432),8) s189 =
    n EXP 2`
  ASSUME_TAC THENL
  ...
  X86_STEPS_TAC P521_JMIXADD_EXEC (190--201) THEN
  ...
  X86_ACCSTEPS_TAC P521_JMIXADD_EXEC (202--211) (202--211) THEN
  SUBGOAL_THEN
  ...
  X86_ACCSTEPS_TAC P521_JMIXADD_EXEC (212--231) (212--231) THEN
...
let LOCAL_MUL_P521_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE p521_jmixadd_tmc) 312 lvs
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ASM_CASES_TAC `a < p_521 /\ b < p_521` THENL
   [ASM_REWRITE_TAC[] THEN FIRST_X_ASSUM(CONJUNCTS_THEN ASSUME_TAC);
    X86_SIM_TAC P521_JMIXADD_EXEC (2--313)] THEN
  ...
  X86_ACCSTEPS_TAC P521_JMIXADD_EXEC (2--271) (2--271) THEN
  ...
  SUBGOAL_THEN
   `2 EXP 512 * bignum_of_wordlist[h0;h1;h2;h3;h4;h5;h6;h7;h8] +
    bignum_from_memory(word_add stackpointer (word 432),8) s271 =
    a * b`
  ASSUME_TAC THENL
   [CONV_TAC(ONCE_DEPTH_CONV BIGNUM_LEXPAND_CONV) THEN
    ASM_REWRITE_TAC[GSYM REAL_OF_NUM_CLAUSES; bignum_of_wordlist] THEN
  ...
  X86_STEPS_TAC P521_JMIXADD_EXEC (272--283) THEN
  ...
  X86_ACCSTEPS_TAC P521_JMIXADD_EXEC (284--293) (284--293) THEN
  SUBGOAL_THEN
   `&(val(word_add (word_ushr h8 9) (word_and h0 (word 511):int64))):real =
    &(val(word_ushr h8 9:int64)) + &(val(word_and h0 (word 511):int64))`
  SUBST_ALL_TAC THENL
  ...
  X86_ACCSTEPS_TAC P521_JMIXADD_EXEC (294--313) (294--313) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  CONV_TAC SYM_CONV THEN CONV_TAC(RAND_CONV BIGNUM_LEXPAND_CONV) THEN
  ASM_REWRITE_TAC[] THEN MATCH_MP_TAC EQUAL_FROM_CONGRUENT_MOD_MOD THEN
  MAP_EVERY EXISTS_TAC
...
let LOCAL_SUB_P521_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE p521_jmixadd_tmc) 37 lvs
  `!(t:x86state) pcin pcout p3 n3 p1 n1 p2 n2.
    !m. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 9)) t = m
    ==>
    !n. read(memory :> bytes(word_add (read p2 t) (word n2),8 * 9)) t = n
    ==>
    nonoverlapping (word pc,0x5941) (word_add (read p3 t) (word n3),72)
    ==> ensures x86
         (\s. bytes_loaded s (word pc) (BUTLAST p521_jmixadd_tmc) /\
              read RIP s = pcin /\
              read RSP s = read RSP t /\
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  X86_ACCSTEPS_TAC P521_JMIXADD_EXEC [2;4;6;8;10;12;14;16;18] (1--18) THEN
  ...
  X86_ACCSTEPS_TAC P521_JMIXADD_EXEC
...
let P521_JMIXADD_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer.
        ALL (nonoverlapping (stackpointer,496))
            [(word pc,0x5941); (p1,216); (p2,144); (p3,216)] /\
        nonoverlapping (p3,216) (word pc,0x5941)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST p521_jmixadd_tmc) /\
  ...
  X86_STEPS_TAC P521_JMIXADD_EXEC (20--168) THEN
  CONV_TAC(ONCE_DEPTH_CONV BIGNUM_LEXPAND_CONV) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
...
let P521_JMIXADD_NOIBT_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 544),544))
            [(word pc,LENGTH p521_jmixadd_tmc); (p1,216); (p2,144)] /\
        ALL (nonoverlapping (p3,216))
            [(word pc,LENGTH p521_jmixadd_tmc); (word_sub stackpointer (word 544),552)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) p521_jmixadd_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
  ...
  X86_PROMOTE_RETURN_STACK_TAC p521_jmixadd_tmc P521_JMIXADD_CORRECT
    `[RBX; RBP; R12; R13; R14; R15]` 544);;

let P521_JMIXADD_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 544),544))
            [(word pc,LENGTH p521_jmixadd_mc); (p1,216); (p2,144)] /\
        ALL (nonoverlapping (p3,216))
            [(word pc,LENGTH p521_jmixadd_mc); (word_sub stackpointer (word 544),552)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) p521_jmixadd_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  C_ARGUMENTS [p3; p1; p2] s /\
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(p3,216);
                      memory :> bytes(word_sub stackpointer (word 544),544)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P521_JMIXADD_NOIBT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let p521_jmixadd_windows_mc = define_from_elf "p521_jmixadd_windows_mc"
      "x86/p521/p521_jmixadd.obj";;

let p521_jmixadd_windows_tmc = define_trimmed "p521_jmixadd_windows_tmc" p521_jmixadd_windows_mc;;

let P521_JMIXADD_NOIBT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 560),560))
            [(word pc,LENGTH p521_jmixadd_windows_tmc); (p1,216); (p2,144)] /\
        ALL (nonoverlapping (p3,216))
            [(word pc,LENGTH p521_jmixadd_windows_tmc); (word_sub stackpointer (word 560),568)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) p521_jmixadd_windows_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
   p521_jmixadd_windows_tmc p521_jmixadd_tmc
   P521_JMIXADD_CORRECT
    `[RBX; RBP; R12; R13; R14; R15]` 544);;

let P521_JMIXADD_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 560),560))
            [(word pc,LENGTH p521_jmixadd_windows_mc); (p1,216); (p2,144)] /\
        ALL (nonoverlapping (p3,216))
            [(word pc,LENGTH p521_jmixadd_windows_mc); (word_sub stackpointer (word 560),568)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) p521_jmixadd_windows_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [p3; p1; p2] s /\
          ...
          (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(p3,216);
                      memory :> bytes(word_sub stackpointer (word 560),560)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P521_JMIXADD_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;
...
let P521_JMIXADD_SAFE = time prove
 (`exists f_events.
       forall e p3 p1 p2 pc stackpointer.
           ...
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST p521_jmixadd_tmc) /\
                    read RIP s = word (pc + 0x11) /\
                    read RSP s = stackpointer /\
                    C_ARGUMENTS [p3; p1; p2] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 0x592f) /\
                    (exists e2.
                         read events s = APPEND e2 e /\
                         e2 = f_events p1 p2 p3 pc stackpointer /\
                         memaccess_inbounds e2
                         [p1,216; p2,144; p3,216; stackpointer,496]
                         [p3,216; stackpointer,496]))
               (MAYCHANGE
                ...
                MAYCHANGE [events] ,,
                MAYCHANGE
                [memory :> bytes (p3,216); memory :> bytes (stackpointer,496)])`,
  ASSERT_CONCL_TAC full_spec THEN
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars P521_JMIXADD_EXEC);;

let P521_JMIXADD_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 544),544))
        [word pc,LENGTH p521_jmixadd_tmc; p1,216; p2,144] /\
        ALL (nonoverlapping (p3,216))
        [word pc,LENGTH p521_jmixadd_tmc;
         word_sub stackpointer (word 544),552]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p521_jmixadd_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      read events s = APPEND e2 e /\
                      e2 =
                      f_events p1 p2 p3 pc (word_sub stackpointer (word 544))
                      returnaddress /\
                      memaccess_inbounds e2
                      [p1,216; p2,144; p3,216;
                       word_sub stackpointer (word 544),552]
                      [p3,216; word_sub stackpointer (word 544),544]))
            (MAYCHANGE [RSP] ,,
             MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE
             [memory :> bytes (p3,216);
              memory :> bytes (word_sub stackpointer (word 544),544)])`,
  X86_PROMOTE_RETURN_STACK_TAC p521_jmixadd_tmc P521_JMIXADD_SAFE
    `[RBX; RBP; R12; R13; R14; R15]` 544
...
let P521_JMIXADD_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ...
        [word pc,LENGTH p521_jmixadd_mc; p1,216; p2,144] /\
        ALL (nonoverlapping (p3,216))
        [word pc,LENGTH p521_jmixadd_mc;
         word_sub stackpointer (word 544),552]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p521_jmixadd_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,216);
              memory :> bytes (word_sub stackpointer (word 544),544)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P521_JMIXADD_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let P521_JMIXADD_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 560),560))
        [word pc,LENGTH p521_jmixadd_windows_tmc; p1,216; p2,144] /\
        ALL (nonoverlapping (p3,216))
        [word pc,LENGTH p521_jmixadd_windows_tmc;
         word_sub stackpointer (word 560),568]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p521_jmixadd_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      read events s = APPEND e2 e /\
                      e2 =
                      f_events p1 p2 p3 pc (word_sub stackpointer (word 560))
                      returnaddress /\
                      memaccess_inbounds e2
                      [p1,216; p2,144; p3,216;
                       word_sub stackpointer (word 560),568]
                      [p3,216; word_sub stackpointer (word 560),560]))
            (MAYCHANGE [RSP] ,,
             WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE
             [memory :> bytes (p3,216);
              memory :> bytes (word_sub stackpointer (word 560),560)])`,
  WINDOWS_X86_WRAP_STACK_TAC p521_jmixadd_windows_tmc p521_jmixadd_tmc
    P521_JMIXADD_SAFE `[RBX; RBP; R12; R13; R14; R15]` 544
  THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let P521_JMIXADD_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 560),560))
        [word pc,LENGTH p521_jmixadd_windows_mc; p1,216; p2,144] /\
        ALL (nonoverlapping (p3,216))
        [word pc,LENGTH p521_jmixadd_windows_mc;
         word_sub stackpointer (word 560),568]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p521_jmixadd_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,216);
              memory :> bytes (word_sub stackpointer (word 560),560)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P521_JMIXADD_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
