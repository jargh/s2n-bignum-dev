...
let bignum_cmul_p25519_tmc = define_trimmed "bignum_cmul_p25519_tmc" bignum_cmul_p25519_mc;;

let BIGNUM_CMUL_P25519_EXEC = X86_MK_CORE_EXEC_RULE bignum_cmul_p25519_tmc;;
...
let BIGNUM_CMUL_P25519_CORRECT = time prove
 (`!z c x a pc.
        ...
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST bignum_cmul_p25519_tmc) /\
                  read RIP s = word pc /\
                  C_ARGUMENTS [z; c; x] s /\
...
let BIGNUM_CMUL_P25519_NOIBT_SUBROUTINE_CORRECT = time prove
 (`!z c x a pc stackpointer returnaddress.
        nonoverlapping (word pc,LENGTH bignum_cmul_p25519_tmc) (z,8 * 4) /\
        nonoverlapping (stackpointer,8) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_cmul_p25519_tmc /\
                  read RIP s = word pc /\
  ...
  X86_PROMOTE_RETURN_NOSTACK_TAC bignum_cmul_p25519_tmc
    BIGNUM_CMUL_P25519_CORRECT);;

let BIGNUM_CMUL_P25519_SUBROUTINE_CORRECT = time prove
 (`!z c x a pc stackpointer returnaddress.
        nonoverlapping (word pc,LENGTH bignum_cmul_p25519_mc) (z,8 * 4) /\
        nonoverlapping (stackpointer,8) (z,8 * 4)
        ==> ensures x86
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_CMUL_P25519_NOIBT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let bignum_cmul_p25519_windows_mc = define_from_elf
   "bignum_cmul_p25519_windows_mc" "x86/curve25519/bignum_cmul_p25519.obj";;

let bignum_cmul_p25519_windows_tmc = define_trimmed "bignum_cmul_p25519_windows_tmc" bignum_cmul_p25519_windows_mc;;

let BIGNUM_CMUL_P25519_NOIBT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z c x a pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,LENGTH bignum_cmul_p25519_windows_tmc); (x,8 * 4)] /\
        nonoverlapping (word pc,LENGTH bignum_cmul_p25519_windows_tmc) (z,8 * 4) /\
        nonoverlapping (word_sub stackpointer (word 16),24) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_cmul_p25519_windows_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_NOSTACK_TAC bignum_cmul_p25519_windows_tmc
    bignum_cmul_p25519_tmc BIGNUM_CMUL_P25519_CORRECT);;

let BIGNUM_CMUL_P25519_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z c x a pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
            [(word pc,LENGTH bignum_cmul_p25519_windows_mc); (x,8 * 4)] /\
        nonoverlapping (word pc,LENGTH bignum_cmul_p25519_windows_mc) (z,8 * 4) /\
        nonoverlapping (word_sub stackpointer (word 16),24) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_cmul_p25519_windows_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [z; c; x] s /\
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                         memory :> bytes(word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_CMUL_P25519_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;
...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "bignum_cmul_p25519" subroutine_signatures)
    BIGNUM_CMUL_P25519_CORRECT
    BIGNUM_CMUL_P25519_EXEC;;

let BIGNUM_CMUL_P25519_SAFE = time prove
 (`exists f_events.
       forall e z c x pc.
           nonoverlapping (word pc,114) (z,8 * 4)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST bignum_cmul_p25519_tmc) /\
                    read RIP s = word pc /\
                    C_ARGUMENTS [z; c; x] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 113) /\
                    (exists e2.
  ...
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars BIGNUM_CMUL_P25519_EXEC);;

(* Has no "word_sub stackpointer (word ..)"; stackofs is None *)
let BIGNUM_CMUL_P25519_NOIBT_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z c x pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_cmul_p25519_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; c; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             MAYCHANGE [memory :> bytes (z,8 * 4)])`,
  X86_PROMOTE_RETURN_NOSTACK_TAC bignum_cmul_p25519_tmc BIGNUM_CMUL_P25519_SAFE THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_CMUL_P25519_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z c x pc stackpointer returnaddress.
        nonoverlapping (word pc,LENGTH bignum_cmul_p25519_mc) (z,8 * 4) /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_cmul_p25519_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; c; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE [memory :> bytes (z,8 * 4)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_CMUL_P25519_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_CMUL_P25519_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z c x pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
        [word pc,LENGTH bignum_cmul_p25519_windows_tmc; x,8 * 4] /\
        nonoverlapping (word pc,LENGTH bignum_cmul_p25519_windows_tmc)
        (z,8 * 4) /\
        nonoverlapping (word_sub stackpointer (word 16),24) (z,8 * 4)
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_cmul_p25519_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; c; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 4);
              memory :> bytes (word_sub stackpointer (word 16),16)])`,
  WINDOWS_X86_WRAP_NOSTACK_TAC bignum_cmul_p25519_windows_tmc bignum_cmul_p25519_tmc BIGNUM_CMUL_P25519_SAFE THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_CMUL_P25519_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z c x pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
        [word pc,LENGTH bignum_cmul_p25519_windows_mc; x,8 * 4] /\
        nonoverlapping (word pc,LENGTH bignum_cmul_p25519_windows_mc)
        (z,8 * 4) /\
        nonoverlapping (word_sub stackpointer (word 16),24) (z,8 * 4)
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_cmul_p25519_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; c; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 4);
              memory :> bytes (word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_CMUL_P25519_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
