...
let bignum_sqr_p25519_alt_tmc = define_trimmed "bignum_sqr_p25519_alt_tmc" bignum_sqr_p25519_alt_mc;;

let BIGNUM_SQR_P25519_ALT_EXEC = X86_MK_CORE_EXEC_RULE bignum_sqr_p25519_alt_tmc;;
...
let BIGNUM_SQR_P25519_ALT_CORRECT = time prove
 (`!z x n pc.
        nonoverlapping (word pc,0x19d) (z,8 * 4)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST bignum_sqr_p25519_alt_tmc) /\
                  read RIP s = word(pc + 0x8) /\
                  C_ARGUMENTS [z; x] s /\
                  bignum_from_memory (x,4) s = n)
             (\s. read RIP s = word (pc + 0x194) /\
  ...
  X86_ACCSTEPS_TAC BIGNUM_SQR_P25519_ALT_EXEC (1--72) (1--72) THEN
  ...
  MAP_EVERY (fun n ->
    X86_STEPS_TAC BIGNUM_SQR_P25519_ALT_EXEC [n] THEN
...
let BIGNUM_SQR_P25519_ALT_NOIBT_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        nonoverlapping (word pc,LENGTH bignum_sqr_p25519_alt_tmc) (z,8 * 4) /\
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 32),40) /\
        ALL (nonoverlapping (word_sub stackpointer (word 32),32))
            [(word pc,LENGTH bignum_sqr_p25519_alt_tmc); (x,8 * 4)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_sqr_p25519_alt_tmc /\
                  read RIP s = word pc /\
  ...
  X86_PROMOTE_RETURN_STACK_TAC
   bignum_sqr_p25519_alt_tmc BIGNUM_SQR_P25519_ALT_CORRECT
   `[R12; R13; R14; R15]` 32);;

let BIGNUM_SQR_P25519_ALT_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        nonoverlapping (word pc,LENGTH bignum_sqr_p25519_alt_mc) (z,8 * 4) /\
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 32),40) /\
        ALL (nonoverlapping (word_sub stackpointer (word 32),32))
            [(word pc,LENGTH bignum_sqr_p25519_alt_mc); (x,8 * 4)]
        ==> ensures x86
             ...
             (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                     memory :> bytes(word_sub stackpointer (word 32),32)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_SQR_P25519_ALT_NOIBT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let bignum_sqr_p25519_alt_windows_mc = define_from_elf
   "bignum_sqr_p25519_alt_windows_mc" "x86/curve25519/bignum_sqr_p25519_alt.obj";;

let bignum_sqr_p25519_alt_windows_tmc = define_trimmed "bignum_sqr_p25519_alt_windows_tmc" bignum_sqr_p25519_alt_windows_mc;;

let BIGNUM_SQR_P25519_ALT_NOIBT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        nonoverlapping (word pc,LENGTH bignum_sqr_p25519_alt_windows_tmc) (z,8 * 4) /\
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 48),56) /\
        ALL (nonoverlapping (word_sub stackpointer (word 48),48))
            [(word pc,LENGTH bignum_sqr_p25519_alt_windows_tmc); (x,8 * 4)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_sqr_p25519_alt_windows_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
   bignum_sqr_p25519_alt_windows_tmc bignum_sqr_p25519_alt_tmc
   BIGNUM_SQR_P25519_ALT_CORRECT `[R12; R13; R14; R15]` 32);;

let BIGNUM_SQR_P25519_ALT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!z x n pc stackpointer returnaddress.
        nonoverlapping (word pc,LENGTH bignum_sqr_p25519_alt_windows_mc) (z,8 * 4) /\
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 48),56) /\
        ALL (nonoverlapping (word_sub stackpointer (word 48),48))
            [(word pc,LENGTH bignum_sqr_p25519_alt_windows_mc); (x,8 * 4)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) bignum_sqr_p25519_alt_windows_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [z; x] s /\
             ...
             (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
              MAYCHANGE [memory :> bytes(z,8 * 4);
                     memory :> bytes(word_sub stackpointer (word 48),48)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_SQR_P25519_ALT_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;
...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "bignum_sqr_p25519_alt" subroutine_signatures)
    BIGNUM_SQR_P25519_ALT_CORRECT
    BIGNUM_SQR_P25519_ALT_EXEC;;

let BIGNUM_SQR_P25519_ALT_SAFE = time prove
 (`exists f_events.
       forall e z x pc.
           nonoverlapping (word pc,413) (z,8 * 4)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc)
                    (BUTLAST bignum_sqr_p25519_alt_tmc) /\
                    read RIP s = word (pc + 8) /\
                    C_ARGUMENTS [z; x] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 404) /\
                    (exists e2.
                         read events s = APPEND e2 e /\
                         e2 = f_events x z pc /\
                         memaccess_inbounds e2 [x,32; z,32] [z,32]))
               (MAYCHANGE
                [RIP; RSI; RAX; RCX; RDX; R8; R9; R10; R11; R12; R13; R14;
                 R15] ,,
                MAYCHANGE SOME_FLAGS ,,
                MAYCHANGE [events] ,,
                MAYCHANGE [memory :> bignum (z,4)])`,
  ASSERT_CONCL_TAC full_spec THEN
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars BIGNUM_SQR_P25519_ALT_EXEC);;

let BIGNUM_SQR_P25519_ALT_NOIBT_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_sqr_p25519_alt_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      read events s = APPEND e2 e /\
                      e2 =
                      f_events x z pc (word_sub stackpointer (word 32))
                      returnaddress /\
                      memaccess_inbounds e2
                      [x,32; z,32; word_sub stackpointer (word 32),40]
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 4);
              memory :> bytes (word_sub stackpointer (word 32),32)])`,
  X86_PROMOTE_RETURN_STACK_TAC bignum_sqr_p25519_alt_tmc BIGNUM_SQR_P25519_ALT_SAFE `[R12; R13; R14; R15]` 32 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_SQR_P25519_ALT_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        nonoverlapping (word pc,LENGTH bignum_sqr_p25519_alt_mc) (z,8 * 4) /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_sqr_p25519_alt_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 4);
              memory :> bytes (word_sub stackpointer (word 32),32)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_SQR_P25519_ALT_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_SQR_P25519_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        nonoverlapping (word pc,LENGTH bignum_sqr_p25519_alt_windows_tmc)
        (z,8 * 4) /\
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 48),56) /\
        ALL (nonoverlapping (word_sub stackpointer (word 48),48))
        [word pc,LENGTH bignum_sqr_p25519_alt_windows_tmc; x,8 * 4]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_sqr_p25519_alt_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 4);
              memory :> bytes (word_sub stackpointer (word 48),48)])`,
  WINDOWS_X86_WRAP_STACK_TAC bignum_sqr_p25519_alt_windows_tmc bignum_sqr_p25519_alt_tmc BIGNUM_SQR_P25519_ALT_SAFE `[R12; R13; R14; R15]` 32 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_SQR_P25519_ALT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        nonoverlapping (word pc,LENGTH bignum_sqr_p25519_alt_windows_mc)
        (z,8 * 4) /\
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 48),56) /\
        ALL (nonoverlapping (word_sub stackpointer (word 48),48))
        [word pc,LENGTH bignum_sqr_p25519_alt_windows_mc; x,8 * 4]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_sqr_p25519_alt_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 4);
              memory :> bytes (word_sub stackpointer (word 48),48)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_SQR_P25519_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
