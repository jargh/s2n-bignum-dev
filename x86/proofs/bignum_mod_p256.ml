...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "bignum_mod_p256" subroutine_signatures)
    BIGNUM_MOD_P256_CORRECT
    BIGNUM_MOD_P256_EXEC;;

(* Containment of a word-indexed byte access buf[j..j+7] within [buf, n * 8]
   whenever j + 8 <= n * 8.  Used for the four x[.] loads in each phase. *)
...
let rec MODP256_CONTAIN_TAC g =
  (FIRST [ MATCH_MP_TAC CONTAINED_GEN THEN SIMPLE_ARITH_TAC;
           DISJ1_TAC THEN MODP256_CONTAIN_TAC; DISJ2_TAC THEN MODP256_CONTAIN_TAC ]) g;;

let CONS_APPEND_SINGLETON = prove
 (`!(h:A) t. CONS h t = APPEND [h] t`, REWRITE_TAC[APPEND]);;

(* The main-loop load reads x[i-1] as word_add x (word (8*(k'-4-i)) - 8),
   with the "- 8" appearing as + (2 EXP 64 - 8) after modular subtraction. *)
...
(* The safety proof is parameterized by the executable decoding rule and a
   byte-offset function `off`, exactly like the functional-correctness `tac`
   above.  It is instantiated once for the System V core (off n = pc + n) and
   once for the Windows core (off n = pc + (n + 11), skipping the 11-byte
   Windows entry shim).  Instruction step counts are fixed since both cores
   share identical bodies. *)
let MODP256_SAFE_TAC spec execth off =
  ...
  (*** k < 4 special cases: writeback of k words directly ***)
  ASM_CASES_TAC `k' < 4` THENL
  ...
  (*** Prologue: read the top four digits x[k-4..k-1] ***)
  ...
  CONJ_TAC THENL
   [ ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC execth (1--21) THEN
     ...
     ASM_REWRITE_TAC[] THEN
     REWRITE_TAC[memaccess_inbounds; ALL; EX; FST; SND] THEN REPEAT CONJ_TAC THEN MODP256_CONTAIN_TAC;
     ALL_TAC ] THEN

  (*** k = 4 (no loop) vs 4 < k (main reduce loop) ***)
  FIRST_ASSUM(STRIP_ASSUME_TAC o MATCH_MP (ARITH_RULE
   ...
   [ ASM_REWRITE_TAC[SUB_REFL; MULT_0] THEN
     ...
     ALL_TAC ] THEN
  ASM_REWRITE_TAC[] THEN
  ENSURES_EVENTS_WHILE_UP2_TAC `k' - 4` (off 0x63) (off 0xef)
   `\i s. read RDI s = z /\ read RCX s = x /\ read RSI s = word((k' - 4) - i)` THEN
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [ (*** loop is nonempty ***)
     SIMPLE_ARITH_TAC;
     (*** pre-block: the loop-entry test/branch ***)
     ENSURES_INIT_TAC "s0" THEN STRIP_EXISTS_ASSUM_TAC THEN
     ...
     ASM_REWRITE_TAC[] THEN
     REWRITE_TAC[memaccess_inbounds; ALL; EX; FST; SND] THEN REPEAT CONJ_TAC THEN MODP256_CONTAIN_TAC;
     (*** loop body: reduce one digit ***)
     X_GEN_TAC `i:num` THEN STRIP_TAC THEN
     ENSURES_INIT_TAC "s0" THEN STRIP_EXISTS_ASSUM_TAC THEN
     X86_STEPS_TAC execth (1--36) THEN
     ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
     ...
     (*** post-block: final writeback ***)
     REWRITE_TAC[SUB_REFL] THEN
     ENSURES_INIT_TAC "s0" THEN STRIP_EXISTS_ASSUM_TAC THEN
     X86_STEPS_TAC execth (1--4) THEN
     ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN DISCHARGE_SAFETY_PROPERTY_TAC ];;

let BIGNUM_MOD_P256_SAFE = prove(full_spec,
  MODP256_SAFE_TAC full_spec BIGNUM_MOD_P256_EXEC
    (fun n -> mk_comb(`(+) (pc:num)`, mk_small_numeral n)));;

let BIGNUM_MOD_P256_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e z k x pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_mod_p256_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; k; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,4);
              memory :> bytes (word_sub stackpointer (word 16),16)])`,
  X86_ADD_RETURN_STACK_TAC BIGNUM_MOD_P256_EXEC BIGNUM_MOD_P256_SAFE
    `[RBX; R12]` 16 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_MOD_P256_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e z k x pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 16),24) (z,32) /\
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_mod_p256_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; k; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,4);
              memory :> bytes (word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MOD_P256_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

(* The Windows binary does not embed the System V core byte-for-byte, so we
   prove a safety spec for the Windows core directly (reusing MODP256_SAFE_TAC
   with the +11 shim offset) and promote it, exactly mirroring the functional
   correctness treatment of the Windows variant. *)
let BIGNUM_MOD_P256_WINDOWS_SAFE = prove
 (`exists f_events.
     forall e z k x pc.
         nonoverlapping (word pc,311) (z,32)
         ==> ensures x86
             (\s.
                  bytes_loaded s (word pc) bignum_mod_p256_windows_tmc /\
                  read RIP s = word (pc + 14) /\
                  C_ARGUMENTS [z; k; x] s /\
                  read events s = e)
             (\s.
                  read RIP s = word (pc + 265) /\
                  (exists e2.
                       read events s = APPEND e2 e /\
       ...
       forall e z k x pc.
           ...
           ==> ensures x86
               ...
               (MAYCHANGE [RIP; RSI; RAX; RDX; RCX; RBX; R8; R9; R10; R11; R12] ,,
                ...
                MAYCHANGE [events] ,,
                MAYCHANGE [memory :> bignum (z,4)])`
    (X86_MK_EXEC_RULE bignum_mod_p256_windows_tmc)
    (fun n -> mk_comb(`(+) (pc:num)`, mk_small_numeral (n + 11))));;

let BIGNUM_MOD_P256_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e z k x pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 32),40) (z,32) /\
        ALL (nonoverlapping (word_sub stackpointer (word 32),32))
        ...
        ==> ensures x86
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,4);
              memory :> bytes (word_sub stackpointer (word 32),32)])`,
  GEN_X86_ADD_RETURN_STACK_TAC (X86_MK_EXEC_RULE bignum_mod_p256_windows_tmc)
    BIGNUM_MOD_P256_WINDOWS_SAFE `[RDI; RSI; RBX; R12]` 32 (7,5) THEN
  DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_MOD_P256_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e z k x pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 32),40) (z,32) /\
        ALL (nonoverlapping (word_sub stackpointer (word 32),32))
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_mod_p256_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; k; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,4);
              memory :> bytes (word_sub stackpointer (word 32),32)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MOD_P256_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
