 ...
 (`exists f_events.
       forall e a pc stackpointer returnaddress.
          ...
          ==> ensures x86
               ...
               (\s s'. true)`,
  X86_PROMOTE_RETURN_NOSTACK_TAC mlkem_tomont_tmc MLKEM_TOMONT_SAFE THEN
...
let MLKEM_TOMONT_NOIBT_WINDOWS_SUBROUTINE_SAFE = prove
 (`exists f_events. forall e a pc stackpointer returnaddress.
        ...
        ==> ensures x86
              (\s.
                  bytes_loaded s (word pc) mlkem_tomont_windows_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [a] s /\
  ...
  ASSUME_CALLEE_SAFETY_TAC MLKEM_TOMONT_SAFE "H_subth" THEN
...
let MLKEM_TOMONT_WINDOWS_SUBROUTINE_SAFE = prove
 (`exists f_events. forall e a pc stackpointer returnaddress.
        aligned 32 a /\
        nonoverlapping (word pc, LENGTH mlkem_tomont_windows_mc) (a, 512) /\
        nonoverlapping (word_sub stackpointer (word 176), 184) (a, 512) /\
        ...
        ==> ensures x86
              (\s.
                  bytes_loaded s (word pc) mlkem_tomont_windows_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [a] s /\
              ...
              (MAYCHANGE [RSP] ,,
               ...
               MAYCHANGE [memory :> bytes(word_sub stackpointer (word 176), 176)] ,,
               MAYCHANGE [memory :> bytes(a, 512)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE MLKEM_TOMONT_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
