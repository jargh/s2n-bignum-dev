...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "bignum_deamont_p256" subroutine_signatures)
    BIGNUM_DEAMONT_P256_CORRECT
    BIGNUM_DEAMONT_P256_EXEC;;

let BIGNUM_DEAMONT_P256_SAFE = time prove
 (`exists f_events.
       forall e z x pc.
           nonoverlapping (word pc,310) (z,8 * 4)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc)
                    (BUTLAST bignum_deamont_p256_tmc) /\
                    read RIP s = word (pc + 1) /\
                    C_ARGUMENTS [z; x] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 308) /\
                    (exists e2.
  ...
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars BIGNUM_DEAMONT_P256_EXEC);;

let BIGNUM_DEAMONT_P256_NOIBT_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_deamont_p256_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      [x,32; z,32; word_sub stackpointer (word 8),16]
                      [z,32; word_sub stackpointer (word 8),8]))
            (MAYCHANGE [RSP] ,,
             MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE
             [memory :> bytes (z,8 * 4);
              memory :> bytes (word_sub stackpointer (word 8),8)])`,
  X86_PROMOTE_RETURN_STACK_TAC bignum_deamont_p256_tmc BIGNUM_DEAMONT_P256_SAFE `[RBX]` 8 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_DEAMONT_P256_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 8),16) /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_deamont_p256_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 4);
              memory :> bytes (word_sub stackpointer (word 8),8)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_DEAMONT_P256_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_DEAMONT_P256_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 24),32) /\
        ALL (nonoverlapping (word_sub stackpointer (word 24),24))
        [word pc,LENGTH bignum_deamont_p256_windows_tmc; x,8 * 4] /\
        nonoverlapping (word pc,LENGTH bignum_deamont_p256_windows_tmc)
        (z,8 * 4)
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_deamont_p256_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      returnaddress /\
                      memaccess_inbounds e2
                      [x,32; z,32; word_sub stackpointer (word 24),32]
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 4);
              memory :> bytes (word_sub stackpointer (word 24),24)])`,
  WINDOWS_X86_WRAP_STACK_TAC bignum_deamont_p256_windows_tmc bignum_deamont_p256_tmc BIGNUM_DEAMONT_P256_SAFE `[RBX]` 8 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_DEAMONT_P256_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 24),32) /\
        ALL (nonoverlapping (word_sub stackpointer (word 24),24))
        [word pc,LENGTH bignum_deamont_p256_windows_mc; x,8 * 4] /\
        nonoverlapping (word pc,LENGTH bignum_deamont_p256_windows_mc)
        (z,8 * 4)
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_deamont_p256_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 4);
              memory :> bytes (word_sub stackpointer (word 24),24)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_DEAMONT_P256_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
