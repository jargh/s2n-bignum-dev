...
[
  ...
  0x0f; 0x85; 0x65; 0xff; 0xff; 0xff;
                           (* JNE (Imm32 (word 4294967141)) *)
...
let tac execth offset =
  ...
  ENSURES_WHILE_PDOWN_TAC `k - 6` (offset 0x87) (offset 0x11c)
  ...
  SUBGOAL_THEN
   `2 EXP 64 * bitval(read CF s2) + val(read RBX s2) = m DIV 2 EXP 384 + 1`
  MP_TAC THENL
   [EXPAND_TAC "m" THEN
  ...
  SUBGOAL_THEN
   `sum_s22:int64 = word_neg(word(bitval(m < val(w:int64) * p_384))) /\
    &(bignum_of_wordlist
       [sum_s9; sum_s17; sum_s18; sum_s19; sum_s20; sum_s21]) =
...
let BIGNUM_MOD_P384_ALT_CORRECT = time prove
 (`!z k x n pc.
...
let BIGNUM_MOD_P384_ALT_SUBROUTINE_CORRECT = time prove
 (`!z k x n pc stackpointer returnaddress.
      nonoverlapping (word_sub stackpointer (word 32),40) (z,48) /\
      ALL (nonoverlapping (word_sub stackpointer (word 32),32))
          [(word pc,LENGTH bignum_mod_p384_alt_mc); (x, 8 * val k)] /\
      nonoverlapping (word pc,LENGTH bignum_mod_p384_alt_mc) (z,48)
      ==> ensures x86
...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "bignum_mod_p384_alt" subroutine_signatures)
    BIGNUM_MOD_P384_ALT_CORRECT
    BIGNUM_MOD_P384_ALT_EXEC;;
...
let MODP_SAFE_TAC spec execth off =
  ...
  FIRST_ASSUM(STRIP_ASSUME_TAC o MATCH_MP (ARITH_RULE
    `~(k < 6) ==> ~(k = 0) /\ ~(k = 1) /\ ~(k = 2) /\ ~(k = 3) /\ ~(k = 4) /\ ~(k = 5)`)) THEN
  ASM_REWRITE_TAC[] THEN
  ENSURES_EVENTS_SEQUENCE_TAC (off 0x7e)
   `\s. read RDI s = z /\ read RCX s = x /\ read RSI s = word(k' - 6)` THEN
  CONJ_TAC THENL
   [ ENSURES_INIT_TAC "s0" THEN X86_STEPS_TAC execth (1--29) THEN
     ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  ...
  FIRST_ASSUM(STRIP_ASSUME_TAC o MATCH_MP (ARITH_RULE
   ...
   [ ASM_REWRITE_TAC[SUB_REFL; MULT_0] THEN
     ...
     ALL_TAC ] THEN
  ASM_REWRITE_TAC[] THEN
  ENSURES_EVENTS_WHILE_UP2_TAC `k' - 6` (off 0x87) (off 0x122)
  ...
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [ SIMPLE_ARITH_TAC;
     ...
     ENSURES_INIT_TAC "s0" THEN STRIP_EXISTS_ASSUM_TAC THEN
     X86_STEPS_TAC execth (1--41) THEN
     ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
...
let BIGNUM_MOD_P384_ALT_SAFE = prove(full_spec,
  MODP_SAFE_TAC full_spec BIGNUM_MOD_P384_ALT_EXEC
    (fun n -> mk_comb(`(+) (pc:num)`, mk_small_numeral n)));;

let BIGNUM_MOD_P384_ALT_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e z k x pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_mod_p384_alt_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; k; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,6);
              memory :> bytes (word_sub stackpointer (word 32),32)])`,
  X86_ADD_RETURN_STACK_TAC BIGNUM_MOD_P384_ALT_EXEC BIGNUM_MOD_P384_ALT_SAFE
    `[RBX; R12; R13; R14]` 32 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_MOD_P384_ALT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e z k x pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 32),40) (z,48) /\
        ALL (nonoverlapping (word_sub stackpointer (word 32),32))
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_mod_p384_alt_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; k; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,6);
              memory :> bytes (word_sub stackpointer (word 32),32)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MOD_P384_ALT_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_MOD_P384_ALT_WINDOWS_SAFE = prove
 (`exists f_events.
     forall e z k x pc.
         nonoverlapping (word pc,398) (z,48)
         ==> ensures x86
             (\s.
                  bytes_loaded s (word pc) bignum_mod_p384_alt_windows_tmc /\
                  read RIP s = word (pc + 18) /\
                  C_ARGUMENTS [z; k; x] s /\
                  read events s = e)
             (\s.
                  read RIP s = word (pc + 324) /\
                  (exists e2.
                       read events s = APPEND e2 e /\
       ...
       forall e z k x pc.
           ...
           ==> ensures x86
               ...
               (MAYCHANGE [RIP; RSI; RAX; RDX; RCX; RBX; R8; R9; R10; R11; R12; R13; R14] ,,
                ...
                MAYCHANGE [events] ,,
                MAYCHANGE [memory :> bignum (z,6)])`
    (X86_MK_EXEC_RULE bignum_mod_p384_alt_windows_tmc)
    (fun n -> mk_comb(`(+) (pc:num)`, mk_small_numeral (n + 11))));;

let BIGNUM_MOD_P384_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e z k x pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 48),56) (z,48) /\
        ALL (nonoverlapping (word_sub stackpointer (word 48),48))
        ...
        ==> ensures x86
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,6);
              memory :> bytes (word_sub stackpointer (word 48),48)])`,
  GEN_X86_ADD_RETURN_STACK_TAC (X86_MK_EXEC_RULE bignum_mod_p384_alt_windows_tmc)
    BIGNUM_MOD_P384_ALT_WINDOWS_SAFE `[RDI; RSI; RBX; R12; R13; R14]` 48 (9,7) THEN
  DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_MOD_P384_ALT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e z k x pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 48),56) (z,48) /\
        ALL (nonoverlapping (word_sub stackpointer (word 48),48))
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_mod_p384_alt_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; k; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,6);
              memory :> bytes (word_sub stackpointer (word 48),48)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MOD_P384_ALT_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
