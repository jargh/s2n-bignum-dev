...
print_literal_from_elf "x86/mlkem/mlkem_basemul_k3.o";;

let mlkem_basemul_k3_mc =
  define_assert_from_elf "mlkem_basemul_k3_mc" "x86/mlkem/mlkem_basemul_k3.o"
[
...
let mlkem_basemul_k3_windows_mc = define_from_elf
    "mlkem_basemul_k3_windows_mc" "x86/mlkem/mlkem_basemul_k3.obj";;

let mlkem_basemul_k3_windows_tmc = define_trimmed
    "mlkem_basemul_k3_windows_tmc" mlkem_basemul_k3_windows_mc;;

let mlkem_basemul_k3_windows_tmc_EXEC = X86_MK_EXEC_RULE mlkem_basemul_k3_windows_tmc;;

let MLKEM_BASEMUL_K3_NOIBT_WINDOWS_SUBROUTINE_CORRECT = prove(
  `!src1 src2 src2t dst a0 b0 c0 d0 dz0 a1 b1 c1 d1 dz1 a2 b2 c2 d2 dz2 pc stackpointer returnaddress.
        aligned 32 src1 /\
        aligned 32 src2 /\
        aligned 32 src2t /\
        aligned 32 dst /\
        nonoverlapping (word pc, LENGTH mlkem_basemul_k3_windows_tmc)
                       (word_sub stackpointer (word 176), 176) /\
        nonoverlapping (src1, 1536)
                       (word_sub stackpointer (word 176), 176) /\
        nonoverlapping (src2, 1536)
                       (word_sub stackpointer (word 176), 176) /\
        nonoverlapping (src2t, 768)
                       (word_sub stackpointer (word 176), 176) /\
        nonoverlapping (dst, 512) (src1, 1536) /\
        nonoverlapping (dst, 512) (src2, 1536) /\
        nonoverlapping (dst, 512) (src2t, 768) /\
        nonoverlapping (dst, 512) (word pc, LENGTH mlkem_basemul_k3_windows_tmc) /\
        nonoverlapping (dst, 512) (word_sub stackpointer (word 176), 184)
        ==> ensures x86 
              (\s. bytes_loaded s (word pc) mlkem_basemul_k3_windows_tmc /\
                   read RIP s = word pc /\
                   read RSP s = stackpointer /\
                   read (memory :> bytes64 stackpointer) s = returnaddress /\
              ...
              (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
               ...
               MAYCHANGE [memory :> bytes(dst, 512)])`,
  REPLICATE_TAC 20 GEN_TAC THEN
  WORD_FORALL_OFFSET_TAC 176 THEN
  REPEAT GEN_TAC THEN

  REWRITE_TAC[fst mlkem_basemul_k3_windows_tmc_EXEC] THEN
  ...
  ENSURES_INIT_TAC "s0" THEN
  X86_STEPS_TAC mlkem_basemul_k3_windows_tmc_EXEC (1--18) THEN
  ...
  MP_TAC(SPECL [`src1:int64`; `src2:int64`; `src2t:int64`; `dst:int64`;
                ...
                `pc + 105`] MLKEM_BASEMUL_K3_CORRECT) THEN
  ASM_REWRITE_TAC[C_ARGUMENTS; SOME_FLAGS] THEN REWRITE_TAC[ALL] THEN
  ANTS_TAC THENL [NONOVERLAPPING_TAC; ALL_TAC] THEN

  X86_BIGSTEP_TAC mlkem_basemul_k3_windows_tmc_EXEC "s19" THENL
   [FIRST_ASSUM(MATCH_ACCEPT_TAC o MATCH_MP
     (BYTES_LOADED_SUBPROGRAM_RULE mlkem_basemul_k3_windows_tmc
     (REWRITE_RULE[BUTLAST_CLAUSES]
      (AP_TERM `BUTLAST:byte list->byte list` mlkem_basemul_k3_tmc))
     105));
  ...
  X86_STEPS_TAC mlkem_basemul_k3_windows_tmc_EXEC (20--34) THEN
...
let MLKEM_BASEMUL_K3_WINDOWS_SUBROUTINE_CORRECT = prove(
  `!src1 src2 src2t dst a0 b0 c0 d0 dz0 a1 b1 c1 d1 dz1 a2 b2 c2 d2 dz2 pc stackpointer returnaddress.
        aligned 32 src1 /\
        aligned 32 src2 /\
        aligned 32 src2t /\
        aligned 32 dst /\
        nonoverlapping (word pc, LENGTH mlkem_basemul_k3_windows_mc)
                       (word_sub stackpointer (word 176), 176) /\
        nonoverlapping (src1, 1536)
                       (word_sub stackpointer (word 176), 176) /\
        nonoverlapping (src2, 1536)
        ...
        nonoverlapping (dst, 512) (src2, 1536) /\
        nonoverlapping (dst, 512) (src2t, 768) /\
        nonoverlapping (dst, 512) (word pc, LENGTH mlkem_basemul_k3_windows_mc) /\
        nonoverlapping (dst, 512) (word_sub stackpointer (word 176), 184)
        ==> ensures x86 
              (\s. bytes_loaded s (word pc) mlkem_basemul_k3_windows_mc /\
                   read RIP s = word pc /\
                   read RSP s = stackpointer /\
                   read (memory :> bytes64 stackpointer) s = returnaddress /\
                   WINDOWS_C_ARGUMENTS [dst; src1; src2; src2t] s /\
              ...
              (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
               MAYCHANGE [memory :> bytes(word_sub stackpointer (word 176), 176)] ,,
               MAYCHANGE [memory :> bytes(dst, 512)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE MLKEM_BASEMUL_K3_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;
...
let MLKEM_BASEMUL_K3_SAFE = time prove
 (`exists f_events.
       forall e src1 src2 src2t dst pc.
           ...
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST mlkem_basemul_k3_tmc) /\
                    read RIP s = word pc /\
                    C_ARGUMENTS [dst; src1; src2; src2t] s /\
                    read events s = e)
               (\s.
    ...
    forall e src1 src2 src2t dst pc stackpointer returnaddress.
        ...
        ==> ensures x86
            ...
            (MAYCHANGE [RSP] ,,
             ...
             MAYCHANGE [memory :> bytes (dst,512)])`,
  X86_PROMOTE_RETURN_NOSTACK_TAC mlkem_basemul_k3_tmc MLKEM_BASEMUL_K3_SAFE
...
let MLKEM_BASEMUL_K3_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e src1 src2 src2t dst pc stackpointer returnaddress.
        aligned 32 src1 /\
        ...
        nonoverlapping (word pc,LENGTH mlkem_basemul_k3_windows_tmc)
        (word_sub stackpointer (word 176),176) /\
        nonoverlapping (src1,1536) (word_sub stackpointer (word 176),176) /\
        nonoverlapping (src2,1536) (word_sub stackpointer (word 176),176) /\
        nonoverlapping (src2t,768) (word_sub stackpointer (word 176),176) /\
        nonoverlapping (dst,512) (src1,1536) /\
        nonoverlapping (dst,512) (src2,1536) /\
        nonoverlapping (dst,512) (src2t,768) /\
        nonoverlapping (dst,512)
        (word pc,LENGTH mlkem_basemul_k3_windows_tmc) /\
        nonoverlapping (dst,512) (word_sub stackpointer (word 176),184)
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) mlkem_basemul_k3_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [dst; src1; src2; src2t] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             MAYCHANGE [memory :> bytes (dst,512)])`,
  ASSUME_CALLEE_SAFETY_TAC MLKEM_BASEMUL_K3_SAFE "H_subth" THEN
...
let MLKEM_BASEMUL_K3_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e src1 src2 src2t dst pc stackpointer returnaddress.
        aligned 32 src1 /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) mlkem_basemul_k3_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [dst; src1; src2; src2t] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (word_sub stackpointer (word 176),176)] ,,
             MAYCHANGE [memory :> bytes (dst,512)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE MLKEM_BASEMUL_K3_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
