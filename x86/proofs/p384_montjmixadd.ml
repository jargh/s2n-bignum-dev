...
let p384_montjmixadd_tmc = define_trimmed "p384_montjmixadd_tmc" p384_montjmixadd_mc;;

let P384_MONTJMIXADD_EXEC = X86_MK_CORE_EXEC_RULE p384_montjmixadd_tmc;;
...
let LOCAL_MONTSQR_P384_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE p384_montjmixadd_tmc) 230 lvs
  `!(t:x86state) pcin pcout p3 n3 p1 n1.
    !a. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 6)) t = a
    ==>
    nonoverlapping (word pc,0x3bcd) (word_add (read p3 t) (word n3),48)
    ==> ensures x86
         (\s. bytes_loaded s (word pc) (BUTLAST p384_montjmixadd_tmc) /\
              read RIP s = pcin /\
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ASM_CASES_TAC `a EXP 2 <= 2 EXP 384 * p_384` THENL
   [ASM_REWRITE_TAC[]; X86_SIM_TAC P384_MONTJMIXADD_EXEC (1--230)] THEN
  ...
  MAP_EVERY (fun s ->
    X86_SINGLE_STEP_TAC P384_MONTJMIXADD_EXEC s THEN
  ...
  X86_ACCSTEPS_TAC P384_MONTJMIXADD_EXEC
...
let LOCAL_MONTMUL_P384_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE p384_montjmixadd_tmc) 263 lvs
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ASM_CASES_TAC `a * b <= 2 EXP 384 * p_384` THENL
   [ASM_REWRITE_TAC[]; X86_SIM_TAC P384_MONTJMIXADD_EXEC (1--263)] THEN
...
let LOCAL_SUB_P384_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE p384_montjmixadd_tmc) 32 lvs
  `!(t:x86state) pcin pcout p3 n3 p1 n1 p2 n2.
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  X86_ACCSTEPS_TAC P384_MONTJMIXADD_EXEC (1--12) (1--12) THEN
  ...
  X86_ACCSTEPS_TAC P384_MONTJMIXADD_EXEC [18;20;25;27;29;31] (13--32) THEN
...
let LOCAL_AMONTSQR_P384_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE p384_montjmixadd_tmc) 222 lvs
  `!(t:x86state) pcin pcout p3 n3 p1 n1.
    !a. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 6)) t = a
    ==>
    nonoverlapping (word pc,0x3bcd) (word_add (read p3 t) (word n3),48)
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  X86_ACCSTEPS_TAC P384_MONTJMIXADD_EXEC (211--216) (204--222) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  CONJ_TAC THENL
...
let P384_MONTJMIXADD_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer.
        ALL (nonoverlapping (stackpointer,304))
            [(word pc,0x3bcd); (p1,144); (p2,96); (p3,144)] /\
        nonoverlapping (p3,144) (word pc,0x3bcd)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST p384_montjmixadd_tmc) /\
  ...
  LOCAL_AMONTSQR_P384_TAC 2 ["zp2";"z_1"] THEN
  ...
  X86_STEPS_TAC P384_MONTJMIXADD_EXEC (29--107) THEN
...
let P384_MONTJMIXADD_NOIBT_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 352),352))
            [(word pc,LENGTH p384_montjmixadd_tmc); (p1,144); (p2,96)] /\
        ALL (nonoverlapping (p3,144))
            [(word pc,LENGTH p384_montjmixadd_tmc); (word_sub stackpointer (word 352),360)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) p384_montjmixadd_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
  ...
  X86_PROMOTE_RETURN_STACK_TAC p384_montjmixadd_tmc P384_MONTJMIXADD_CORRECT
    `[RBX; RBP; R12; R13; R14; R15]` 352);;

let P384_MONTJMIXADD_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 352),352))
            [(word pc,LENGTH p384_montjmixadd_mc); (p1,144); (p2,96)] /\
        ALL (nonoverlapping (p3,144))
            [(word pc,LENGTH p384_montjmixadd_mc); (word_sub stackpointer (word 352),360)]
        ==> ensures x86
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(p3,144);
                      memory :> bytes(word_sub stackpointer (word 352),352)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P384_MONTJMIXADD_NOIBT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let p384_montjmixadd_windows_mc = define_from_elf "p384_montjmixadd_windows_mc"
      "x86/p384/p384_montjmixadd.obj";;

let p384_montjmixadd_windows_tmc = define_trimmed "p384_montjmixadd_windows_tmc" p384_montjmixadd_windows_mc;;

let P384_MONTJMIXADD_NOIBT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 368),368))
            [(word pc,LENGTH p384_montjmixadd_windows_tmc); (p1,144); (p2,96)] /\
        ALL (nonoverlapping (p3,144))
            [(word pc,LENGTH p384_montjmixadd_windows_tmc); (word_sub stackpointer (word 368),376)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) p384_montjmixadd_windows_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
   p384_montjmixadd_windows_tmc p384_montjmixadd_tmc
   P384_MONTJMIXADD_CORRECT
    `[RBX; RBP; R12; R13; R14; R15]` 352);;

let P384_MONTJMIXADD_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 368),368))
            [(word pc,LENGTH p384_montjmixadd_windows_mc); (p1,144); (p2,96)] /\
        ALL (nonoverlapping (p3,144))
            [(word pc,LENGTH p384_montjmixadd_windows_mc); (word_sub stackpointer (word 368),376)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) p384_montjmixadd_windows_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [p3; p1; p2] s /\
          ...
          (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(p3,144);
                      memory :> bytes(word_sub stackpointer (word 368),368)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P384_MONTJMIXADD_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;
...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "p384_montjmixadd" subroutine_signatures)
    P384_MONTJMIXADD_CORRECT
    P384_MONTJMIXADD_EXEC;;

let P384_MONTJMIXADD_SAFE = time prove
 (`exists f_events.
       forall e p3 p1 p2 pc stackpointer.
           ...
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST p384_montjmixadd_tmc) /\
                    read RIP s = word (pc + 0x11) /\
                    read RSP s = stackpointer /\
                    C_ARGUMENTS [p3; p1; p2] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 0x3bbb) /\
                    (exists e2.
                         read events s = APPEND e2 e /\
                         e2 = f_events p1 p2 p3 pc stackpointer /\
                         memaccess_inbounds e2
                         [p1,144; p2,96; p3,144; stackpointer,304]
                         [p3,144; stackpointer,304]))
               (MAYCHANGE
                [RIP; RSI; RAX; RDX; RCX; R8; R9; R10; R11; RBX; RBP; R12; R13; R14; R15] ,,
                MAYCHANGE SOME_FLAGS ,,
                MAYCHANGE [events] ,,
                MAYCHANGE
                [memory :> bytes (p3,144); memory :> bytes (stackpointer,304)])`,
  ASSERT_CONCL_TAC full_spec THEN
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars P384_MONTJMIXADD_EXEC);;

let P384_MONTJMIXADD_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 352),352))
        [word pc,LENGTH p384_montjmixadd_tmc; p1,144; p2,96] /\
        ALL (nonoverlapping (p3,144))
        [word pc,LENGTH p384_montjmixadd_tmc;
         word_sub stackpointer (word 352),360]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p384_montjmixadd_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      read events s = APPEND e2 e /\
                      e2 =
                      f_events p1 p2 p3 pc (word_sub stackpointer (word 352))
                      returnaddress /\
                      memaccess_inbounds e2
                      [p1,144; p2,96; p3,144;
                       word_sub stackpointer (word 352),360]
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,144);
              memory :> bytes (word_sub stackpointer (word 352),352)])`,
  X86_PROMOTE_RETURN_STACK_TAC p384_montjmixadd_tmc P384_MONTJMIXADD_SAFE
    `[RBX; RBP; R12; R13; R14; R15]` 352
  THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let P384_MONTJMIXADD_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 352),352))
        [word pc,LENGTH p384_montjmixadd_mc; p1,144; p2,96] /\
        ALL (nonoverlapping (p3,144))
        [word pc,LENGTH p384_montjmixadd_mc;
         word_sub stackpointer (word 352),360]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p384_montjmixadd_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,144);
              memory :> bytes (word_sub stackpointer (word 352),352)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P384_MONTJMIXADD_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let P384_MONTJMIXADD_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 368),368))
        [word pc,LENGTH p384_montjmixadd_windows_tmc; p1,144; p2,96] /\
        ALL (nonoverlapping (p3,144))
        [word pc,LENGTH p384_montjmixadd_windows_tmc;
         word_sub stackpointer (word 368),376]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p384_montjmixadd_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      read events s = APPEND e2 e /\
                      e2 =
                      f_events p1 p2 p3 pc (word_sub stackpointer (word 368))
                      returnaddress /\
                      memaccess_inbounds e2
                      [p1,144; p2,96; p3,144;
                       word_sub stackpointer (word 368),376]
                      [p3,144; word_sub stackpointer (word 368),368]))
            (MAYCHANGE [RSP] ,,
             WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE
             [memory :> bytes (p3,144);
              memory :> bytes (word_sub stackpointer (word 368),368)])`,
  WINDOWS_X86_WRAP_STACK_TAC p384_montjmixadd_windows_tmc p384_montjmixadd_tmc
    P384_MONTJMIXADD_SAFE `[RBX; RBP; R12; R13; R14; R15]` 352
  THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let P384_MONTJMIXADD_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 368),368))
        [word pc,LENGTH p384_montjmixadd_windows_mc; p1,144; p2,96] /\
        ALL (nonoverlapping (p3,144))
        [word pc,LENGTH p384_montjmixadd_windows_mc;
         word_sub stackpointer (word 368),376]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p384_montjmixadd_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,144);
              memory :> bytes (word_sub stackpointer (word 368),368)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P384_MONTJMIXADD_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
