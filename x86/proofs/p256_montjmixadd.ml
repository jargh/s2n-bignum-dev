...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "p256_montjmixadd" subroutine_signatures)
    P256_MONTJMIXADD_CORRECT
    P256_MONTJMIXADD_EXEC;;

let P256_MONTJMIXADD_SAFE = time prove
 (`exists f_events.
       forall e p3 p1 p2 pc stackpointer.
           ...
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST p256_montjmixadd_tmc) /\
                    read RIP s = word (pc + 0x11) /\
                    read RSP s = stackpointer /\
                    C_ARGUMENTS [p3; p1; p2] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 0x1ca2) /\
                    (exists e2.
  ...
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars P256_MONTJMIXADD_EXEC);;

let P256_MONTJMIXADD_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 240),240))
        [word pc,LENGTH p256_montjmixadd_tmc; p1,96; p2,64] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH p256_montjmixadd_tmc;
         word_sub stackpointer (word 240),248]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p256_montjmixadd_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,96);
              memory :> bytes (word_sub stackpointer (word 240),240)])`,
  X86_PROMOTE_RETURN_STACK_TAC p256_montjmixadd_tmc P256_MONTJMIXADD_SAFE
    `[RBX; RBP; R12; R13; R14; R15]` 240
  THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let P256_MONTJMIXADD_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 240),240))
        [word pc,LENGTH p256_montjmixadd_mc; p1,96; p2,64] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH p256_montjmixadd_mc;
         word_sub stackpointer (word 240),248]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p256_montjmixadd_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,96);
              memory :> bytes (word_sub stackpointer (word 240),240)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P256_MONTJMIXADD_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let P256_MONTJMIXADD_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 256),256))
        [word pc,LENGTH p256_montjmixadd_windows_tmc; p1,96; p2,64] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH p256_montjmixadd_windows_tmc;
         word_sub stackpointer (word 256),264]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p256_montjmixadd_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,96);
              memory :> bytes (word_sub stackpointer (word 256),256)])`,
  WINDOWS_X86_WRAP_STACK_TAC p256_montjmixadd_windows_tmc p256_montjmixadd_tmc
    P256_MONTJMIXADD_SAFE `[RBX; RBP; R12; R13; R14; R15]` 240
  THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let P256_MONTJMIXADD_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 256),256))
        [word pc,LENGTH p256_montjmixadd_windows_mc; p1,96; p2,64] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH p256_montjmixadd_windows_mc;
         word_sub stackpointer (word 256),264]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) p256_montjmixadd_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,96);
              memory :> bytes (word_sub stackpointer (word 256),256)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE P256_MONTJMIXADD_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
