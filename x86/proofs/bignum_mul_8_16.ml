...
let BIGNUM_MUL_8_16_SAFE = time prove
 (`exists f_events.
       forall e z x y pc.
           nonoverlapping (word pc,1227) (z,8 * 16) /\
           nonoverlapping (x,8 * 8) (z,8 * 16) /\
           (y = z \/ nonoverlapping (y,8 * 8) (z,8 * 16))
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST bignum_mul_8_16_tmc) /\
...
let BIGNUM_MUL_8_16_NOIBT_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x y pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_mul_8_16_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x; y] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      [x,64; y,64; z,128; word_sub stackpointer (word 48),56]
                      [z,128; word_sub stackpointer (word 48),48]))
            (MAYCHANGE [RSP] ,,
             MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE
             [memory :> bytes (z,8 * 16);
              memory :> bytes (word_sub stackpointer (word 48),48)])`,
  X86_PROMOTE_RETURN_STACK_TAC bignum_mul_8_16_tmc BIGNUM_MUL_8_16_SAFE `[RBX; RBP; R12; R13; R14; R15]` 48 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;
...
exists f_events.
    forall e z x y pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_mul_8_16_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x; y] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 16);
              memory :> bytes (word_sub stackpointer (word 48),48)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MUL_8_16_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_MUL_8_16_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x y pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 64),72) (z,8 * 16) /\
        ALL (nonoverlapping (word_sub stackpointer (word 64),64))
        [word pc,LENGTH bignum_mul_8_16_windows_tmc; x,8 * 8; y,8 * 8] /\
        nonoverlapping (word pc,LENGTH bignum_mul_8_16_windows_tmc)
        (z,8 * 16) /\
        nonoverlapping (x,8 * 8) (z,8 * 16) /\
        (y = z \/ nonoverlapping (y,8 * 8) (z,8 * 16))
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_mul_8_16_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x; y] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      returnaddress /\
                      memaccess_inbounds e2
                      [x,64; y,64; z,128; word_sub stackpointer (word 64),72]
                      [z,128; word_sub stackpointer (word 64),64]))
            (MAYCHANGE [RSP] ,,
             WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE
             [memory :> bytes (z,8 * 16);
              memory :> bytes (word_sub stackpointer (word 64),64)])`,
  WINDOWS_X86_WRAP_STACK_TAC bignum_mul_8_16_windows_tmc bignum_mul_8_16_tmc BIGNUM_MUL_8_16_SAFE `[RBX; RBP; R12; R13; R14; R15]` 48 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_MUL_8_16_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x y pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 64),72) (z,8 * 16) /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_mul_8_16_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x; y] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 16);
              memory :> bytes (word_sub stackpointer (word 64),64)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_MUL_8_16_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
