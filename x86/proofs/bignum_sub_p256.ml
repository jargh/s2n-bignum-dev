...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "bignum_sub_p256" subroutine_signatures)
    BIGNUM_SUB_P256_CORRECT
    BIGNUM_SUB_P256_EXEC;;

let BIGNUM_SUB_P256_SAFE = time prove
 (`exists f_events.
       forall e z x y pc.
           nonoverlapping (word pc,77) (z,8 * 4)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST bignum_sub_p256_tmc) /\
                    read RIP s = word pc /\
                    C_ARGUMENTS [z; x; y] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 76) /\
                    (exists e2.
  ...
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars BIGNUM_SUB_P256_EXEC);;

(* Has no "word_sub stackpointer (word ..)"; stackofs is None *)
let BIGNUM_SUB_P256_NOIBT_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x y pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_sub_p256_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x; y] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             MAYCHANGE [memory :> bignum (z,4)])`,
  X86_PROMOTE_RETURN_NOSTACK_TAC bignum_sub_p256_tmc BIGNUM_SUB_P256_SAFE THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_SUB_P256_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x y pc stackpointer returnaddress.
        nonoverlapping (word pc,LENGTH bignum_sub_p256_mc) (z,8 * 4) /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_sub_p256_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x; y] s /\
            ...
            (MAYCHANGE [RSP] ,,
             MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE [memory :> bignum (z,4)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_SUB_P256_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_SUB_P256_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x y pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_sub_p256_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x; y] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,4);
              memory :> bytes (word_sub stackpointer (word 16),16)])`,
  WINDOWS_X86_WRAP_NOSTACK_TAC bignum_sub_p256_windows_tmc bignum_sub_p256_tmc BIGNUM_SUB_P256_SAFE THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_SUB_P256_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x y pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_sub_p256_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x; y] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,4);
              memory :> bytes (word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_SUB_P256_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
