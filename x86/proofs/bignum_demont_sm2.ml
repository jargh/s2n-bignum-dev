...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "bignum_demont_sm2" subroutine_signatures)
    BIGNUM_DEMONT_SM2_CORRECT
    BIGNUM_DEMONT_SM2_EXEC;;

let BIGNUM_DEMONT_SM2_SAFE = time prove
 (`exists f_events.
       forall e z x pc.
           nonoverlapping (word pc,187) (z,8 * 4)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST bignum_demont_sm2_tmc) /\
                    read RIP s = word pc /\
                    C_ARGUMENTS [z; x] s /\
                    read events s = e)
               (\s.
  ...
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars BIGNUM_DEMONT_SM2_EXEC);;

(* Has no "word_sub stackpointer (word ..)"; stackofs is None *)
let BIGNUM_DEMONT_SM2_NOIBT_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_demont_sm2_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             MAYCHANGE [memory :> bytes (z,8 * 4)])`,
  X86_PROMOTE_RETURN_NOSTACK_TAC bignum_demont_sm2_tmc BIGNUM_DEMONT_SM2_SAFE THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_DEMONT_SM2_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        nonoverlapping (word pc,LENGTH bignum_demont_sm2_mc) (z,8 * 4) /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_demont_sm2_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE [memory :> bytes (z,8 * 4)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_DEMONT_SM2_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_DEMONT_SM2_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 16),24) /\
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
        [word pc,LENGTH bignum_demont_sm2_windows_tmc; x,8 * 4] /\
        nonoverlapping (word pc,LENGTH bignum_demont_sm2_windows_tmc)
        (z,8 * 4)
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_demont_sm2_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 4);
              memory :> bytes (word_sub stackpointer (word 16),16)])`,
  WINDOWS_X86_WRAP_NOSTACK_TAC bignum_demont_sm2_windows_tmc bignum_demont_sm2_tmc BIGNUM_DEMONT_SM2_SAFE THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_DEMONT_SM2_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        nonoverlapping (z,8 * 4) (word_sub stackpointer (word 16),24) /\
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
        [word pc,LENGTH bignum_demont_sm2_windows_mc; x,8 * 4] /\
        nonoverlapping (word pc,LENGTH bignum_demont_sm2_windows_mc)
        (z,8 * 4)
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_demont_sm2_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 4);
              memory :> bytes (word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_DEMONT_SM2_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
