...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "bignum_demont_p384" subroutine_signatures)
    BIGNUM_DEMONT_P384_CORRECT
    BIGNUM_DEMONT_P384_EXEC;;

let BIGNUM_DEMONT_P384_SAFE = time prove
 (`exists f_events.
       forall e z x pc.
           nonoverlapping (word pc,487) (z,8 * 6)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST bignum_demont_p384_tmc) /\
                    read RIP s = word (pc + 4) /\
                    C_ARGUMENTS [z; x] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 482) /\
                    (exists e2.
  ...
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars BIGNUM_DEMONT_P384_EXEC);;

let BIGNUM_DEMONT_P384_NOIBT_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_demont_p384_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 6);
              memory :> bytes (word_sub stackpointer (word 16),16)])`,
  X86_PROMOTE_RETURN_STACK_TAC bignum_demont_p384_tmc BIGNUM_DEMONT_P384_SAFE `[R12; R13]` 16 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_DEMONT_P384_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        nonoverlapping (z,8 * 6) (word_sub stackpointer (word 16),24) /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_demont_p384_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 6);
              memory :> bytes (word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_DEMONT_P384_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_DEMONT_P384_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        nonoverlapping (z,8 * 6) (word_sub stackpointer (word 32),40) /\
        ALL (nonoverlapping (word_sub stackpointer (word 32),32))
        [word pc,LENGTH bignum_demont_p384_windows_tmc; x,8 * 6] /\
        nonoverlapping (word pc,LENGTH bignum_demont_p384_windows_tmc)
        (z,8 * 6)
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_demont_p384_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 6);
              memory :> bytes (word_sub stackpointer (word 32),32)])`,
  WINDOWS_X86_WRAP_STACK_TAC bignum_demont_p384_windows_tmc bignum_demont_p384_tmc BIGNUM_DEMONT_P384_SAFE `[R12; R13]` 16 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_DEMONT_P384_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z x pc stackpointer returnaddress.
        nonoverlapping (z,8 * 6) (word_sub stackpointer (word 32),40) /\
        ALL (nonoverlapping (word_sub stackpointer (word 32),32))
        [word pc,LENGTH bignum_demont_p384_windows_mc; x,8 * 6] /\
        nonoverlapping (word pc,LENGTH bignum_demont_p384_windows_mc)
        (z,8 * 6)
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_demont_p384_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (z,8 * 6);
              memory :> bytes (word_sub stackpointer (word 32),32)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_DEMONT_P384_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
