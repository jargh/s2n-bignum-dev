...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "mlkem_ntt_x86" subroutine_signatures)
    MLKEM_NTT_CORRECT
    MLKEM_NTT_TMC_EXEC;;
           ...
           ==> ensures x86
               ...
               (\s.
                    ...
                    (exists e2.
                         ...
                         memaccess_inbounds e2 [a,512; zetas,1248] [a,512]))
               (MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
                MAYCHANGE
                [ZMM0; ZMM2; ZMM3; ZMM4; ZMM5; ZMM6; ZMM7; ZMM8; ZMM9; ZMM10;
        ...
        ==> ensures x86
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      [a,512; zetas,1248; stackpointer,8]
                      [a,512; stackpointer,0]))
            (MAYCHANGE [RSP] ,,
             MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE [memory :> bytes (a,512)])`,
  X86_PROMOTE_RETURN_NOSTACK_TAC mlkem_ntt_tmc MLKEM_NTT_SAFE
...
let MLKEM_NTT_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e a zetas pc stackpointer returnaddress.
        aligned 32 a /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) mlkem_ntt_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [a; zetas] s /\
            ...
            (\s.
                 ...
                 (exists e2.
                      ...
                      [a,512; zetas,1248;
                       word_sub stackpointer (word 176),184]
                      [a,512; word_sub stackpointer (word 176),176]))
            (MAYCHANGE [RSP] ,,
             WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE
             [memory :> bytes (word_sub stackpointer (word 176),176)] ,,
             MAYCHANGE [memory :> bytes (a,512)])`,
  ASSUME_CALLEE_SAFETY_TAC MLKEM_NTT_SAFE "H_subth" THEN
...
let MLKEM_NTT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e a zetas pc stackpointer returnaddress.
        aligned 32 a /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) mlkem_ntt_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [a; zetas] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (word_sub stackpointer (word 176),176)] ,,
             MAYCHANGE [memory :> bytes (a,512)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE MLKEM_NTT_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
