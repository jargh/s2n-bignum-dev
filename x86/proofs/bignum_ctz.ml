...
let full_spec,public_vars = mk_safety_spec ~keep_maychanges:true
    (assoc "bignum_ctz" subroutine_signatures) BIGNUM_CTZ_CORRECT BIGNUM_CTZ_EXEC;;

let BIGNUM_CTZ_SAFE = prove
 (full_spec,
  ...
  REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS] THEN
  ASM_CASES_TAC `k = 0` THENL
   [ ASM_REWRITE_TAC[] THEN X86_SIM_TAC BIGNUM_CTZ_EXEC (1--3) THEN DISCHARGE_SAFETY_PROPERTY_TAC; ALL_TAC ] THEN
  ASM_REWRITE_TAC[ASSUME `val(word k:int64) = k`] THEN
  ENSURES_EVENTS_WHILE_UP2_TAC `k:num` `pc + 0x13` `pc + 0x28`
   `\i s. read RSI s = a /\ read RDI s = word(k - i)` THEN
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [ X86_SIM_TAC BIGNUM_CTZ_EXEC (1--6) THEN ASM_REWRITE_TAC[SUB_0] THEN DISCHARGE_SAFETY_PROPERTY_TAC;
     ALL_TAC;
     ENSURES_INIT_TAC "s0" THEN STRIP_EXISTS_ASSUM_TAC THEN
     X86_STEPS_TAC BIGNUM_CTZ_EXEC (1--4) THEN ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
     DISCHARGE_SAFETY_PROPERTY_TAC ] THEN
  REPEAT STRIP_TAC THEN ASM_REWRITE_TAC[] THEN
  ENSURES_INIT_TAC "s0" THEN STRIP_EXISTS_ASSUM_TAC THEN
  X86_STEPS_TAC BIGNUM_CTZ_EXEC (1--6) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
...
let BIGNUM_CTZ_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e k a pc stackpointer returnaddress.
        ensures x86
            ...
            (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI)`,
  X86_PROMOTE_RETURN_NOSTACK_TAC bignum_ctz_tmc BIGNUM_CTZ_SAFE THEN DISCHARGE_SAFETY_PROPERTY_TAC);;
        ...
        ensures x86
            ...
            (\s. read RIP s = returnaddress /\
                 ...
                 (exists e2. read events s = APPEND e2 e /\
                      ...
                      memaccess_inbounds e2 [a,val k * 8; stackpointer,8] []))
            (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI)`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_CTZ_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_CTZ_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e k a pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
        [word pc,LENGTH bignum_ctz_windows_tmc; a,8 * val k]
        ==> ensures x86
            ...
            (MAYCHANGE [RSP] ,,
             ...
             MAYCHANGE [memory :> bytes (word_sub stackpointer (word 16),16)])`,
  WINDOWS_X86_WRAP_NOSTACK_TAC bignum_ctz_windows_tmc bignum_ctz_tmc BIGNUM_CTZ_SAFE THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_CTZ_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e k a pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 16),16))
        [word pc,LENGTH bignum_ctz_windows_mc; a,8 * val k]
        ==> ensures x86
            ...
            (MAYCHANGE [RSP] ,,
             WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
             MAYCHANGE [memory :> bytes (word_sub stackpointer (word 16),16)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_CTZ_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
