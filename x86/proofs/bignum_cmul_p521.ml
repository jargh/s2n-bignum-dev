...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "bignum_cmul_p521" subroutine_signatures)
    BIGNUM_CMUL_P521_CORRECT
    BIGNUM_CMUL_P521_EXEC;;

let BIGNUM_CMUL_P521_SAFE = time prove
 (`exists f_events.
       forall e z c x pc.
           nonoverlapping (word pc,223) (z,8 * 9)
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST bignum_cmul_p521_tmc) /\
                    read RIP s = word (pc + 6) /\
                    C_ARGUMENTS [z; c; x] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 216) /\
                    (exists e2.
  ...
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars BIGNUM_CMUL_P521_EXEC);;

let BIGNUM_CMUL_P521_NOIBT_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z c x pc stackpointer returnaddress.
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_cmul_p521_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; c; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,9);
              memory :> bytes (word_sub stackpointer (word 32),32)])`,
  X86_PROMOTE_RETURN_STACK_TAC bignum_cmul_p521_tmc BIGNUM_CMUL_P521_SAFE `[RBX; RBP; R12; R13]` 32 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_CMUL_P521_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z c x pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 32),32) (x,8 * 9) /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_cmul_p521_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [z; c; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,9);
              memory :> bytes (word_sub stackpointer (word 32),32)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_CMUL_P521_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let BIGNUM_CMUL_P521_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z c x pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 48),48) (x,8 * 9) /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_cmul_p521_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; c; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,9);
              memory :> bytes (word_sub stackpointer (word 48),48)])`,
  WINDOWS_X86_WRAP_STACK_TAC bignum_cmul_p521_windows_tmc bignum_cmul_p521_tmc BIGNUM_CMUL_P521_SAFE `[RBX; RBP; R12; R13]` 32 THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let BIGNUM_CMUL_P521_WINDOWS_SUBROUTINE_SAFE = time prove
 (`
exists f_events.
    forall e z c x pc stackpointer returnaddress.
        nonoverlapping (word_sub stackpointer (word 48),48) (x,8 * 9) /\
        ...
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) bignum_cmul_p521_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [z; c; x] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bignum (z,9);
              memory :> bytes (word_sub stackpointer (word 48),48)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE BIGNUM_CMUL_P521_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
