...
(**** print_literal_from_elf "x86/sm2/sm2_montjmixadd.o";;
 ****)

let sm2_montjmixadd_mc = define_assert_from_elf
  "sm2_montjmixadd_mc" "x86/sm2/sm2_montjmixadd.o"
[
...
let sm2_montjmixadd_tmc = define_trimmed "sm2_montjmixadd_tmc" sm2_montjmixadd_mc;;

let SM2_MONTJMIXADD_EXEC = X86_MK_CORE_EXEC_RULE sm2_montjmixadd_tmc;;
...
let LOCAL_MONTSQR_SM2_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE sm2_montjmixadd_tmc) 113 lvs
  `!(t:x86state) pcin pcout p3 n3 p1 n1.
    !a. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 4)) t = a
    ==>
    nonoverlapping (word pc,0x1b2c) (word_add (read p3 t) (word n3),32)
    ==> ensures x86
         (\s. bytes_loaded s (word pc) (BUTLAST sm2_montjmixadd_tmc) /\
              read RIP s = pcin /\
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ASM_CASES_TAC `a EXP 2 <= 2 EXP 256 * p_sm2` THENL
   [ASM_REWRITE_TAC[]; X86_SIM_TAC SM2_MONTJMIXADD_EXEC (1--113)] THEN
  ENSURES_INIT_TAC "s0" THEN
  FIRST_ASSUM(BIGNUM_DIGITIZE_TAC "x_" o lhand o concl) THEN

  X86_ACCSTEPS_TAC SM2_MONTJMIXADD_EXEC (1--94) (1--95) THEN
  ABBREV_TAC
  ...
  X86_ACCSTEPS_TAC SM2_MONTJMIXADD_EXEC [99;101;103;104;105] (96--113) THEN
...
let LOCAL_MONTMUL_SM2_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE sm2_montjmixadd_tmc) 127 lvs
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ASM_CASES_TAC `a * b <= 2 EXP 256 * p_sm2` THENL
   [ASM_REWRITE_TAC[]; X86_SIM_TAC SM2_MONTJMIXADD_EXEC (1--127)] THEN
  ...
  X86_ACCSTEPS_TAC SM2_MONTJMIXADD_EXEC (2--109) (2--110) THEN
  ABBREV_TAC
   `t = bignum_of_wordlist
  ...
  X86_ACCSTEPS_TAC SM2_MONTJMIXADD_EXEC [114;116;118;119;120] (111--128) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  CONV_TAC(LAND_CONV BIGNUM_EXPAND_CONV) THEN ASM_REWRITE_TAC[] THEN
  TRANS_TAC EQ_TRANS `t MOD p_sm2` THEN CONJ_TAC THENL
   [ALL_TAC;
...
let LOCAL_SUB_SM2_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE sm2_montjmixadd_tmc) 21 lvs
  `!(t:x86state) pcin pcout p3 n3 p1 n1 p2 n2.
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  X86_ACCSTEPS_TAC SM2_MONTJMIXADD_EXEC (1--8) (1--8) THEN
  ...
  X86_ACCSTEPS_TAC SM2_MONTJMIXADD_EXEC (14--21) (9--21) THEN
...
let LOCAL_AMONTSQR_SM2_TAC =
  X86_MACRO_SIM_ABBREV_TAC (X86_TRIM_EXEC_RULE sm2_montjmixadd_tmc) 106 lvs
  `!(t:x86state) pcin pcout p3 n3 p1 n1.
    !a. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 4)) t = a
    ==>
    nonoverlapping (word pc,0x1b2c) (word_add (read p3 t) (word n3),32)
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  X86_ACCSTEPS_TAC SM2_MONTJMIXADD_EXEC (1--93) (1--94) THEN
  ABBREV_TAC
   `t = bignum_of_wordlist
  ...
  X86_ACCSTEPS_TAC SM2_MONTJMIXADD_EXEC [99;100;101;102] (95--106) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  CONJ_TAC THENL
...
let SM2_MONTJMIXADD_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer.
        ALL (nonoverlapping (stackpointer,192))
            [(word pc,0x1b2c); (p1,96); (p2,64); (p3,96)] /\
        nonoverlapping (p3,96) (word pc,0x1b2c)
        ==> ensures x86
             (\s. bytes_loaded s (word pc) (BUTLAST sm2_montjmixadd_tmc) /\
  ...
  LOCAL_AMONTSQR_SM2_TAC 1 ["zp2";"z_1"] THEN
  ...
  X86_STEPS_TAC SM2_MONTJMIXADD_EXEC (20--72) THEN
...
let SM2_MONTJMIXADD_NOIBT_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 240),240))
            [(word pc,LENGTH sm2_montjmixadd_tmc); (p1,96); (p2,64)] /\
        ALL (nonoverlapping (p3,96))
            [(word pc,LENGTH sm2_montjmixadd_tmc); (word_sub stackpointer (word 240),248)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) sm2_montjmixadd_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
  ...
  X86_PROMOTE_RETURN_STACK_TAC sm2_montjmixadd_tmc SM2_MONTJMIXADD_CORRECT
 ...
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ...
        ==> ensures x86
          ...
          (MAYCHANGE [RSP] ,, MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(p3,96);
                      memory :> bytes(word_sub stackpointer (word 240),240)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE SM2_MONTJMIXADD_NOIBT_SUBROUTINE_CORRECT));;

(* ------------------------------------------------------------------------- *)
(* Correctness of Windows ABI version.                                       *)
(* ------------------------------------------------------------------------- *)

let sm2_montjmixadd_windows_mc = define_from_elf "sm2_montjmixadd_windows_mc"
      "x86/sm2/sm2_montjmixadd.obj";;

let sm2_montjmixadd_windows_tmc = define_trimmed "sm2_montjmixadd_windows_tmc" sm2_montjmixadd_windows_mc;;

let SM2_MONTJMIXADD_NOIBT_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 256),256))
            [(word pc,LENGTH sm2_montjmixadd_windows_tmc); (p1,96); (p2,64)] /\
        ALL (nonoverlapping (p3,96))
            [(word pc,LENGTH sm2_montjmixadd_windows_tmc); (word_sub stackpointer (word 256),264)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) sm2_montjmixadd_windows_tmc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
  ...
  WINDOWS_X86_WRAP_STACK_TAC
   sm2_montjmixadd_windows_tmc sm2_montjmixadd_tmc
   SM2_MONTJMIXADD_CORRECT
    `[RBX; RBP; R12; R13; R14; R15]` 240);;

let SM2_MONTJMIXADD_WINDOWS_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 256),256))
            [(word pc,LENGTH sm2_montjmixadd_windows_mc); (p1,96); (p2,64)] /\
        ALL (nonoverlapping (p3,96))
            [(word pc,LENGTH sm2_montjmixadd_windows_mc); (word_sub stackpointer (word 256),264)]
        ==> ensures x86
             (\s. bytes_loaded s (word pc) sm2_montjmixadd_windows_mc /\
                  read RIP s = word pc /\
                  read RSP s = stackpointer /\
                  read (memory :> bytes64 stackpointer) s = returnaddress /\
                  WINDOWS_C_ARGUMENTS [p3; p1; p2] s /\
          ...
          (MAYCHANGE [RSP] ,, WINDOWS_MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(p3,96);
                      memory :> bytes(word_sub stackpointer (word 256),256)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE SM2_MONTJMIXADD_NOIBT_WINDOWS_SUBROUTINE_CORRECT));;
...
let SM2_MONTJMIXADD_SAFE = time prove
 (`exists f_events.
       forall e p3 p1 p2 pc stackpointer.
           ...
           ==> ensures x86
               (\s.
                    bytes_loaded s (word pc) (BUTLAST sm2_montjmixadd_tmc) /\
                    read RIP s = word (pc + 0x11) /\
                    read RSP s = stackpointer /\
                    C_ARGUMENTS [p3; p1; p2] s /\
                    read events s = e)
               (\s.
                    read RIP s = word (pc + 0x1b1a) /\
                    (exists e2.
                         read events s = APPEND e2 e /\
                         e2 = f_events p1 p2 p3 pc stackpointer /\
                         memaccess_inbounds e2
                         [p1,96; p2,64; p3,96; stackpointer,192]
  ...
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars SM2_MONTJMIXADD_EXEC);;

let SM2_MONTJMIXADD_NOIBT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 240),240))
        [word pc,LENGTH sm2_montjmixadd_tmc; p1,96; p2,64] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH sm2_montjmixadd_tmc;
         word_sub stackpointer (word 240),248]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) sm2_montjmixadd_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,96);
              memory :> bytes (word_sub stackpointer (word 240),240)])`,
  X86_PROMOTE_RETURN_STACK_TAC sm2_montjmixadd_tmc SM2_MONTJMIXADD_SAFE
    `[RBX; RBP; R12; R13; R14; R15]` 240
...
let SM2_MONTJMIXADD_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ...
        [word pc,LENGTH sm2_montjmixadd_mc; p1,96; p2,64] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH sm2_montjmixadd_mc;
         word_sub stackpointer (word 240),248]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) sm2_montjmixadd_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,96);
              memory :> bytes (word_sub stackpointer (word 240),240)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE SM2_MONTJMIXADD_NOIBT_SUBROUTINE_SAFE));;

(* ------------------------------------------------------------------------- *)
(* Constant-time and memory safety proof of Windows ABI version.             *)
(* ------------------------------------------------------------------------- *)

let SM2_MONTJMIXADD_NOIBT_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 256),256))
        [word pc,LENGTH sm2_montjmixadd_windows_tmc; p1,96; p2,64] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH sm2_montjmixadd_windows_tmc;
         word_sub stackpointer (word 256),264]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) sm2_montjmixadd_windows_tmc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,96);
              memory :> bytes (word_sub stackpointer (word 256),256)])`,
  WINDOWS_X86_WRAP_STACK_TAC sm2_montjmixadd_windows_tmc sm2_montjmixadd_tmc
    SM2_MONTJMIXADD_SAFE `[RBX; RBP; R12; R13; R14; R15]` 240
  THEN DISCHARGE_SAFETY_PROPERTY_TAC);;

let SM2_MONTJMIXADD_WINDOWS_SUBROUTINE_SAFE = time prove
 (`exists f_events.
    forall e p3 p1 p2 pc stackpointer returnaddress.
        ALL (nonoverlapping (word_sub stackpointer (word 256),256))
        [word pc,LENGTH sm2_montjmixadd_windows_mc; p1,96; p2,64] /\
        ALL (nonoverlapping (p3,96))
        [word pc,LENGTH sm2_montjmixadd_windows_mc;
         word_sub stackpointer (word 256),264]
        ==> ensures x86
            (\s.
                 bytes_loaded s (word pc) sm2_montjmixadd_windows_mc /\
                 read RIP s = word pc /\
                 read RSP s = stackpointer /\
                 read (memory :> bytes64 stackpointer) s = returnaddress /\
                 WINDOWS_C_ARGUMENTS [p3; p1; p2] s /\
            ...
            (MAYCHANGE [RSP] ,,
             ...
             [memory :> bytes (p3,96);
              memory :> bytes (word_sub stackpointer (word 256),256)])`,
  MATCH_ACCEPT_TAC(ADD_IBT_RULE SM2_MONTJMIXADD_NOIBT_WINDOWS_SUBROUTINE_SAFE));;
