...
(**** print_literal_from_elf "arm/curve25519/edwards25519_epdouble.o";;
 ****)

let edwards25519_epdouble_mc = define_assert_from_elf
  "edwards25519_epdouble_mc" "arm/curve25519/edwards25519_epdouble.o"
[
...
let EDWARDS25519_EPDOUBLE_EXEC = ARM_MK_EXEC_RULE edwards25519_epdouble_mc;;
...
let LOCAL_MUL_P25519_TAC =
  ARM_MACRO_SIM_ABBREV_TAC edwards25519_epdouble_mc 180 lvs
   `!(t:armstate) pcin pcout p3 n3 p1 n1 p2 n2.
      ...
      nonoverlapping (word pc,0x14dc) (word_add (read p3 t) (word n3),8 * 4)
      ==> ensures arm
           (\s. aligned_bytes_loaded s (word pc) edwards25519_epdouble_mc /\
                read PC s = pcin /\
                read SP s = read SP t /\
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ARM_ACCSTEPS_TAC EDWARDS25519_EPDOUBLE_EXEC [9;11;12;14] (1--14) THEN
  ...
  ARM_ACCSTEPS_TAC EDWARDS25519_EPDOUBLE_EXEC
  ...
  ARM_ACCSTEPS_TAC EDWARDS25519_EPDOUBLE_EXEC [34;36;37;39] (26--39) THEN
  RULE_ASSUM_TAC(REWRITE_RULE
   [VAL_WORD_MADDL_0; VAL_WORD_USHR; VAL_WORD_SHL;
    DIVMOD_32_32; DIMINDEX_64]) THEN
  SUBGOAL_THEN
  ...
  ARM_ACCSTEPS_TAC EDWARDS25519_EPDOUBLE_EXEC (69--72) (69--72) THEN
  MAP_EVERY ABBREV_TAC
  ...
  ARM_STEPS_TAC EDWARDS25519_EPDOUBLE_EXEC (113--137) THEN
  ...
  STRIP_TAC THEN
  ARM_ACCSTEPS_TAC EDWARDS25519_EPDOUBLE_EXEC [142;144;146;150] (138--150) THEN
  ...
  ARM_ACCSTEPS_TAC EDWARDS25519_EPDOUBLE_EXEC (154--157) (151--160) THEN
...
let LOCAL_SQR_4_TAC =
  ARM_MACRO_SIM_ABBREV_TAC edwards25519_epdouble_mc 130 lvs
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ARM_ACCSTEPS_TAC EDWARDS25519_EPDOUBLE_EXEC [47;48;51;53] (47--53) THEN
  ...
  ARM_ACCSTEPS_TAC EDWARDS25519_EPDOUBLE_EXEC (54--57) (54--57) THEN
  SUBGOAL_THEN
  ...
  ARM_ACCSTEPS_TAC EDWARDS25519_EPDOUBLE_EXEC (80--90) (80--90) THEN
  RULE_ASSUM_TAC(REWRITE_RULE
  ...
  ARM_STEPS_TAC EDWARDS25519_EPDOUBLE_EXEC (91--115) THEN
  MAP_EVERY (fun t -> REABBREV_TAC t THEN POP_ASSUM MP_TAC)
   [`u0 = read X2 s115`;
    `u1 = read X3 s115`;
    `u2 = read X4 s115`;
  ...
  SIMP_TAC[MOD_LT; VAL_BOUND_64; ARITH_RULE
   `n < 2 EXP 64 ==> n MOD 2 EXP 32 * 38 < 2 EXP 64`] THEN
  STRIP_TAC THEN
  ARM_ACCSTEPS_TAC EDWARDS25519_EPDOUBLE_EXEC [120;122;124;128] (116--130) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
...
let LOCAL_ADD_4_TAC =
  ARM_MACRO_SIM_ABBREV_TAC edwards25519_epdouble_mc 10 lvs
   `!(t:armstate) pcin pcout p3 n3 p1 n1 p2 n2.
      !m. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 4)) t = m
      ==>
      !n. read(memory :> bytes(word_add (read p2 t) (word n2),8 * 4)) t = n
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ARM_ACCSTEPS_TAC EDWARDS25519_EPDOUBLE_EXEC [3;4;7;8] (1--10) THEN
...
let LOCAL_ADD_TWICE4_TAC =
  ARM_MACRO_SIM_ABBREV_TAC edwards25519_epdouble_mc 16 lvs
   `!(t:armstate) pcin pcout p3 n3 p1 n1 p2 n2.
      !m. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 4)) t = m
      ==>
      !n. read(memory :> bytes(word_add (read p2 t) (word n2),8 * 4)) t = n
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  FIRST_ASSUM(BIGNUM_LDIGITIZE_TAC "n_" o lhand o concl) THEN
  FIRST_ASSUM(BIGNUM_LDIGITIZE_TAC "m_" o lhand o concl) THEN
  ARM_ACCSTEPS_TAC EDWARDS25519_EPDOUBLE_EXEC (1--8) (1--8) THEN
  SUBGOAL_THEN `carry_s8 <=> 2 EXP 256 <= m + n` SUBST_ALL_TAC THENL
   [MATCH_MP_TAC FLAG_FROM_CARRY_LE THEN EXISTS_TAC `256` THEN
    ...
    DISCH_THEN(fun th -> REWRITE_TAC[th]) THEN BOUNDER_TAC[];
    ALL_TAC] THEN
  ARM_ACCSTEPS_TAC EDWARDS25519_EPDOUBLE_EXEC (11--14) (9--16) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
...
let LOCAL_DOUBLE_TWICE4_TAC =
  ARM_MACRO_SIM_ABBREV_TAC edwards25519_epdouble_mc 14 lvs
   `!(t:armstate) pcin pcout p3 n3 p1 n1.
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
  FIRST_ASSUM(BIGNUM_LDIGITIZE_TAC "n_" o lhand o concl) THEN
  ARM_ACCSTEPS_TAC EDWARDS25519_EPDOUBLE_EXEC (1--6) (1--6) THEN
  SUBGOAL_THEN `carry_s6 <=> 2 EXP 256 <= 2 * n` SUBST_ALL_TAC THENL
   [MATCH_MP_TAC FLAG_FROM_CARRY_LE THEN EXISTS_TAC `256` THEN
    ...
    DISCH_THEN(fun th -> REWRITE_TAC[th]) THEN BOUNDER_TAC[];
    ALL_TAC] THEN
  ARM_ACCSTEPS_TAC EDWARDS25519_EPDOUBLE_EXEC (9--12) (7--14) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
  REWRITE_TAC[num_congruent; GSYM INT_OF_NUM_ADD] THEN
  MATCH_MP_TAC(MESON[INT_OF_NUM_LT]
   `!x':int. (x' == a) (mod p) /\ x = x'
...
let EDWARDS25519_EPDOUBLE_CORRECT = time prove
 (`!p3 p1 T1 pc stackpointer.
    ...
    ALL (nonoverlapping (stackpointer,160))
        [(word pc,0x14dc); (p3,128); (p1,96)] /\
    nonoverlapping (p3,128) (word pc,0x14dc)
    ==> ensures arm
         (\s. aligned_bytes_loaded s (word pc) edwards25519_epdouble_mc /\
...
let EDWARDS25519_EPDOUBLE_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 T1 pc stackpointer returnaddress.
  ...
  ARM_ADD_RETURN_STACK_TAC EDWARDS25519_EPDOUBLE_EXEC
    EDWARDS25519_EPDOUBLE_CORRECT `[X19; X20]` 176);;
(* ------------------------------------------------------------------------- *)
...
let EDWARDS25519_EPDOUBLE_SAFE = time prove
 (`exists f_events.
       forall e p3 p1 pc stackpointer.
           aligned 16 stackpointer /\
           ALL (nonoverlapping (stackpointer,160))
           [word pc,LENGTH edwards25519_epdouble_mc; p1,96; p3,128] /\
           nonoverlapping (p3,128) (word pc,LENGTH edwards25519_epdouble_mc)
           ==> ensures arm
               (\s.
                    aligned_bytes_loaded s (word pc) edwards25519_epdouble_mc /\
                    read PC s = word (pc + 0x8) /\
                    read SP s = stackpointer /\
                    C_ARGUMENTS [p3; p1] s /\
                    read events s = e)
               (\s.
                    read PC s = word (pc + 0x14d0) /\
                    (exists e2.
                         read events s = APPEND e2 e /\
                         e2 = f_events p1 p3 pc stackpointer /\
                         memaccess_inbounds e2
                         [p1,96; p3,128; stackpointer,160]
                         [p3,128; stackpointer,160]))
               (MAYCHANGE
                [PC; X0; X1; X2; X3; X4; X5; X6; X7; X8; X9; X10; X11; X12; X13; X14; X15; X16; X17; X19] ,,
                MAYCHANGE MODIFIABLE_SIMD_REGS ,,
                MAYCHANGE SOME_FLAGS ,,
                MAYCHANGE [events] ,,
                MAYCHANGE
                [memory :> bytes (p3,128); memory :> bytes (stackpointer,160)])`,
       ...
       forall e p3 p1 pc stackpointer returnaddress.
        ...
        ==> ensures arm
          ...
          (MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(p3,128);
                      memory :> bytes(word_sub stackpointer (word 176),176)])`,
  REWRITE_TAC[fst EDWARDS25519_EPDOUBLE_EXEC] THEN
  ARM_ADD_RETURN_STACK_TAC EDWARDS25519_EPDOUBLE_EXEC
    (REWRITE_RULE[fst EDWARDS25519_EPDOUBLE_EXEC]EDWARDS25519_EPDOUBLE_SAFE)
    `[X19; X20]` 176 THEN
