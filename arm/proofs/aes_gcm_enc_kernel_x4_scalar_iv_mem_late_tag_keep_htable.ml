...
(* print_literal_from_elf "arm/aes_gcm/aes_gcm_enc_kernel_x4_scalar_iv_mem_late_tag_keep_htable.o";; *)

let aes_gcm_enc_kernel_x4_scalar_iv_mem_late_tag_keep_htable_mc =
  define_from_elf "aes_gcm_enc_kernel_x4_scalar_iv_mem_late_tag_keep_htable_mc"
    "arm/aes_gcm/aes_gcm_enc_kernel_x4_scalar_iv_mem_late_tag_keep_htable.o";;

let AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_EXEC = ARM_MK_EXEC_RULE aes_gcm_enc_kernel_x4_scalar_iv_mem_late_tag_keep_htable_mc;;
...
let AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_CORRECT = prove
 (`!in_p out_p len_bits tag_p ivec_p key_p htable_p tag0 nonce rk inblock pc
     stackpointer.
       ...
       ALLPAIRS nonoverlapping
        ...
        [(word pc, LENGTH aes_gcm_enc_kernel_x4_scalar_iv_mem_late_tag_keep_htable_mc);
    ...
    ==>
    ensures arm
      (\s. aligned_bytes_loaded s (word pc) aes_gcm_enc_kernel_x4_scalar_iv_mem_late_tag_keep_htable_mc /\
  ...
  REWRITE_TAC[ALLPAIRS; PAIRWISE; ALL; fst AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_EXEC] THEN
  ...
  REWRITE_TAC[htable_mem_4; GSYM CONJ_ASSOC] THEN CONJ_TAC THENL
   [ENSURES_INIT_TAC "s0" THEN
    ...
    ARM_STEPS_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_EXEC (1--29) THEN
  ...
  REWRITE_TAC[htable_mem_4; GSYM CONJ_ASSOC] THEN CONJ_TAC THENL
   [ASM_CASES_TAC `loop_count = 0` THENL
     [POP_ASSUM SUBST_ALL_TAC THEN
      ARM_SIM_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_EXEC [1] THEN
    ...
    ASM_REWRITE_TAC[htable_mem_4; GSYM CONJ_ASSOC] THEN REPEAT CONJ_TAC THENL
     [ARM_SIM_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_EXEC [1] THEN
      ...
      (*** 5/39/76/113, offsets 208/192/176/160 descending) then reloads it        ***)
      (*** ("ldr q0,[sp,#OFF]" at the next step); splice MERGE_CTR128_TAC after     ***)
      (*** each stp so the 128-bit reload resolves against the two 64-bit stores.   ***)
      MAP_EVERY(fun n -> ARM_STEPS_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_EXEC [n] THEN
      ...
      MAP_EVERY(fun n -> ARM_STEPS_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_EXEC [n] THEN
            RULE_ASSUM_TAC(CONV_RULE(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV)))
          (13--14) THEN
      MERGE_CTR128_TAC 160 "s14" THEN
      MAP_EVERY(fun n -> ARM_STEPS_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_EXEC [n] THEN
            RULE_ASSUM_TAC(CONV_RULE(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV)))
          (15--22) THEN
      MERGE_CTR128_TAC 192 "s22" THEN
      MAP_EVERY(fun n -> ARM_STEPS_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_EXEC [n] THEN
            RULE_ASSUM_TAC(CONV_RULE(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV)))
          (23--23) THEN
      ...
      (*** Scalar counter reconstruction.  Unlike scalar_iv_mem (which increments   ***)
      (*** w13 per block), the late_tag schedule derives each block's counter as     ***)
      (*** "add w14,w13,#N" (N=3,2,1,0) from a fixed base w13 = 4*i+2.  CTR_ZX_NORM   ***)
      (*** collapses the resulting word_zx nest (both the bare 4-deep form and the    ***)
      (*** word_add-with-offset form), GSYM WORD_ADD + the (4*i+2)+N fold turn the     ***)
      (*** counter word into word(4*i+2+N), then CTR_BLOCK_BUILD_INSERT rebuilds       ***)
      (*** rev8(ctr_block nonce (4*i+2+N)).                                            ***)
...
let AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_SUBROUTINE_CORRECT = prove
 (`!in_p out_p len_bits tag_p ivec_p key_p htable_p tag0 nonce rk inblock
  ...
  REWRITE_TAC[fst AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_EXEC; htable_mem_4] THEN
  CONV_TAC(ONCE_DEPTH_CONV WORDLIST_FROM_MEMORY_CONV) THEN
  ARM_ADD_RETURN_STACK_TAC
    ~pre_post_nsteps:(11, 11)
    AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_EXEC
    (CONV_RULE(ONCE_DEPTH_CONV WORDLIST_FROM_MEMORY_CONV)
       (REWRITE_RULE[fst AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_EXEC; htable_mem_4]
          AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_CORRECT))
...
(*                                                                            *)
(* The event trace `e2` produced from pc+0x2c to pc+0x400 is a function of     *)
(* the PUBLIC arguments only (in/out/tag/ivec/key/htable pointers, len_bits,   *)
(* pc, stackpointer) -- established by the outer `exists f_events` over public  *)
(* data -- and every memory access it makes lies in the declared readable /    *)
(* writable ranges (memaccess_inbounds).  This gives constant-time execution    *)
(* and memory safety of the kernel body.                                       *)
(*                                                                            *)
(* NB (events / UP2 loops): ENSURES_EVENTS_WHILE_UP2_TAC's "pc2" is the         *)
(* fall-through target ONE PAST the cbnz (the correctness proof, using          *)
(* ENSURES_WHILE_UP_TAC, points at the cbnz itself); the loop body must step    *)
(* THROUGH the cbnz.  So the main loop is 0xa4<->0x324 with a 160-instruction   *)
(* body and the tail loop is 0x328<->0x3ec with a 49-instruction body.          *)
(* ------------------------------------------------------------------------- *)

needs "arm/proofs/consttime.ml";;

let EXEC = AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_EXEC;;
...
let AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_SAFE = prove
 (`exists f_events.
...
let AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_SUBROUTINE_SAFE = prove
 (`exists f_events.
