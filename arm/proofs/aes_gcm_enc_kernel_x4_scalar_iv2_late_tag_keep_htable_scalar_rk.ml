...
(* print_literal_from_elf "arm/aes_gcm/aes_gcm_enc_kernel_x4_scalar_iv2_late_tag_keep_htable_scalar_rk.o";; *)

let aes_gcm_enc_kernel_x4_scalar_iv2_late_tag_keep_htable_scalar_rk_mc =
  define_from_elf "aes_gcm_enc_kernel_x4_scalar_iv2_late_tag_keep_htable_scalar_rk_mc"
    "arm/aes_gcm/aes_gcm_enc_kernel_x4_scalar_iv2_late_tag_keep_htable_scalar_rk.o";;

let AES_GCM_ENC_KERNEL_X4_SCALAR_IV2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC = ARM_MK_EXEC_RULE aes_gcm_enc_kernel_x4_scalar_iv2_late_tag_keep_htable_scalar_rk_mc;;
...
(* This variant (scalar_iv2) assembles the counter block in a vector register with   *)
(* "dup v0.2d,x11" then "fmov v0.d[1],x14".  The dup gives Q0 low half = word_join    *)
(* x11 x11 (the simulator EXPANDS word_duplicate to two copies), then the fmov        *)
(* inserts x14 into the top 64 bits — reconstructing the reversed ctr_block.          *)
...
(* save instructions) and ends at pc + 0x43c (first ldp of the postamble).   *)
...
let AES_GCM_ENC_KERNEL_X4_SCALAR_IV2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_CORRECT = prove
 (`!in_p out_p len_bits tag_p ivec_p key_p htable_p tag0 nonce rk inblock pc
     stackpointer.
       ...
       ALLPAIRS nonoverlapping
        [(out_p, 16 * val len_bits DIV 128); (tag_p, 16); (ivec_p, 16);
         (word_add stackpointer (word 224), 64)]
        [(word pc, LENGTH aes_gcm_enc_kernel_x4_scalar_iv2_late_tag_keep_htable_scalar_rk_mc);
    ...
    ==>
    ensures arm
      (\s. aligned_bytes_loaded s (word pc) aes_gcm_enc_kernel_x4_scalar_iv2_late_tag_keep_htable_scalar_rk_mc /\
      ...
      (MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
       ...
       MAYCHANGE [memory :> bytes(out_p, 16 * val len_bits DIV 128);
                  ...
                  memory :> bytes(word_add stackpointer (word 224), 64)])`,
  ...
  REWRITE_TAC[ALLPAIRS; PAIRWISE; ALL; fst AES_GCM_ENC_KERNEL_X4_SCALAR_IV2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC] THEN
  ...
  REWRITE_TAC[htable_mem_4; GSYM CONJ_ASSOC] THEN CONJ_TAC THENL
   [ENSURES_INIT_TAC "s0" THEN
    ...
    (*** The scalar_rk variant loads the final round key (EL 10 rk) into the   ***)
    (*** scalar pair X20/X21 via "ldp x20,x21,[x5,#160]", so that key cell     ***)
    (*** must be present at 64-bit granularity.  Split it (leaving the k=0..9  ***)
    (*** cells as bytes128 for the "ldr q18..q27" vector loads).               ***)
    ...
    ARM_STEPS_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC (1--29) THEN
  ...
  REWRITE_TAC[htable_mem_4; GSYM CONJ_ASSOC] THEN CONJ_TAC THENL
   [ASM_CASES_TAC `loop_count = 0` THENL
     [POP_ASSUM SUBST_ALL_TAC THEN
      ARM_SIM_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC [1] THEN
    ...
    ENSURES_WHILE_UP_TAC `loop_count:num` `pc + 0x0a4` `pc + 0x350`
      `\i s.
    ...
    ASM_REWRITE_TAC[htable_mem_4; GSYM CONJ_ASSOC] THEN REPEAT CONJ_TAC THENL
     [ARM_SIM_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC [1] THEN
      ...
      (*** counter block on the stack ("stp x11,x14,[sp,#OFF]" at steps 5/42/82/122, ***)
      ...
      (*** same slot ("stp x22,x23,[sp,#OFF]" at steps 28/65/105/145, "ldr q29");   ***)
  ...
  ENSURES_WHILE_UP_TAC `loop_remain:num` `pc + 0x358` `pc + 0x424`
    `\i s.
   ...
   [ENSURES_INIT_TAC "s0" THEN
    ...
    FIRST_X_ASSUM(STRIP_ASSUME_TAC o CONV_RULE SPLIT_INPUT_TAIL_CONV o
      check (fun th -> let c = concl th in
        ...
        can (find_term (fun t -> is_const t && fst(dest_const t) = "bytes128"))
            (lhs c))) THEN
    (*** The single tail block assembles its counter in a vector register via     ***)
    (*** "dup v0.2d,x11 ; fmov v0.d[1],x14" (steps 5/6, Q0 built directly -- no     ***)
    (*** stack merge).  After the 9 AES rounds the scalar final round round-trips   ***)
    (*** the ciphertext through the stack ("stp x22,x23,[sp,#224]" at step 28,      ***)
...
let AES_GCM_ENC_KERNEL_X4_SCALAR_IV2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_SUBROUTINE_CORRECT = prove
 (`!in_p out_p len_bits tag_p ivec_p key_p htable_p tag0 nonce rk inblock
  ...
  REWRITE_TAC[fst AES_GCM_ENC_KERNEL_X4_SCALAR_IV2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC; htable_mem_4] THEN
  CONV_TAC(ONCE_DEPTH_CONV WORDLIST_FROM_MEMORY_CONV) THEN
  ARM_ADD_RETURN_STACK_TAC
    ~pre_post_nsteps:(11, 11)
    AES_GCM_ENC_KERNEL_X4_SCALAR_IV2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC
    (CONV_RULE(ONCE_DEPTH_CONV WORDLIST_FROM_MEMORY_CONV)
       (REWRITE_RULE[fst AES_GCM_ENC_KERNEL_X4_SCALAR_IV2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC; htable_mem_4]
          AES_GCM_ENC_KERNEL_X4_SCALAR_IV2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_CORRECT))
