...
(**** print_literal_from_elf "arm/secp256k1/secp256k1_jadd_alt.o";;
 ****)

let secp256k1_jadd_alt_mc = define_assert_from_elf
  "secp256k1_jadd_alt_mc" "arm/secp256k1/secp256k1_jadd_alt.o"
[
...
let SECP256K1_JADD_ALT_EXEC = ARM_MK_EXEC_RULE secp256k1_jadd_alt_mc;;
...
let LOCAL_SQR_P256K1_TAC =
  ARM_MACRO_SIM_ABBREV_TAC secp256k1_jadd_alt_mc 78 lvs
  `!(t:armstate) pcin pcout p3 n3 p1 n1.
    !n. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 4)) t = n
    ==>
    aligned 16 (read SP t) /\
    nonoverlapping (word pc,0x1a80) (word_add (read p3 t) (word n3),32)
    ==> ensures arm
         (\s. aligned_bytes_loaded s (word pc) secp256k1_jadd_alt_mc /\
              read PC s = pcin /\
              read SP s = read SP t /\
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ARM_ACCSTEPS_TAC SECP256K1_JADD_ALT_EXEC (1--45) (1--45) THEN
  ...
  ARM_ACCSTEPS_TAC SECP256K1_JADD_ALT_EXEC (46--64) (46--64) THEN
  ...
  ARM_STEPS_TAC SECP256K1_JADD_ALT_EXEC [93] THEN
  ...
  ARM_ACCSTEPS_TAC SECP256K1_JADD_ALT_EXEC (66--78) (66--78) THEN
...
let LOCAL_MUL_P256K1_TAC =
  ARM_MACRO_SIM_ABBREV_TAC secp256k1_jadd_alt_mc 100 lvs
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ARM_ACCSTEPS_TAC SECP256K1_JADD_ALT_EXEC (1--67) (1--67) THEN
  MAP_EVERY ABBREV_TAC
  ...
  ARM_ACCSTEPS_TAC SECP256K1_JADD_ALT_EXEC (68--86) (68--86) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[COND_SWAP; GSYM WORD_BITVAL]) THEN
  ABBREV_TAC
   `ca = bignum_of_wordlist
  ...
  SUBGOAL_THEN `ca DIV 2 EXP 256 = val(sum_s86:int64)` SUBST_ALL_TAC THENL
   [EXPAND_TAC "ca" THEN CONV_TAC(LAND_CONV BIGNUM_OF_WORDLIST_DIV_CONV) THEN
    ...
    FIRST_ASSUM(ASSUME_TAC o MATCH_MP (ARITH_RULE
     `n + 1 < 2 EXP 64 ==> n < 2 EXP 64 - 1`))] THEN
  ARM_STEPS_TAC SECP256K1_JADD_ALT_EXEC [87] THEN
  ...
  ARM_ACCSTEPS_TAC SECP256K1_JADD_ALT_EXEC (88--100) (88--100) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  CONV_TAC(LAND_CONV BIGNUM_EXPAND_CONV) THEN ASM_REWRITE_TAC[] THEN
  CONV_TAC SYM_CONV THEN MATCH_MP_TAC MOD_UNIQ_BALANCED_REAL THEN
  MAP_EVERY EXISTS_TAC [`val(q:int64)`; `256`] THEN ASM_REWRITE_TAC[] THEN
...
let LOCAL_SUB_P256K1_TAC =
  ARM_MACRO_SIM_ABBREV_TAC secp256k1_jadd_alt_mc 17 lvs
  `!(t:armstate) pcin pcout p3 n3 p1 n1 p2 n2.
    !m. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 4)) t = m
    ==>
    !n. read(memory :> bytes(word_add (read p2 t) (word n2),8 * 4)) t = n
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ARM_ACCSTEPS_TAC SECP256K1_JADD_ALT_EXEC (1--8) (1--8) THEN
  ...
  ARM_ACCSTEPS_TAC SECP256K1_JADD_ALT_EXEC (12--17) (9--17) THEN
...
let SECP256K1_JADD_ALT_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer.
        ...
        ALL (nonoverlapping (stackpointer,224))
            [(word pc,0x1a80); (p1,96); (p2,96); (p3,96)] /\
        nonoverlapping (p3,96) (word pc,0x1a80)
        ==> ensures arm
             (\s. aligned_bytes_loaded s (word pc) secp256k1_jadd_alt_mc /\
  ...
  LOCAL_SQR_P256K1_TAC 3 ["z1sq";"z_1"] THEN
  ...
  ARM_STEPS_TAC SECP256K1_JADD_ALT_EXEC (27--85) THEN
...
let SECP256K1_JADD_ALT_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
  ...
  ARM_ADD_RETURN_STACK_TAC SECP256K1_JADD_ALT_EXEC
   SECP256K1_JADD_ALT_CORRECT `[]` 224);;
...
let SECP256K1_JADD_ALT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e p3 p1 p2 pc stackpointer returnaddress.
        aligned 16 stackpointer /\
        ALL (nonoverlapping (word_sub stackpointer (word 224),224))
            [(word pc,LENGTH secp256k1_jadd_alt_mc); (p1,96); (p2,96); (p3,96)] /\
        nonoverlapping (p3,96) (word pc,LENGTH secp256k1_jadd_alt_mc)
        ==> ensures arm
             (\s. aligned_bytes_loaded s (word pc) secp256k1_jadd_alt_mc /\
                  read PC s = word pc /\
                  read SP s = stackpointer /\
             ...
             (\s. read PC s = returnaddress /\
                  (exists e2.
                         ...
                         e2 = f_events p1 p2 p3 pc
                              (word_sub stackpointer (word 224))
                              returnaddress /\
          ...
          (MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(p3,96);
                      memory :> bytes(word_sub stackpointer (word 224),224)])`,
  REWRITE_TAC[fst SECP256K1_JADD_ALT_EXEC] THEN
  ARM_ADD_RETURN_STACK_TAC SECP256K1_JADD_ALT_EXEC
    (REWRITE_RULE[fst SECP256K1_JADD_ALT_EXEC]SECP256K1_JADD_ALT_SAFE)
    `[]` 224 THEN
