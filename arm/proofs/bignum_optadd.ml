...
let full_spec,public_vars = mk_safety_spec
    ...
    BIGNUM_OPTADD_SUBROUTINE_CORRECT
    BIGNUM_OPTADD_EXEC;;
...
let BIGNUM_OPTADD_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e k z x p y pc returnaddress.
           ...
           ==> ensures arm
               (\s.
                    aligned_bytes_loaded s (word pc) bignum_optadd_mc /\
                    read PC s = word pc /\
                    read X30 s = returnaddress /\
                    C_ARGUMENTS [k; z; x; p; y] s /\
                    read events s = e)
  ...
  ASM_CASES_TAC `k = 0` THENL
   [ARM_SIM_TAC ~preprocess_tac:(TRY STRIP_EXISTS_ASSUM_TAC)
        ~canonicalize_pc_diff:false BIGNUM_OPTADD_EXEC (1--2) THEN
  ...
  ENSURES_EVENTS_WHILE_UP2_TAC `k:num` `pc + 0x10` `pc + 0x30`
   `\i s. read X1 s = z /\
          read X2 s = x /\
          read X4 s = y /\
          read X7 s = word (8 * i) /\
  ...
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL [
    (* pre *)
    ARM_SIM_TAC ~preprocess_tac:(TRY STRIP_EXISTS_ASSUM_TAC)
        ~canonicalize_pc_diff:false BIGNUM_OPTADD_EXEC (1--4) THEN
  ...
  REWRITE_TAC[] THEN
  ARM_SIM_TAC ~preprocess_tac:(TRY STRIP_EXISTS_ASSUM_TAC)
      ~canonicalize_pc_diff:false BIGNUM_OPTADD_EXEC (1--8) THEN
  CONJ_TAC THENL
