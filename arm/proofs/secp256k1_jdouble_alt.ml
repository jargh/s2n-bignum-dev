...
let SECP256K1_JDOUBLE_ALT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e p3 p1 pc stackpointer returnaddress.
        aligned 16 stackpointer /\
        ALL (nonoverlapping (word_sub stackpointer (word 384),384))
            [(word pc,LENGTH secp256k1_jdouble_alt_mc); (p1,96); (p3,96)] /\
        nonoverlapping (p3,96) (word pc,LENGTH secp256k1_jdouble_alt_mc)
        ==> ensures arm
             (\s. aligned_bytes_loaded s (word pc) secp256k1_jdouble_alt_mc /\
                  read PC s = word pc /\
                  read SP s = stackpointer /\
             ...
             (\s. read PC s = returnaddress /\
                  (exists e2.
                         ...
                         [p1,96; p3,96; word_sub stackpointer (word 384),384]
                         [p3,96; word_sub stackpointer (word 384),384]))
          (MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(p3,96);
                      memory :> bytes(word_sub stackpointer (word 384),384)])`,
  REWRITE_TAC[fst SECP256K1_JDOUBLE_ALT_EXEC] THEN
  ARM_ADD_RETURN_STACK_TAC SECP256K1_JDOUBLE_ALT_EXEC
    (REWRITE_RULE[fst SECP256K1_JDOUBLE_ALT_EXEC]SECP256K1_JDOUBLE_ALT_SAFE)
    `[]` 384 THEN
