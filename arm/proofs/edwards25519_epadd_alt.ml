...
let edwards25519_epadd_alt_mc = define_assert_from_elf
  "edwards25519_epadd_alt_mc" "arm/curve25519/edwards25519_epadd_alt.o"
[
  ...
  0xa9462287;       (* arm_LDP X7 X8 X20 (Immediate_Offset (iword (&96))) *)
  ...
  0xa9472a89;       (* arm_LDP X9 X10 X20 (Immediate_Offset (iword (&112))) *)
...
let EDWARDS25519_EPADD_ALT_EXEC = ARM_MK_EXEC_RULE edwards25519_epadd_alt_mc;;
...
let LOCAL_MUL_P25519_TAC =
  ARM_MACRO_SIM_ABBREV_TAC edwards25519_epadd_alt_mc 100 lvs
   `!(t:armstate) pcin pcout p3 n3 p1 n1 p2 n2.
      ...
      nonoverlapping (word pc,0xff0) (word_add (read p3 t) (word n3),8 * 4)
      ==> ensures arm
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ARM_ACCSTEPS_TAC EDWARDS25519_EPADD_ALT_EXEC (1--67) (1--67) THEN
  ...
  ARM_ACCSTEPS_TAC EDWARDS25519_EPADD_ALT_EXEC (68--92) (68--92) THEN
  ...
  ARM_ACCSTEPS_TAC EDWARDS25519_EPADD_ALT_EXEC (93--100) (93--100) THEN
...
let LOCAL_MUL_4_TAC =
  ARM_MACRO_SIM_ABBREV_TAC edwards25519_epadd_alt_mc 94 lvs
   `!(t:armstate) pcin pcout p3 n3 p1 n1 p2 n2.
      !m. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 4)) t = m
      ==>
      !n. read(memory :> bytes(word_add (read p2 t) (word n2),8 * 4)) t = n
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ARM_ACCSTEPS_TAC EDWARDS25519_EPADD_ALT_EXEC (68--92) (68--94) THEN
  RULE_ASSUM_TAC(REWRITE_RULE
   [COND_SWAP; GSYM WORD_BITVAL; VAL_WORD_BITVAL]) THEN
  SUBGOAL_THEN
   `(val(sum_s86:int64) + 1) * p_25519 <= (38 * h + l) + p_25519 /\
...
let LOCAL_ADD_4_TAC =
  ARM_MACRO_SIM_ABBREV_TAC edwards25519_epadd_alt_mc 10 lvs
   `!(t:armstate) pcin pcout p3 n3 p1 n1 p2 n2.
      !m. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 4)) t = m
      ==>
      !n. read(memory :> bytes(word_add (read p2 t) (word n2),8 * 4)) t = n
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  FIRST_ASSUM(BIGNUM_LDIGITIZE_TAC "n_" o lhand o concl) THEN
  FIRST_ASSUM(BIGNUM_LDIGITIZE_TAC "m_" o lhand o concl) THEN
  ARM_ACCSTEPS_TAC EDWARDS25519_EPADD_ALT_EXEC [3;4;7;8] (1--10) THEN
...
let LOCAL_DOUBLE_4_TAC =
  ARM_MACRO_SIM_ABBREV_TAC edwards25519_epadd_alt_mc 8 lvs
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN ENSURES_INIT_TAC "s0" THEN
  FIRST_ASSUM(BIGNUM_LDIGITIZE_TAC "n_" o lhand o concl) THEN
  ARM_ACCSTEPS_TAC EDWARDS25519_EPADD_ALT_EXEC [2;3;5;6] (1--8) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
  CONV_TAC(LAND_CONV BIGNUM_LEXPAND_CONV) THEN ASM_REWRITE_TAC[] THEN
  MATCH_MP_TAC CONG_IMP_EQ THEN EXISTS_TAC `2 EXP 256` THEN
  REPEAT CONJ_TAC THENL
...
let LOCAL_SUB_4_TAC =
  ARM_MACRO_SIM_ABBREV_TAC edwards25519_epadd_alt_mc 16 lvs
   `!(t:armstate) pcin pcout p3 n3 p1 n1 p2 n2.
      !m. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 4)) t = m
      ==>
      !n. read(memory :> bytes(word_add (read p2 t) (word n2),8 * 4)) t = n
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  FIRST_ASSUM(BIGNUM_LDIGITIZE_TAC "n_" o lhand o concl) THEN
  FIRST_ASSUM(BIGNUM_LDIGITIZE_TAC "m_" o lhand o concl) THEN
  ARM_ACCSTEPS_TAC EDWARDS25519_EPADD_ALT_EXEC
...
let LOCAL_ADD_TWICE4_TAC =
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  FIRST_ASSUM(BIGNUM_LDIGITIZE_TAC "n_" o lhand o concl) THEN
  FIRST_ASSUM(BIGNUM_LDIGITIZE_TAC "m_" o lhand o concl) THEN
  ARM_ACCSTEPS_TAC EDWARDS25519_EPADD_ALT_EXEC (1--8) (1--8) THEN
  SUBGOAL_THEN `carry_s8 <=> 2 EXP 256 <= m + n` SUBST_ALL_TAC THENL
   [MATCH_MP_TAC FLAG_FROM_CARRY_LE THEN EXISTS_TAC `256` THEN
    ...
    DISCH_THEN(fun th -> REWRITE_TAC[th]) THEN BOUNDER_TAC[];
    ALL_TAC] THEN
  ARM_ACCSTEPS_TAC EDWARDS25519_EPADD_ALT_EXEC (11--14) (9--16) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN DISCH_TAC THEN
...
let EDWARDS25519_EPADD_ALT_CORRECT = time prove
 (`!p3 p1 Q1 p2 Q2 pc stackpointer.
    ...
    ALL (nonoverlapping (stackpointer,192))
        [(word pc,0xff0); (p3,128); (p1,128); (p2,128)] /\
    nonoverlapping (p3,128) (word pc,0xff0)
    ==> ensures arm
         (\s. aligned_bytes_loaded s (word pc) edwards25519_epadd_alt_mc /\
  ...
  ARM_STEPS_TAC EDWARDS25519_EPADD_ALT_EXEC (12--29) THEN
  SUBGOAL_THEN
