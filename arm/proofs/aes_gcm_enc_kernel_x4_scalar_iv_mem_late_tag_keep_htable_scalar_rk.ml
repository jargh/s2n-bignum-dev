...
(* print_literal_from_elf "arm/aes_gcm/aes_gcm_enc_kernel_x4_scalar_iv_mem_late_tag_keep_htable_scalar_rk.o";; *)

let aes_gcm_enc_kernel_x4_scalar_iv_mem_late_tag_keep_htable_scalar_rk_mc =
  define_from_elf "aes_gcm_enc_kernel_x4_scalar_iv_mem_late_tag_keep_htable_scalar_rk_mc"
    "arm/aes_gcm/aes_gcm_enc_kernel_x4_scalar_iv_mem_late_tag_keep_htable_scalar_rk.o";;

let AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC = ARM_MK_EXEC_RULE aes_gcm_enc_kernel_x4_scalar_iv_mem_late_tag_keep_htable_scalar_rk_mc;;
...
let AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_SCALAR_RK_CORRECT = prove
 (`!in_p out_p len_bits tag_p ivec_p key_p htable_p tag0 nonce rk inblock pc
     stackpointer.
       ...
       ALLPAIRS nonoverlapping
        ...
        [(word pc, LENGTH aes_gcm_enc_kernel_x4_scalar_iv_mem_late_tag_keep_htable_scalar_rk_mc);
    ...
    ==>
    ensures arm
      (\s. aligned_bytes_loaded s (word pc) aes_gcm_enc_kernel_x4_scalar_iv_mem_late_tag_keep_htable_scalar_rk_mc /\
  ...
  REWRITE_TAC[ALLPAIRS; PAIRWISE; ALL; fst AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC] THEN
  ...
  REWRITE_TAC[htable_mem_4; GSYM CONJ_ASSOC] THEN CONJ_TAC THENL
   [ENSURES_INIT_TAC "s0" THEN
    ...
    ARM_STEPS_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC (1--29) THEN
  ...
  REWRITE_TAC[htable_mem_4; GSYM CONJ_ASSOC] THEN CONJ_TAC THENL
   [ASM_CASES_TAC `loop_count = 0` THENL
     [POP_ASSUM SUBST_ALL_TAC THEN
      ARM_SIM_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC [1] THEN
    ...
    ASM_REWRITE_TAC[htable_mem_4; GSYM CONJ_ASSOC] THEN REPEAT CONJ_TAC THENL
     [ARM_SIM_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC [1] THEN
      ...
      MAP_EVERY(fun n -> ARM_STEPS_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC [n] THEN
      ...
      MAP_EVERY(fun n -> ARM_STEPS_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC [n] THEN
            RULE_ASSUM_TAC(CONV_RULE(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV)))
          (84--105) THEN
      MERGE_CTR128_TAC 176 "s105" THEN
      MAP_EVERY(fun n -> ARM_STEPS_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC [n] THEN
            RULE_ASSUM_TAC(CONV_RULE(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV)))
          (106--122) THEN
      MERGE_CTR128_TAC 160 "s122" THEN
      MAP_EVERY(fun n -> ARM_STEPS_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC [n] THEN
            RULE_ASSUM_TAC(CONV_RULE(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV)))
          (123--123) THEN
      DISCARD_MATCHING_ASSUMPTIONS
      ...
      MAP_EVERY(fun n -> ARM_STEPS_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC [n] THEN
            RULE_ASSUM_TAC(CONV_RULE(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV)))
          (124--145) THEN
      MERGE_CTR128_TAC 160 "s145" THEN
      MAP_EVERY(fun n -> ARM_STEPS_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC [n] THEN
            RULE_ASSUM_TAC(CONV_RULE(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV)))
          (146--171) THEN
  ...
  ASM_REWRITE_TAC[htable_mem_4; GSYM CONJ_ASSOC] THEN REPEAT CONJ_TAC THENL
   [ENSURES_INIT_TAC "s0" THEN
    ...
    (*** merge the two 64-bit stores, discard the slot, then the scalar final    ***)
    (*** round round-trips the ciphertext through the same slot                  ***)
...
let AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_SCALAR_RK_SUBROUTINE_CORRECT = prove
 (`!in_p out_p len_bits tag_p ivec_p key_p htable_p tag0 nonce rk inblock
  ...
  REWRITE_TAC[fst AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC; htable_mem_4] THEN
  CONV_TAC(ONCE_DEPTH_CONV WORDLIST_FROM_MEMORY_CONV) THEN
  ARM_ADD_RETURN_STACK_TAC
    ~pre_post_nsteps:(11, 11)
    AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC
    (CONV_RULE(ONCE_DEPTH_CONV WORDLIST_FROM_MEMORY_CONV)
       (REWRITE_RULE[fst AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC; htable_mem_4]
          AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_KEEP_HTABLE_SCALAR_RK_CORRECT))
