...
let bignum_bitsize_mc = define_assert_from_elf "bignum_bitsize_mc" "arm/generic/bignum_bitsize.o"
[
...
let BIGNUM_BITSIZE_EXEC = ARM_MK_EXEC_RULE bignum_bitsize_mc;;
...
let BIGNUM_BITSIZE_CORRECT = prove
 (`!k a x pc.
        ensures arm
         (\s. aligned_bytes_loaded s (word pc) bignum_bitsize_mc /\
              read PC s = word pc /\
              C_ARGUMENTS [k;a] s /\
  ...
  ASM_CASES_TAC `k = 0` THENL
   [UNDISCH_TAC `x < 2 EXP (64 * k)` THEN
    ...
    ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC BIGNUM_BITSIZE_EXEC [1] THEN
  ...
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [REWRITE_TAC[BIGNUM_FROM_MEMORY_BYTES] THEN
    ENSURES_INIT_TAC "s0" THEN ARM_STEPS_TAC BIGNUM_BITSIZE_EXEC (1--4) THEN
    ...
    FIRST_X_ASSUM(DISJ_CASES_THEN MP_TAC) THENL
     [DISCH_THEN(CONJUNCTS_THEN SUBST_ALL_TAC) THEN
      ...
      UNDISCH_TAC `read (memory :> bytes (a,8 * k)) s4 = x` THEN
    ...
    ASM_REWRITE_TAC[WORD_ADD; WORD_SUB; ARITH_RULE
     `64 <= 64 * i <=> ~(i = 0)`] THEN
  ...
  ASSUME_TAC THENL
   [EXPAND_TAC "x" THEN
    ...
    ALL_TAC] THEN
  ARM_STEPS_TAC BIGNUM_BITSIZE_EXEC (1--6) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
...
let BIGNUM_BITSIZE_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e k a pc returnaddress.
           ensures arm
           (\s.
                aligned_bytes_loaded s (word pc) bignum_bitsize_mc /\
                read PC s = word pc /\
                read X30 s = returnaddress /\
                C_ARGUMENTS [k; a] s /\
                read events s = e)
  ...
  ASM_CASES_TAC `k = 0` THENL
   [ ASM_REWRITE_TAC[] THEN
     ARM_SIM_TAC ~preprocess_tac:(TRY STRIP_EXISTS_ASSUM_TAC) ~canonicalize_pc_diff:false
       BIGNUM_BITSIZE_EXEC (1--2) THEN DISCHARGE_SAFETY_PROPERTY_TAC;
     ALL_TAC ] THEN
  ASM_REWRITE_TAC[ASSUME `val(word k:int64) = k`] THEN
  ENSURES_EVENTS_WHILE_UP2_TAC `k:num` `pc + 0x10` `pc + 0x2c`
   `\i s. read X0 s = word k /\ read X1 s = a /\ read X5 s = word i /\ read X30 s = returnaddress` THEN
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL
   [ ARM_SIM_TAC ~preprocess_tac:(TRY STRIP_EXISTS_ASSUM_TAC) ~canonicalize_pc_diff:false
       BIGNUM_BITSIZE_EXEC (1--4) THEN DISCHARGE_SAFETY_PROPERTY_TAC;

     ALL_TAC;

     REWRITE_TAC[] THEN
     ARM_SIM_TAC ~preprocess_tac:(TRY STRIP_EXISTS_ASSUM_TAC) ~canonicalize_pc_diff:false
       BIGNUM_BITSIZE_EXEC (1--4) THEN DISCHARGE_SAFETY_PROPERTY_TAC ] THEN
  X_GEN_TAC `i:num` THEN STRIP_TAC THEN VAL_INT64_TAC `i:num` THEN REWRITE_TAC[] THEN
  ARM_SIM_TAC ~preprocess_tac:(TRY STRIP_EXISTS_ASSUM_TAC) ~canonicalize_pc_diff:false
    BIGNUM_BITSIZE_EXEC (1--7) THEN
