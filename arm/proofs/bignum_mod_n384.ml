...
let BIGNUM_MOD_N384_SAFE = time prove
 (`exists f_events.
        forall e z k x pc.
            nonoverlapping (word pc,0x16c) (z,48)
            ==> ensures arm
                (\s.
                     aligned_bytes_loaded s (word pc) bignum_mod_n384_mc /\
                     read PC s = word pc /\
                     C_ARGUMENTS [z; k; x] s /\
                     read events s = e)
                (\s.
                ...
                (MAYCHANGE [PC; X1; X2; X3; X4; X5; X6; X7; X8; X9; X10; X11; X12; X13; X14; X15; X16; X17] ,,
  ...
  CONCRETIZE_F_EVENTS_TAC
    `\(x:int64) (z:int64) (k:int64) (pc:num).
      if val k = 0 then f_ev_k0 x z k pc
        ...
        APPEND
          (if val k = 6 then f_ev_6 x z k pc
           else APPEND
            (f_ev_loop_post x z k pc)
            (APPEND
              (ENUMERATEL (val k - 6) (f_ev_loop x z k pc))
  ...
  ASM_CASES_TAC `k' < 6` THENL [
    ...
    STRIP_TAC THENL [
      ...
      ARM_SIM_TAC BIGNUM_MOD_N384_EXEC (1--12) THEN
      DISCHARGE_SAFETY_PROPERTY_TAC;
      ASM_REWRITE_TAC[ARITH] THEN
      SUBST1_TAC (ISPEC `k:int64` (GSYM WORD_VAL)) THEN
      ARM_SIM_TAC BIGNUM_MOD_N384_EXEC (1--15) THEN
      DISCHARGE_SAFETY_PROPERTY_TAC;
      ASM_REWRITE_TAC[ARITH] THEN
      SUBST1_TAC (ISPEC `k:int64` (GSYM WORD_VAL)) THEN
      ARM_SIM_TAC BIGNUM_MOD_N384_EXEC (1--18) THEN
      DISCHARGE_SAFETY_PROPERTY_TAC;
      ASM_REWRITE_TAC[ARITH] THEN
      SUBST1_TAC (ISPEC `k:int64` (GSYM WORD_VAL)) THEN
      ARM_SIM_TAC BIGNUM_MOD_N384_EXEC (1--21) THEN
      DISCHARGE_SAFETY_PROPERTY_TAC;
      ASM_REWRITE_TAC[ARITH] THEN
      SUBST1_TAC (ISPEC `k:int64` (GSYM WORD_VAL)) THEN
      ARM_SIM_TAC BIGNUM_MOD_N384_EXEC (1--24) THEN
      DISCHARGE_SAFETY_PROPERTY_TAC;
      ASM_REWRITE_TAC[ARITH] THEN
      SUBST1_TAC (ISPEC `k:int64` (GSYM WORD_VAL)) THEN
      ARM_SIM_TAC BIGNUM_MOD_N384_EXEC (1--26) THEN
      DISCHARGE_SAFETY_PROPERTY_TAC;
    ];
    ALL_TAC
  ] THEN
  FIRST_ASSUM(STRIP_ASSUME_TAC o MATCH_MP (ARITH_RULE
    `~(k < 6) ==> ~(k = 0) /\ ~(k = 1) /\ ~(k = 2) /\ ~(k = 3) /\ ~(k = 4) /\ ~(k = 5)`)) THEN
  ASM_REWRITE_TAC[] THEN
  ENSURES_EVENTS_SEQUENCE_TAC `pc + 0x80`
   `\s. read X0 s = z /\
        read X2 s = x /\
        read X1 s = word(k' - 6)` THEN CONJ_TAC THENL [
    ARM_SIM_TAC ~canonicalize_pc_diff:false
      BIGNUM_MOD_N384_EXEC (1--32) THEN
    CONJ_TAC THENL [
    ...
    SUBGOAL_THEN
      `word_add (word_shl (word_sub k (word 6)) 3) x:int64 =
       word_add x (word (8 * (k' - 6)))` MP_TAC THENL [
      SUBST1_TAC (ISPEC `k:int64` (GSYM WORD_VAL)) THEN
  ...
  THENL
   [ASM_REWRITE_TAC[SUB_REFL;MULT_0] THEN
    ...
    ARM_STEPS_TAC BIGNUM_MOD_N384_EXEC (1--4) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    DISCHARGE_SAFETY_PROPERTY_TAC;
    ALL_TAC] THEN
  ASM_REWRITE_TAC[] THEN
  ENSURES_EVENTS_WHILE_UP2_TAC `k' - 6` `pc + 0x84` `pc + 0x108`
   `\i s. read X0 s = z /\
          read X2 s = x /\
          read X1 s = word ((k' - 6) - i)` THEN
  ASM_REWRITE_TAC[] THEN REPEAT CONJ_TAC THENL [
    SIMPLE_ARITH_TAC;
    ENSURES_INIT_TAC "s0" THEN STRIP_EXISTS_ASSUM_TAC THEN
    ARM_STEPS_TAC BIGNUM_MOD_N384_EXEC [1] THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    CONJ_TAC THENL [
    ...
    ALL_TAC;
    ENSURES_INIT_TAC "s0" THEN STRIP_EXISTS_ASSUM_TAC THEN
    ARM_STEPS_TAC BIGNUM_MOD_N384_EXEC (1--3) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    DISCHARGE_SAFETY_PROPERTY_TAC
  ] THEN
  REPEAT STRIP_TAC THEN
  ASM_REWRITE_TAC[] THEN
  ENSURES_INIT_TAC "s0" THEN STRIP_EXISTS_ASSUM_TAC THEN
  ARM_STEPS_TAC BIGNUM_MOD_N384_EXEC (1--33) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  CONJ_TAC THENL [
  ...
  SUBST1_TAC (ISPEC `k:int64` (GSYM WORD_VAL)) THEN
  REWRITE_TAC[ASSUME `val (k:int64) = k'`] THEN
  SUBGOAL_THEN `word_sub (word (k' - 6 - i)) (word 1) =
                word (k' - 6 - (i + 1)):int64` SUBST_ALL_TAC THENL [
    IMP_REWRITE_TAC[WORD_SUB2] THEN
    CONJ_TAC THENL [ AP_TERM_TAC THEN SIMPLE_ARITH_TAC; SIMPLE_ARITH_TAC];
    ALL_TAC
  ] THEN
  MAP_EVERY VAL_INT64_TAC [`k':num`; `k'-6`] THEN
  DISCHARGE_SAFETY_PROPERTY_TAC);;
 ...
 (`exists f_events.
        forall e z k x pc returnaddress.
          ...
          ==> ensures arm
                ...
                (MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
                 MAYCHANGE [memory :> bignum(z,6)])`,
  ARM_ADD_RETURN_NOSTACK_TAC BIGNUM_MOD_N384_EXEC BIGNUM_MOD_N384_SAFE
