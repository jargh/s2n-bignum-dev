...
let LOCAL_JADD_SAFETY_TAC (assump_name:string) (n:int) =
  REMOVE_THEN assump_name (fun safety_th ->
    ...
    ARM_SUBROUTINE_SIM_TAC ~is_safety_thm:true
        ...
        `read X2 s`;
        `pc + 0x80c`; `read SP s`; `read X30 s`] n THENL [
      EXISTS_E2_TAC(ref
  ...
  REMOVE_THEN assump_name (fun safety_th ->
    ...
    ARM_SUBROUTINE_SIM_TAC ~is_safety_thm:true
        (sm2_montjscalarmul_alt_mc,SM2_MONTJSCALARMUL_ALT_EXEC,
          0x2690,sm2_montjdouble_alt_mc,th)
        [`e:(uarch_event)list`; `read X0 s`; `read X1 s`;
        `pc + 0x2690`; `read SP s`; `read X30 s`] n THENL [
      EXISTS_E2_TAC(ref
        [`scalar:int64`;`point:int64`;`res:int64`;`pc:num`;
         `stackpointer:int64`;
         (* inside the loop... *)
...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "sm2_montjscalarmul_alt" subroutine_signatures)
    SM2_MONTJSCALARMUL_ALT_CORRECT
    SM2_MONTJSCALARMUL_ALT_EXEC;;

let SM2_MONTJSCALARMUL_ALT_SAFE = time prove
 (`exists f_events.
       forall e res scalar point pc stackpointer.
           ...
           ==> ensures arm
               (\s.
                    aligned_bytes_loaded s (word pc) sm2_montjscalarmul_alt_mc /\
                    read PC s = word (pc + 12) /\
                    read SP s = word_add stackpointer (word 224) /\
                    C_ARGUMENTS [res; scalar; point] s /\
                    read events s = e)
               (\s.
                    read PC s = word (pc + 0x7fc) /\
                    (exists e2.
                         read events s = APPEND e2 e /\
                         e2 = f_events scalar point res pc stackpointer /\
                         memaccess_inbounds e2
                         [scalar,32; point,96; res,96; stackpointer,1216]
                         [res,96; stackpointer,1216]))
               (MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
                MAYCHANGE [X19; X20; X21; X30] ,,
                MAYCHANGE
                [memory :> bytes (res,96);
  ...
  ASSUME_CALLEE_SAFETY_TAILED_TAC SM2_MONTJADD_ALT_SUBROUTINE_SAFE
      "H_JADD_SAFE" THEN
  ASSUME_CALLEE_SAFETY_TAILED_TAC SM2_MONTJDOUBLE_ALT_SUBROUTINE_SAFE
      "H_JDOUBLE_SAFE" THEN
  ...
  CONCRETIZE_F_EVENTS_TAC
    `\(scalar:int64) (point:int64) (res:int64) (pc:num) (stackpointer:int64).
      APPEND
        ...
        (APPEND
          (ENUMERATEL 63 (f_ev_loop scalar point res pc stackpointer))
  ...
  REWRITE_TAC[GSYM SEQ_ASSOC; MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI; ALL] THEN
  REPEAT GEN_TAC THEN STRIP_TAC THEN
  REWRITE_TAC[C_ARGUMENTS] THEN

  (*** Main loop invariant setup. ***)

  ENSURES_EVENTS_WHILE_UP2_TAC `63` `pc + 0x458` `pc + 0x7cc`
   `\i s.
      read SP s = word_add stackpointer (word 224) /\
      read X19 s = res /\
      read X20 s = word (4 * (63 - i))` THEN
  REPEAT CONJ_TAC THENL [
    ARITH_TAC;
    ...
    ARM_STEPS_TAC SM2_MONTJSCALARMUL_ALT_EXEC (1--72) THEN
    LOCAL_JDOUBLE_SAFETY_TAC "H_JDOUBLE_SAFE" 73 THEN
    ARM_STEPS_TAC SM2_MONTJSCALARMUL_ALT_EXEC (74--77) THEN
    LOCAL_JADD_SAFETY_TAC "H_JADD_SAFE" 78 THEN
    ARM_STEPS_TAC SM2_MONTJSCALARMUL_ALT_EXEC (79--81) THEN
    LOCAL_JDOUBLE_SAFETY_TAC "H_JDOUBLE_SAFE" 82 THEN
    ARM_STEPS_TAC SM2_MONTJSCALARMUL_ALT_EXEC (83--86) THEN
    LOCAL_JADD_SAFETY_TAC "H_JADD_SAFE" 87 THEN
    ARM_STEPS_TAC SM2_MONTJSCALARMUL_ALT_EXEC (88--90) THEN
    LOCAL_JDOUBLE_SAFETY_TAC "H_JDOUBLE_SAFE" 91 THEN
    ARM_STEPS_TAC SM2_MONTJSCALARMUL_ALT_EXEC (92--95) THEN
    LOCAL_JADD_SAFETY_TAC "H_JADD_SAFE" 96 THEN
    ARM_STEPS_TAC SM2_MONTJSCALARMUL_ALT_EXEC (97--99) THEN
    LOCAL_JDOUBLE_SAFETY_TAC "H_JDOUBLE_SAFE" 100 THEN
    ARM_STEPS_TAC SM2_MONTJSCALARMUL_ALT_EXEC (101--102) THEN
    ARM_STEPS_TAC SM2_MONTJSCALARMUL_ALT_EXEC (103--282) THEN
    ...
    ARM_STEPS_TAC SM2_MONTJSCALARMUL_ALT_EXEC (1--12) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    DISCHARGE_SAFETY_PROPERTY_TAC
  ] THEN
  ...
  ARM_STEPS_TAC SM2_MONTJSCALARMUL_ALT_EXEC (6--8) THEN
  ...
  ARM_STEPS_TAC SM2_MONTJSCALARMUL_ALT_EXEC (18--24) THEN
  ARM_STEPS_TAC SM2_MONTJSCALARMUL_ALT_EXEC (25--197) THEN
  ARM_STEPS_TAC SM2_MONTJSCALARMUL_ALT_EXEC (198--220) THEN
  ARM_STEPS_TAC SM2_MONTJSCALARMUL_ALT_EXEC (221--224) THEN
  LOCAL_JADD_SAFETY_TAC "H_JADD_SAFE" 225 THEN
  ARM_STEPS_TAC SM2_MONTJSCALARMUL_ALT_EXEC (226--226) THEN
...
let SM2_MONTJSCALARMUL_ALT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
             ...
             (\s. read PC s = returnaddress /\
                  (exists e2.
                         ...
                         e2 = f_events scalar point res pc
                                       (word_sub stackpointer (word 1248))
                                       returnaddress /\
                         memaccess_inbounds e2
                         [scalar,32; point,96; res,96;
                          word_sub stackpointer (word 1248),1248]
                         [res,96;
                          word_sub stackpointer (word 1248),1248]))
          (MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE[memory :> bytes(res,96);
                     memory :> bytes(word_sub stackpointer (word 1248),1248)])`,
  ARM_ADD_RETURN_STACK_TAC SM2_MONTJSCALARMUL_ALT_EXEC
    SM2_MONTJSCALARMUL_ALT_SAFE `[X19; X20; X21; X30]` 1248 THEN
  DISCHARGE_SAFETY_PROPERTY_TAC);;