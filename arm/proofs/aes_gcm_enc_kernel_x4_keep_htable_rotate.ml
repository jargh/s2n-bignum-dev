...
(* print_literal_from_elf "arm/aes_gcm/aes_gcm_enc_kernel_x4_keep_htable_rotate.o";; *)

let aes_gcm_enc_kernel_x4_keep_htable_rotate_mc =
...
(* ------------------------------------------------------------------------- *)
(* Software-pipelined ("rotate") variant: the main loop carries a NINE-round *)
(* partial AES state in Q0-Q3 across the back-edge (the preamble / previous   *)
(* iteration primes 9 rounds; the loop body applies only the final aese +     *)
(* XOR).  aes9 is that 9-pair partial tower; the two lemmas below complete it  *)
(* to the full cipher, so the standard AES_CTR_BLOCK reconstruction fires.     *)
(* ------------------------------------------------------------------------- *)

let aes9 = new_definition
 `aes9 (plaintext:int128) rk0 rk1 rk2 rk3 rk4 rk5 rk6 rk7 rk8 : int128 =
    aesmc
    (aese
     (aesmc
     (aese
      ...
      (aese
       ...
       (aese
        ...
        (aese
         (aesmc
         (aese (aesmc (aese (aesmc (aese (aesmc (aese plaintext rk0)) rk1)) rk2))
         rk3))
        rk4))
       rk5))
      rk6))
...
(* The carried loop-invariant value for a block with counter c: the 9-round     *)
(* partial AES of the byte-reversed counter block, keyed by byte-reversed keys.  *)

let aes9c = new_definition
 `aes9c nonce (rk:int128 list) c : int128 =
    aes9 (word_reversefields 8 (ctr_block nonce c))
...
(* When the code primes a counter block it reassembles four 32-bit lanes of the   *)
(* byte-reversed value; rev8 o rev8 = id and the (96,64,32,0) join rebuilds the    *)
(* original word.  This folds the stepped AES-input tower back to the counter word.*)

let AES_INPUT_FOLD = prove
 (`word_join
      ((word_join
        (word_reversefields 8 (word_reversefields 8 (word_subword (X:int128) (96,32):int32):int32):int32)
        (word_reversefields 8 (word_reversefields 8 (word_subword X (64,32):int32):int32):int32)):int64)
      ((word_join
        (word_reversefields 8 (word_reversefields 8 (word_subword X (32,32):int32):int32):int32)
        (word_reversefields 8 (word_reversefields 8 (word_subword X (0,32):int32):int32):int32)):int64):int128
   = X`,
  CONV_TAC WORD_BLAST);;

(* The rotate loop primes the next iteration's counter blocks by INCREMENTING the *)
(* already-byte-reversed lanes of an earlier counter, rather than materializing    *)
(* the counter block afresh.  Reconstructing those primed blocks leaves            *)
(* word_reversefields-of-word_subword terms that WORD_BLAST cannot discharge        *)
(* monolithically (it never terminates on a byte-reversal of a subword of a         *)
(* symbolic 128-bit word).  These two structural lemmas eliminate the reversefields *)
(* first so the residual splits into small per-lane bit-identities; see            *)
(* WORD_SUBWORD_REVERSEFIELDS above for the 8-bit-lane analogue.                     *)

(* rev8 o (32-bit subword) o rev8 collapses to a plain lane subword (lanes swap    *)
(* 0<->3, 1<->2 since the outer 32-bit reversal undoes the inner byte reversal).   *)

let WORD_REVERSEFIELDS_SUBWORD_CANCEL_32 = prove
 (`word_reversefields 8 (word_subword (word_reversefields 8 (x:int128)) (96,32):int32):int32 =
     word_subword x (0,32) /\
   word_reversefields 8 (word_subword (word_reversefields 8 (x:int128)) (64,32):int32):int32 =
     word_subword x (32,32) /\
   word_reversefields 8 (word_subword (word_reversefields 8 (x:int128)) (32,32):int32):int32 =
     word_subword x (64,32) /\
   word_reversefields 8 (word_subword (word_reversefields 8 (x:int128)) (0,32):int32):int32 =
     word_subword x (96,32)`,
  CONV_TAC WORD_BLAST);;

(* Applying the final round (aese rk9) and folding in the round-10 key + input *)
(* to a 9-round partial reconstructs exactly the standard full-tower form.      *)

let AES9_COMPLETE = prove
 (`word_xor (aese (aes9 plaintext rk0 rk1 rk2 rk3 rk4 rk5 rk6 rk7 rk8) rk9)
            (word_xor rk10 inblock) =
   word_xor
    (word_reversefields 8
      (aes128_cipher (word_reversefields 8 plaintext)
         (MAP (word_reversefields 8)
              [rk0; rk1; rk2; rk3; rk4; rk5; rk6; rk7; rk8; rk9; rk10])))
    inblock`,
  REWRITE_TAC[aes9] THEN REWRITE_TAC[XOR_AES128_CIPHER_RECONSTRUCT]);;

(* The full rotate-body reconstruction: the carried aes9 partial of the byte-  *)
(* reversed counter block, plus the body's final round and input XOR, equals   *)
(* the little-endian keystream block.  The counter is left as a BARE variable c  *)
(* (rather than i + 2) so that as a rewrite the LHS matches all four unrolled    *)
(* blocks 4*i+2, 4*i+3, 4*i+4, 4*i+5 - a "(i + 2)" pattern only first-order-      *)
(* matches counters syntactically of the form (_ + 2), so it would collapse only  *)
(* the first block and leave the other three stuck.  The RHS is expressed with    *)
(* aes128_cipher directly; the subsequent AES_CTR_BLOCK_RECONSTRUCT folds the     *)
(* four (ctr_block nonce (i + 2..5)) forms back to aes_ctr_block.                 *)

let ROTATE_BLOCK_RECONSTRUCT = prove
 (`LENGTH (rk:int128 list) = 11
   ==> word_xor
         (aese (aes9c nonce rk c) (word_reversefields 8 (EL 9 rk)))
         (word_xor (word_reversefields 8 (EL 10 rk)) inb) =
       word_xor (word_reversefields 8 (aes128_cipher (ctr_block nonce c) rk)) inb`,
  DISCH_TAC THEN
  REWRITE_TAC[aes9c; AES9_COMPLETE; WORD_REVERSEFIELDS_REVERSEFIELDS; MAP] THEN
  AP_THM_TAC THEN AP_TERM_TAC THEN AP_TERM_TAC THEN
  REWRITE_TAC[aes128_cipher; LET_DEF; LET_END_DEF] THEN
  CONV_TAC(DEPTH_CONV EL_CONV) THEN REFL_TAC);;

(* ------------------------------------------------------------------------- *)
(* Discharge tactics for the carried rotate state (Q0-Q3 = aes9c partials,   *)
(* Q31 = the running rev32 counter).  Both hold a counter block that the code *)
(* materialises by INCREMENTING the byte-reversed lanes of an earlier         *)
(* counter, so the reconstructed values carry                                 *)
(* word_reversefields-of-word_subword terms.  WORD_BLAST cannot discharge     *)
(* those against a symbolic counter (word (4*i+k)), so instead eliminate the  *)
(* reversefields structurally with the CANCEL / ctr-block lane lemmas, split  *)
(* per 32-bit lane (REPEAT BINOP_TAC), and finish the counter arithmetic with *)
(* WORD_RULE - all symbolic-safe and fast (~0.02s per conjunct).             *)

(* Q0-Q3: unfold both aes9 towers, peel the (opaque) aese/aesmc layers by     *)
(* congruence down to the AES input, then reconcile the primed input with the *)
(* byte-reversed counter block lane by lane.                                  *)

(* The lane subwords come in two shapes depending on where the block was primed *)
(* from: the setup primes Q0-Q3/Q31 from the raw ivec, giving the rev8-of-subword- *)
(* of-rev8 sandwich (WORD_REVERSEFIELDS_SUBWORD_CANCEL_32); the main loop and tail  *)
(* re-prime from the running rev32 counter Q31, giving a subword-of-rev32           *)
(* (WORD_SUBWORD_REVERSEFIELDS_32).  Both lemma sets are supplied so a single       *)
(* tactic covers all three discharge sites.                                        *)

let AES9C_PRIME_TAC =
  REWRITE_TAC[aes9c; aes9] THEN
  REPEAT(AP_TERM_TAC ORELSE (AP_THM_TAC THEN AP_TERM_TAC)) THEN
  REWRITE_TAC[WORD_REVERSEFIELDS_SUBWORD_CANCEL_32; WORD_SUBWORD_REVERSEFIELDS_32;
              WORD_SUBWORD_CTR_BLOCK_32] THEN
  GEN_REWRITE_TAC RAND_CONV [GSYM CTR_BLOCK_RECONSTRUCT_REV8] THEN
  REPEAT BINOP_TAC THEN TRY AP_TERM_TAC THEN CONV_TAC WORD_RULE;;

(* Q31: same idea for the rev32-encoded running counter.                      *)

let CTR31_PRIME_TAC =
  REWRITE_TAC[WORD_REVERSEFIELDS_SUBWORD_CANCEL_32; WORD_SUBWORD_REVERSEFIELDS_32;
              WORD_SUBWORD_CTR_BLOCK_32] THEN
  REWRITE_TAC[GSYM CTR_BLOCK_RECONSTRUCT_REV32] THEN
  REWRITE_TAC[WORD_ADD_0] THEN REPEAT BINOP_TAC THEN CONV_TAC WORD_RULE;;
...
(* save instructions) and ends at pc + 0x510 (first ldp of the postamble).   *)
...
let AES_GCM_ENC_KERNEL_X4_KEEP_HTABLE_ROTATE_CORRECT = prove
 (`!in_p out_p len_bits tag_p ivec_p key_p htable_p tag0 nonce rk inblock pc.
       ALLPAIRS nonoverlapping
        ...
        [(word pc, LENGTH aes_gcm_enc_kernel_x4_keep_htable_rotate_mc);
         (in_p,  16 * val len_bits DIV 128); (key_p, 176); (htable_p, 192)] /\
       PAIRWISE nonoverlapping
        [(out_p, 16 * val len_bits DIV 128); (tag_p, 16); (ivec_p, 16)]
    ==>
    ensures arm
      (\s. aligned_bytes_loaded s (word pc) aes_gcm_enc_kernel_x4_keep_htable_rotate_mc /\
  ...
  REWRITE_TAC[ALLPAIRS; PAIRWISE; ALL; fst AES_GCM_ENC_KERNEL_X4_KEEP_HTABLE_ROTATE_EXEC] THEN
  ...
  ENSURES_SEQUENCE_TAC `pc + 0x1e4`
   `\s. read X0 s = in_p /\
        ...
        read Q31 s = word_reversefields 32 (ctr_block nonce 6) /\
        read Q0 s = aes9c nonce rk 2 /\
        read Q1 s = aes9c nonce rk 3 /\
        read Q2 s = aes9c nonce rk 4 /\
        read Q3 s = aes9c nonce rk 5 /\
        read Q11 s =
          byteswap128 tag0 /\
  ...
  REWRITE_TAC[htable_mem_4; GSYM CONJ_ASSOC] THEN CONJ_TAC THENL
   [ENSURES_INIT_TAC "s0" THEN
    MAP_EVERY(fun n -> ARM_STEPS_TAC AES_GCM_ENC_KERNEL_X4_KEEP_HTABLE_ROTATE_EXEC [n] THEN
  ...
  ENSURES_SEQUENCE_TAC `pc + 0x434`
   `\s. read X0 s = word_add in_p (word (64 * loop_count)) /\
        ...
        read Q31 s = word_reversefields 32
                       (ctr_block nonce (4 * loop_count + 6)) /\
        read Q0 s = aes9c nonce rk (4 * loop_count + 2) /\
        read Q1 s = aes9c nonce rk (4 * loop_count + 3) /\
        read Q2 s = aes9c nonce rk (4 * loop_count + 4) /\
        read Q3 s = aes9c nonce rk (4 * loop_count + 5) /\
        read Q11 s =
          byteswap128
            (nist_ghash (aes128_cipher (word 0) rk) tag0
               (list_of_seq (nist_cipher_block nonce rk inblock)
  ...
  REWRITE_TAC[htable_mem_4; GSYM CONJ_ASSOC] THEN CONJ_TAC THENL
   [ASM_CASES_TAC `loop_count = 0` THENL
     [POP_ASSUM SUBST_ALL_TAC THEN
      ARM_SIM_TAC AES_GCM_ENC_KERNEL_X4_KEEP_HTABLE_ROTATE_EXEC [1] THEN
    ...
    ENSURES_WHILE_UP_TAC `loop_count:num` `pc + 0x1e8` `pc + 0x430`
      `\i s.
        ...
        read X7 s = word nblocks /\
        read X9 s = word loop_remain /\
        read Q31 s = word_reversefields 32 (ctr_block nonce (4 * i + 6)) /\
        read Q0 s = aes9c nonce rk (4 * i + 2) /\
        read Q1 s = aes9c nonce rk (4 * i + 3) /\
        read Q2 s = aes9c nonce rk (4 * i + 4) /\
        read Q3 s = aes9c nonce rk (4 * i + 5) /\
        read Q11 s =
          byteswap128
            (nist_ghash (aes128_cipher (word 0) rk) tag0
    ...
    ASM_REWRITE_TAC[htable_mem_4; GSYM CONJ_ASSOC] THEN REPEAT CONJ_TAC THENL
     [ARM_SIM_TAC AES_GCM_ENC_KERNEL_X4_KEEP_HTABLE_ROTATE_EXEC [1] THEN
      ...
      (*** Rotated output stores: each stored block is aese(carried aes9c partial)+XOR;   ***)
      (*** ROTATE_BLOCK_RECONSTRUCT collapses it to word_xor(aes_ctr_block ..)(inblock).  ***)
      SUBGOAL_THEN `LENGTH(rk:int128 list) = 11` ASSUME_TAC THENL
       [FIRST_X_ASSUM(fun th ->
          if rand(concl th) = `rk:int128 list` then SUBST1_TAC(SYM th) else failwith "") THEN
        REWRITE_TAC[LENGTH] THEN CONV_TAC NUM_REDUCE_CONV;
        ALL_TAC] THEN
      FIRST_ASSUM(fun th -> REWRITE_TAC[MATCH_MP ROTATE_BLOCK_RECONSTRUCT th]) THEN
      REWRITE_TAC[aes_ctr_block; GSYM ADD_ASSOC] THEN
      ...
      REWRITE_TAC[WORD_SUBWORD_XOR] THEN
      CONV_TAC(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV) THEN
      (*** Re-establish the next iteration's carried state: the four Q0-Q3 hold the     ***)
      (*** freshly-primed 9-round partials aes9c(4(i+1)+2..+5) and Q31 the running       ***)
      (*** counter.  The rotate loop primes each next block by incrementing the byte-    ***)
      (*** reversed lanes of an earlier counter, so the reconstructed partials/counter   ***)
      (*** carry word_reversefields-of-word_subword terms that WORD_BLAST cannot solve   ***)
      (*** against the symbolic counter word (4*i+k).  Discharge them with the           ***)
      (*** structural, symbolic-safe priming tactics (see AES9C_PRIME_TAC /              ***)
      (*** CTR31_PRIME_TAC above): the loop-counter/address conjuncts fall to WORD_RULE, ***)
      (*** the four Q0-Q3 aes9c partials to AES9C_PRIME_TAC and the Q31 counter to        ***)
      (*** CTR31_PRIME_TAC, leaving only the GHASH tag for the bit-blasted               ***)
      (*** reconstruction below.                                                         ***)
      REPEAT(CONJ_TAC THENL
       [CONV_TAC WORD_RULE ORELSE AES9C_PRIME_TAC ORELSE CTR31_PRIME_TAC;
        ALL_TAC]) THEN
  ...
  ASM_CASES_TAC `loop_remain = 0` THENL
   [POP_ASSUM SUBST_ALL_TAC THEN
    ENSURES_INIT_TAC "s0" THEN
    (*** The pipelined kernel takes 10 steps (not 6) from pc + 0x434 to the         ***)
    (*** postamble stores: three "sub v31,v31,v30" drains, the "cbz x9" taken to    ***)
    (*** Lloop_1x_end, a FOURTH "sub v31,v31,v30" (peculiar to the rotated kernel,   ***)
    (*** draining the last in-flight counter), "mov x0,x15", and then the tag/ivec  ***)
    (*** byte-reversals and stores.  Stopping at 6 leaves the ivec/tag unstored.    ***)
    MAP_EVERY(fun n -> ARM_STEPS_TAC AES_GCM_ENC_KERNEL_X4_KEEP_HTABLE_ROTATE_EXEC [n] THEN
          RULE_ASSUM_TAC(CONV_RULE(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV)))
        (1--10) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    ...
    CONV_TAC(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV) THEN
    (*** The remaining tag and ivec conjuncts are pure byte-reversal identities, but a  ***)
    (*** monolithic BITBLAST runs away on them: the tag wraps the opaque 128-bit        ***)
    (*** nist_ghash value and the ivec wraps a byte-reversed symbolic counter.  Discharge ***)
    (*** each structurally: abstract the nist_ghash value to a plain word before         ***)
    (*** blasting the tag byteswap, and for the ivec collapse the four counter drains     ***)
    (*** (WORD_RULE) and rebuild the byte-reversed counter block lane by lane.            ***)
    CONJ_TAC THENL
     [ABBREV_TAC
       `tagval:int128 =
          nist_ghash (aes128_cipher (word 0) rk) tag0
      ...
      GEN_REWRITE_TAC (LAND_CONV o TOP_DEPTH_CONV)
       [WORD_RULE
         `word_sub (word_sub (word_sub (word_sub (word (n + 6):int32) (word 1))
                    (word 1)) (word 1)) (word 1) = word (n + 2)`;
        WORD_RULE
         `word_sub (word_sub (word_sub (word_sub (x:int32) (word 0)) (word 0))
                    (word 0)) (word 0) = x`] THEN
      REWRITE_TAC[CTR_BLOCK_RECONSTRUCT_REV8]];

    ALL_TAC] THEN

  (*** Loop setup for the tail loop ***)

  ENSURES_WHILE_UP_TAC `loop_remain:num` `pc + 0x444` `pc + 0x4f4`
    `\i s.
      ...
      read Q31 s = word_reversefields 32
                    (ctr_block nonce (4 * loop_count + i + 3)) /\
      read Q0 s = aes9c nonce rk (4 * loop_count + i + 2) /\
      read Q11 s =
        byteswap128
            (nist_ghash (aes128_cipher (word 0) rk) tag0
               (list_of_seq (nist_cipher_block nonce rk inblock)
      ...
      (!j. j < 4 * loop_count + i
           ==> read (memory :> bytes128 (word_add out_p (word(16*j)))) s =
               word_xor (aes_ctr_block nonce rk j) (inblock j))` THEN
  ASM_REWRITE_TAC[htable_mem_4; GSYM CONJ_ASSOC] THEN REPEAT CONJ_TAC THENL
   [(*** Entry to the tail loop: from pc + 0x434 the three "sub v31,v31,v30" pipeline ***)
    (*** drains run, then "cbz x9" falls through (loop_remain =/= 0) to the loop head  ***)
    (*** pc + 0x444 - four steps, not one.                                            ***)
    ENSURES_INIT_TAC "s0" THEN
    MAP_EVERY(fun n -> ARM_STEPS_TAC AES_GCM_ENC_KERNEL_X4_KEEP_HTABLE_ROTATE_EXEC [n] THEN
          RULE_ASSUM_TAC(CONV_RULE(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV)))
        (1--4) THEN
    ENSURES_FINAL_STATE_TAC THEN
    ASM_REWRITE_TAC[ADD_CLAUSES; MULT_CLAUSES; SUB_0] THEN
    (*** The three pipeline drains take Q31 from the four-ahead                        ***)
    (*** rev32(ctr_block (4*loop_count+6)) to the tail head's one-ahead                ***)
    (*** rev32(ctr_block (4*loop_count+3)); reconstruct that counter structurally.     ***)
    REWRITE_TAC[WORD_SUBWORD_REVERSEFIELDS_32; WORD_SUBWORD_CTR_BLOCK_32] THEN
    GEN_REWRITE_TAC (LAND_CONV o TOP_DEPTH_CONV)
     [WORD_RULE
       `word_sub (word_sub (word_sub (word (n + 6):int32) (word 1)) (word 1))
                 (word 1) = word (n + 3)`;
      WORD_RULE
       `word_sub (word_sub (word_sub (x:int32) (word 0)) (word 0)) (word 0) = x`] THEN
    REWRITE_TAC[CTR_BLOCK_RECONSTRUCT_REV32];
    ...
    (*** Like the main loop, the tail block is stored as aese(aes9c ..) (the carried  ***)
    (*** nine-round partial finished with the tenth round), so it collapses with       ***)
    (*** ROTATE_BLOCK_RECONSTRUCT, not the raw-tower XOR_AES128_CIPHER_RECONSTRUCT.     ***)
    SUBGOAL_THEN `LENGTH(rk:int128 list) = 11` ASSUME_TAC THENL
     [FIRST_X_ASSUM(fun th ->
        if rand(concl th) = `rk:int128 list` then SUBST1_TAC(SYM th) else failwith "") THEN
      REWRITE_TAC[LENGTH] THEN CONV_TAC NUM_REDUCE_CONV;
    ...
    ARM_SIM_TAC AES_GCM_ENC_KERNEL_X4_KEEP_HTABLE_ROTATE_EXEC (1--7) THEN
    REWRITE_TAC[ADD_ASSOC] THEN
    SUBGOAL_THEN `4 * loop_count + loop_remain = nblocks` SUBST_ALL_TAC THENL
     [SIMPLE_ARITH_TAC; ASM_REWRITE_TAC[]] THEN
    REWRITE_TAC[byteswap128] THEN
    CONV_TAC(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV) THEN
    (*** From the loop back-edge pc + 0x4f4 the exit falls through the postamble         ***)
    (*** (fourth drain sub, mov x0,x15, tag rev64+store, ivec rev32+store) in seven      ***)
    (*** steps to reach pc + 0x510; stopping at six leaves the ivec store unexecuted.    ***)
    (*** Same byte-reversal blowup as the loop_remain = 0 case: discharge the tag and    ***)
    (*** ivec conjuncts structurally rather than with a monolithic BITBLAST.  The ivec    ***)
    (*** here is drained one step further than the four-block main loop (single "sub      ***)
    (*** v31,v31,v30" in the postamble), so a couple of counter-arithmetic collapses are  ***)
    (*** supplied covering both drain depths.                                             ***)
    CONJ_TAC THENL
     [ABBREV_TAC
       `tagval:int128 =
          nist_ghash (aes128_cipher (word 0) rk) tag0
      ...
      REWRITE_TAC[WORD_SUBWORD_REVERSEFIELDS_32; WORD_SUBWORD_CTR_BLOCK_32] THEN
      GEN_REWRITE_TAC (LAND_CONV o TOP_DEPTH_CONV)
       [WORD_RULE `word_sub (word (n + 3):int32) (word 1) = word (n + 2)`;
        WORD_RULE
         `word_sub (word_sub (word_sub (word_sub (word (n + 6):int32) (word 1))
                    (word 1)) (word 1)) (word 1) = word (n + 2)`;
        WORD_RULE `word_sub (x:int32) (word 0) = x`] THEN
      REWRITE_TAC[CTR_BLOCK_RECONSTRUCT_REV8]]]);;
...
let AES_GCM_ENC_KERNEL_X4_KEEP_HTABLE_ROTATE_SUBROUTINE_CORRECT = prove
 (`!in_p out_p len_bits tag_p ivec_p key_p htable_p tag0 nonce rk inblock
  ...
  REWRITE_TAC[fst AES_GCM_ENC_KERNEL_X4_KEEP_HTABLE_ROTATE_EXEC; htable_mem_4] THEN
  CONV_TAC(ONCE_DEPTH_CONV WORDLIST_FROM_MEMORY_CONV) THEN
  ARM_ADD_RETURN_STACK_TAC
    ~pre_post_nsteps:(11, 11)
    AES_GCM_ENC_KERNEL_X4_KEEP_HTABLE_ROTATE_EXEC
    (CONV_RULE(ONCE_DEPTH_CONV WORDLIST_FROM_MEMORY_CONV)
       (REWRITE_RULE[fst AES_GCM_ENC_KERNEL_X4_KEEP_HTABLE_ROTATE_EXEC; htable_mem_4]
          AES_GCM_ENC_KERNEL_X4_KEEP_HTABLE_ROTATE_CORRECT))
