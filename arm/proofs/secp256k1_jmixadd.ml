...
let SECP256K1_JMIXADD_SAFE = time prove
 (`exists f_events.
       forall e p3 p1 p2 pc stackpointer.
           aligned 16 stackpointer /\
           ALL (nonoverlapping (stackpointer,192))
           [word pc,LENGTH secp256k1_jmixadd_mc; p1,96; p2,64; p3,96] /\
           nonoverlapping (p3,96) (word pc,LENGTH secp256k1_jmixadd_mc)
           ==> ensures arm
               (\s.
                    aligned_bytes_loaded s (word pc) secp256k1_jmixadd_mc /\
                    read PC s = word (pc + 0xc) /\
                    read SP s = stackpointer /\
                    C_ARGUMENTS [p3; p1; p2] s /\
                    read events s = e)
               (\s.
                    read PC s = word (pc + 0x1c50) /\
                    (exists e2.
       ...
       forall e p3 p1 p2 pc stackpointer returnaddress.
        ...
        ==> ensures arm
          ...
          (MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(p3,96);
                      memory :> bytes(word_sub stackpointer (word 224),224)])`,
  REWRITE_TAC[fst SECP256K1_JMIXADD_EXEC] THEN
  ARM_ADD_RETURN_STACK_TAC SECP256K1_JMIXADD_EXEC
    (REWRITE_RULE[fst SECP256K1_JMIXADD_EXEC]SECP256K1_JMIXADD_SAFE)
    `[X19; X20; X21; X22]` 224 THEN
