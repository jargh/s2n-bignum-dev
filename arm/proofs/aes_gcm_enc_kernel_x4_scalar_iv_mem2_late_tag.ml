...
(* print_literal_from_elf "arm/aes_gcm/aes_gcm_enc_kernel_x4_scalar_iv_mem2_late_tag.o";; *)

let aes_gcm_enc_kernel_x4_scalar_iv_mem2_late_tag_mc =
  define_from_elf "aes_gcm_enc_kernel_x4_scalar_iv_mem2_late_tag_mc"
    "arm/aes_gcm/aes_gcm_enc_kernel_x4_scalar_iv_mem2_late_tag.o";;

let AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_EXEC = ARM_MK_EXEC_RULE aes_gcm_enc_kernel_x4_scalar_iv_mem2_late_tag_mc;;
...
let AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_CORRECT = prove
 (`!in_p out_p len_bits tag_p ivec_p key_p htable_p tag0 nonce rk inblock pc
     stackpointer.
       ...
       ALLPAIRS nonoverlapping
        ...
        [(word pc, LENGTH aes_gcm_enc_kernel_x4_scalar_iv_mem2_late_tag_mc);
    ...
    ==>
    ensures arm
      (\s. aligned_bytes_loaded s (word pc) aes_gcm_enc_kernel_x4_scalar_iv_mem2_late_tag_mc /\
  ...
  REWRITE_TAC[ALLPAIRS; PAIRWISE; ALL; fst AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_EXEC] THEN
  ...
  REWRITE_TAC[htable_mem_4; GSYM CONJ_ASSOC] THEN CONJ_TAC THENL
   [ENSURES_INIT_TAC "s0" THEN
    ...
    ARM_STEPS_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_EXEC (1--27) THEN
  ...
  REWRITE_TAC[htable_mem_4; GSYM CONJ_ASSOC] THEN CONJ_TAC THENL
   [ASM_CASES_TAC `loop_count = 0` THENL
     [POP_ASSUM SUBST_ALL_TAC THEN
      ARM_SIM_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_EXEC [1] THEN
  ...
  ENSURES_WHILE_UP_TAC `loop_remain:num` `pc + 0x334` `pc + 0x3f0`
    `\i s.
   ...
   [ENSURES_INIT_TAC "s0" THEN
    ...
    ASSUME_TAC THENL
     [REWRITE_TAC[ARITH_RULE `64 * a + 16 * b = 16 * (4 * a + b)`] THEN
      ...
      ALL_TAC] THEN
    (*** The single tail block assembles its counter word on the stack: "add     ***)
    (*** w14,w13,#0 ; rev w14,w14 ; str w14,[sp,#172]" (step 4) then "ldr         ***)
...
let AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_SUBROUTINE_CORRECT = prove
 (`!in_p out_p len_bits tag_p ivec_p key_p htable_p tag0 nonce rk inblock
  ...
  REWRITE_TAC[fst AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_EXEC; htable_mem_4] THEN
  CONV_TAC(ONCE_DEPTH_CONV WORDLIST_FROM_MEMORY_CONV) THEN
  ARM_ADD_RETURN_STACK_TAC
    ~pre_post_nsteps:(11, 11)
    AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_EXEC
    (CONV_RULE(ONCE_DEPTH_CONV WORDLIST_FROM_MEMORY_CONV)
       (REWRITE_RULE[fst AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_EXEC; htable_mem_4]
          AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_CORRECT))
