...
(**** print_literal_from_elf "arm/sm2/sm2_montjmixadd_alt.o";;
 ****)

let sm2_montjmixadd_alt_mc = define_assert_from_elf
  "sm2_montjmixadd_alt_mc" "arm/sm2/sm2_montjmixadd_alt.o"
[
...
let SM2_MONTJMIXADD_ALT_EXEC = ARM_MK_EXEC_RULE sm2_montjmixadd_alt_mc;;
...
let LOCAL_MONTSQR_SM2_TAC =
  ARM_MACRO_SIM_ABBREV_TAC sm2_montjmixadd_alt_mc 95 lvs
  `!(t:armstate) pcin pcout p3 n3 p1 n1.
    !a. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 4)) t = a
    ==>
    aligned 16 (read SP t) /\
    nonoverlapping (word pc,0x1574) (word_add (read p3 t) (word n3),32)
    ==> ensures arm
         (\s. aligned_bytes_loaded s (word pc) sm2_montjmixadd_alt_mc /\
              read PC s = pcin /\
              read SP s = read SP t /\
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ASM_CASES_TAC `a EXP 2 <= 2 EXP 256 * p_sm2` THENL
   [ASM_REWRITE_TAC[]; ARM_SIM_TAC SM2_MONTJMIXADD_ALT_EXEC (1--95)] THEN
  ENSURES_INIT_TAC "s0" THEN
  FIRST_ASSUM(BIGNUM_DIGITIZE_TAC "x_" o lhand o concl) THEN

  (*** Simulate the core pre-reduced result accumulation ***)

  ARM_ACCSTEPS_TAC SM2_MONTJMIXADD_ALT_EXEC (1--82) (1--82) THEN
  ...
  ARM_ACCSTEPS_TAC SM2_MONTJMIXADD_ALT_EXEC (85--89) (83--95) THEN
...
let LOCAL_MONTMUL_SM2_TAC =
  ARM_MACRO_SIM_ABBREV_TAC sm2_montjmixadd_alt_mc 117 lvs
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ASM_CASES_TAC `a * b <= 2 EXP 256 * p_sm2` THENL
   [ASM_REWRITE_TAC[]; ARM_SIM_TAC SM2_MONTJMIXADD_ALT_EXEC (1--117)] THEN
  ENSURES_INIT_TAC "s0" THEN
  ...
  ARM_ACCSTEPS_TAC SM2_MONTJMIXADD_ALT_EXEC (1--104) (1--104) THEN
  ...
  ARM_ACCSTEPS_TAC SM2_MONTJMIXADD_ALT_EXEC (107--111) (105--117) THEN
  RULE_ASSUM_TAC(REWRITE_RULE
   [GSYM WORD_BITVAL; COND_SWAP; REAL_BITVAL_NOT]) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  CONV_TAC(LAND_CONV BIGNUM_EXPAND_CONV) THEN ASM_REWRITE_TAC[] THEN
...
let LOCAL_SUB_SM2_TAC =
  ARM_MACRO_SIM_ABBREV_TAC sm2_montjmixadd_alt_mc 17 lvs
  `!(t:armstate) pcin pcout p3 n3 p1 n1 p2 n2.
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ARM_ACCSTEPS_TAC SM2_MONTJMIXADD_ALT_EXEC (1--8) (1--8) THEN
  ...
  ARM_STEPS_TAC SM2_MONTJMIXADD_ALT_EXEC [9] THEN
  RULE_ASSUM_TAC(REWRITE_RULE[WORD_UNMASK_64; NOT_LE]) THEN
  ARM_ACCSTEPS_TAC SM2_MONTJMIXADD_ALT_EXEC (10--17) (10--17) THEN
...
let LOCAL_AMONTSQR_SM2_TAC =
  ARM_MACRO_SIM_ABBREV_TAC sm2_montjmixadd_alt_mc 90 lvs
  `!(t:armstate) pcin pcout p3 n3 p1 n1.
    !a. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 4)) t = a
    ==>
    aligned 16 (read SP t) /\
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ARM_ACCSTEPS_TAC SM2_MONTJMIXADD_ALT_EXEC (85--89) (83--90) THEN
  RULE_ASSUM_TAC(REWRITE_RULE
   [GSYM WORD_BITVAL; COND_SWAP; REAL_BITVAL_NOT]) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  CONJ_TAC THENL
...
let SM2_MONTJMIXADD_ALT_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer.
        ...
        ALL (nonoverlapping (stackpointer,192))
            [(word pc,0x1574); (p1,96); (p2,64); (p3,96)] /\
        nonoverlapping (p3,96) (word pc,0x1574)
        ==> ensures arm
             (\s. aligned_bytes_loaded s (word pc) sm2_montjmixadd_alt_mc /\
  ...
  LOCAL_AMONTSQR_SM2_TAC 3 ["zp2";"z_1"] THEN
  ...
  ARM_STEPS_TAC SM2_MONTJMIXADD_ALT_EXEC (22--58) THEN
...
let SM2_MONTJMIXADD_ALT_SUBROUTINE_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer returnaddress.
  ...
  ARM_ADD_RETURN_STACK_TAC SM2_MONTJMIXADD_ALT_EXEC
   SM2_MONTJMIXADD_ALT_CORRECT `[]` 192);;
...
let full_spec,public_vars = mk_safety_spec
    ...
    (assoc "sm2_montjmixadd_alt" subroutine_signatures)
    SM2_MONTJMIXADD_ALT_CORRECT
    SM2_MONTJMIXADD_ALT_EXEC;;

let SM2_MONTJMIXADD_ALT_SAFE = time prove(full_spec,
  REWRITE_TAC[MODIFIABLE_SIMD_REGS;SOME_FLAGS] THEN
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars SM2_MONTJMIXADD_ALT_EXEC);;

let SM2_MONTJMIXADD_ALT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e p3 p1 p2 pc stackpointer returnaddress.
        aligned 16 stackpointer /\
        ALL (nonoverlapping (word_sub stackpointer (word 192),192))
            [(word pc,LENGTH sm2_montjmixadd_alt_mc); (p1,96); (p2,64); (p3,96)] /\
        nonoverlapping (p3,96) (word pc,LENGTH sm2_montjmixadd_alt_mc)
        ==> ensures arm
             (\s. aligned_bytes_loaded s (word pc) sm2_montjmixadd_alt_mc /\
                  read PC s = word pc /\
                  read SP s = stackpointer /\
          ...
          (MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(p3,96);
                      memory :> bytes(word_sub stackpointer (word 192),192)])`,
  REWRITE_TAC[fst SM2_MONTJMIXADD_ALT_EXEC] THEN
  ARM_ADD_RETURN_STACK_TAC SM2_MONTJMIXADD_ALT_EXEC
    (REWRITE_RULE[fst SM2_MONTJMIXADD_ALT_EXEC]SM2_MONTJMIXADD_ALT_SAFE)
    `[]` 192 THEN
