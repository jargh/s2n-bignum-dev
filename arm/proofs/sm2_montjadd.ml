...
(**** print_literal_from_elf "arm/sm2/sm2_montjadd.o";;
 ****)

let sm2_montjadd_mc = define_assert_from_elf
  "sm2_montjadd_mc" "arm/sm2/sm2_montjadd.o"
[
...
let SM2_MONTJADD_EXEC = ARM_MK_EXEC_RULE sm2_montjadd_mc;;
...
let LOCAL_MONTSQR_SM2_TAC =
  ARM_MACRO_SIM_ABBREV_TAC sm2_montjadd_mc 126 lvs
  `!(t:armstate) pcin pcout p3 n3 p1 n1.
    !a. read(memory :> bytes(word_add (read p1 t) (word n1),8 * 4)) t = a
    ==>
    aligned 16 (read SP t) /\
    nonoverlapping (word pc,0x2aa8) (word_add (read p3 t) (word n3),32)
    ==> ensures arm
         (\s. aligned_bytes_loaded s (word pc) sm2_montjadd_mc /\
              read PC s = pcin /\
              read SP s = read SP t /\
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ARM_ACCSTEPS_TAC SM2_MONTJADD_EXEC (66--113) (66--113) THEN
  ABBREV_TAC
...
let LOCAL_MONTMUL_SM2_TAC =
  ARM_MACRO_SIM_ABBREV_TAC sm2_montjadd_mc 170 lvs
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ASM_CASES_TAC `a * b <= 2 EXP 256 * p_sm2` THENL
   [ASM_REWRITE_TAC[]; ARM_SIM_TAC SM2_MONTJADD_EXEC (1--170)] THEN
  ENSURES_INIT_TAC "s0" THEN
  ...
  ARM_ACCSTEPS_TAC SM2_MONTJADD_EXEC (29--46) (29--46) THEN
  SUBGOAL_THEN
  ...
  ARM_ACCSTEPS_TAC SM2_MONTJADD_EXEC [71;72;76;78] (71--78) THEN
  ...
  ARM_ACCSTEPS_TAC SM2_MONTJADD_EXEC [79;80;84;86] (79--86) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[ADD_CLAUSES; WORD_RULE
   `word_sub (word 0) x = word_neg x`]) THEN
  SUBGOAL_THEN
  ...
  ARM_STEPS_TAC SM2_MONTJADD_EXEC [87] THEN
  ...
  ARM_ACCSTEPS_TAC SM2_MONTJADD_EXEC (88--94) (88--94) THEN
  RULE_ASSUM_TAC(REWRITE_RULE[ADD_CLAUSES]) THEN
  SUBGOAL_THEN
   `bignum_of_wordlist
  ...
  ARM_ACCSTEPS_TAC SM2_MONTJADD_EXEC (136--154) (136--154) THEN
  ABBREV_TAC
   `t = bignum_of_wordlist
  ...
  ARM_ACCSTEPS_TAC SM2_MONTJADD_EXEC (158--161) (155--162) THEN
  MP_TAC(SPECL
  ...
  ARM_ACCSTEPS_TAC SM2_MONTJADD_EXEC [163;165;166;168] (163--170) THEN
  ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
  CONV_TAC(LAND_CONV BIGNUM_EXPAND_CONV) THEN ASM_REWRITE_TAC[] THEN
...
let LOCAL_SUB_SM2_TAC =
  ARM_MACRO_SIM_ABBREV_TAC sm2_montjadd_mc 17 lvs
  `!(t:armstate) pcin pcout p3 n3 p1 n1 p2 n2.
 ...
 (REWRITE_TAC[C_ARGUMENTS; C_RETURN; SOME_FLAGS; NONOVERLAPPING_CLAUSES] THEN
  ...
  ARM_ACCSTEPS_TAC SM2_MONTJADD_EXEC (1--8) (1--8) THEN
  ...
  ARM_STEPS_TAC SM2_MONTJADD_EXEC [9] THEN
  RULE_ASSUM_TAC(REWRITE_RULE[WORD_UNMASK_64; NOT_LE]) THEN
  ARM_ACCSTEPS_TAC SM2_MONTJADD_EXEC (10--17) (10--17) THEN
...
let SM2_MONTJADD_CORRECT = time prove
 (`!p3 p1 t1 p2 t2 pc stackpointer.
        ...
        ALL (nonoverlapping (stackpointer,224))
            [(word pc,0x2aa8); (p1,96); (p2,96); (p3,96)] /\
        nonoverlapping (p3,96) (word pc,0x2aa8)
        ==> ensures arm
             (\s. aligned_bytes_loaded s (word pc) sm2_montjadd_mc /\
  ...
  LOCAL_MONTSQR_SM2_TAC 3 ["z1sq";"z_1"] THEN
  ...
  ARM_STEPS_TAC SM2_MONTJADD_EXEC (27--85) THEN
...
let SM2_MONTJADD_SAFE = time prove
 (`exists f_events.
       forall e p3 p1 p2 pc stackpointer.
           aligned 16 stackpointer /\
           ALL (nonoverlapping (stackpointer,224))
           [word pc,LENGTH sm2_montjadd_mc; p1,96; p2,96; p3,96] /\
           nonoverlapping (p3,96) (word pc,LENGTH sm2_montjadd_mc)
           ==> ensures arm
               (\s.
                    aligned_bytes_loaded s (word pc) sm2_montjadd_mc /\
                    read PC s = word (pc + 0x8) /\
                    read SP s = stackpointer /\
                    C_ARGUMENTS [p3; p1; p2] s /\
                    read events s = e)
               (\s.
                    read PC s = word (pc + 0x2a9c) /\
                    (exists e2.
  ...
  PROVE_SAFETY_SPEC_TAC ~public_vars:public_vars SM2_MONTJADD_EXEC);;
             ...
             (\s. read PC s = returnaddress /\
                  (exists e2.
                         ...
                         e2 = f_events p1 p2 p3 pc
                              (word_sub stackpointer (word 240))
                              returnaddress /\
                         memaccess_inbounds e2
                         [p1,96; p2,96; p3,96;
                          word_sub stackpointer (word 240),240]
                         [p3,96;
                          word_sub stackpointer (word 240),240]))
          (MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(p3,96);
                      memory :> bytes(word_sub stackpointer (word 240),240)])`,
  REWRITE_TAC[fst SM2_MONTJADD_EXEC] THEN
  ARM_ADD_RETURN_STACK_TAC SM2_MONTJADD_EXEC
    (REWRITE_RULE[fst SM2_MONTJADD_EXEC]SM2_MONTJADD_SAFE)
    `[X19; X20]` 240 THEN
