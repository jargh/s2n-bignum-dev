  ...
  REMOVE_THEN assump_name (fun safety_th ->
    ...
    ARM_SUBROUTINE_SIM_TAC ~is_safety_thm:true
        (sm2_montjscalarmul_mc,SM2_MONTJSCALARMUL_EXEC,
          0x32b4,sm2_montjdouble_mc,th)
        [`e:(uarch_event)list`; `read X0 s`; `read X1 s`;
        `pc + 0x32b4`; `read SP s`; `read X30 s`] n THENL [
      EXISTS_E2_TAC(ref
        [`scalar:int64`;`point:int64`;`res:int64`;`pc:num`;
         `stackpointer:int64`;
         (* inside the loop... *)
...
let full_spec,public_vars = mk_safety_spec
    ...
    SM2_MONTJSCALARMUL_CORRECT
    SM2_MONTJSCALARMUL_EXEC;;

let SM2_MONTJSCALARMUL_SAFE = time prove
 (`exists f_events.
       forall e res scalar point pc stackpointer.
           ...
           ==> ensures arm
               (\s.
                    aligned_bytes_loaded s (word pc) sm2_montjscalarmul_mc /\
               ...
               (\s.
                    ...
                    (exists e2.
                         ...
                         [scalar,32; point,96; res,96; stackpointer,1232]
                         [res,96; stackpointer,1232]))
               (MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
                MAYCHANGE [X19; X20; X21; X30] ,,
                MAYCHANGE
                [memory :> bytes (res,96);
  ...
  ASSUME_CALLEE_SAFETY_TAILED_TAC SM2_MONTJADD_SUBROUTINE_SAFE
      "H_JADD_SAFE" THEN
  ASSUME_CALLEE_SAFETY_TAILED_TAC SM2_MONTJDOUBLE_SUBROUTINE_SAFE
      "H_JDOUBLE_SAFE" THEN
  ...
  REPEAT CONJ_TAC THENL [
    ...
    ARM_STEPS_TAC SM2_MONTJSCALARMUL_EXEC (1--72) THEN
    ...
    ARM_STEPS_TAC SM2_MONTJSCALARMUL_EXEC (1--12) THEN
    ENSURES_FINAL_STATE_TAC THEN ASM_REWRITE_TAC[] THEN
    DISCHARGE_SAFETY_PROPERTY_TAC
  ] THEN
  ...
  ARM_STEPS_TAC SM2_MONTJSCALARMUL_EXEC (6--8) THEN
  ...
  ARM_STEPS_TAC SM2_MONTJSCALARMUL_EXEC (18--24) THEN
  ARM_STEPS_TAC SM2_MONTJSCALARMUL_EXEC (25--197) THEN
  ARM_STEPS_TAC SM2_MONTJSCALARMUL_EXEC (198--220) THEN
  ARM_STEPS_TAC SM2_MONTJSCALARMUL_EXEC (221--224) THEN
  LOCAL_JADD_SAFETY_TAC "H_JADD_SAFE" 225 THEN
  ARM_STEPS_TAC SM2_MONTJSCALARMUL_EXEC (226--226) THEN
             ...
             (\s. read PC s = returnaddress /\
                  (exists e2.
                         ...
                         e2 = f_events scalar point res pc
                                       (word_sub stackpointer (word 1264))
                                       returnaddress /\
                         memaccess_inbounds e2
                         [scalar,32; point,96; res,96;
                          word_sub stackpointer (word 1264),1264]
                         [res,96;
                          word_sub stackpointer (word 1264),1264]))
          (MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE[memory :> bytes(res,96);
                     memory :> bytes(word_sub stackpointer (word 1264),1264)])`,
  ARM_ADD_RETURN_STACK_TAC SM2_MONTJSCALARMUL_EXEC
    SM2_MONTJSCALARMUL_SAFE `[X19; X20; X21; X30]` 1264 THEN
  DISCHARGE_SAFETY_PROPERTY_TAC);;