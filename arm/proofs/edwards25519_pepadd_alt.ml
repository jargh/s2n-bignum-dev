...
let EDWARDS25519_PEPADD_ALT_SUBROUTINE_SAFE = time prove
 (`exists f_events.
       forall e p3 p1 p2 pc stackpointer returnaddress.
        aligned 16 stackpointer /\
        ALL (nonoverlapping (word_sub stackpointer (word 208),208))
            [(word pc,LENGTH edwards25519_pepadd_alt_mc); (p1,128); (p2,96); (p3,128)] /\
        ...
        ==> ensures arm
             ...
             (\s. read PC s = returnaddress /\
                  (exists e2.
                         ...
                         [p1,128; p2,96; p3,128; word_sub stackpointer (word 208),208]
                         [p3,128; word_sub stackpointer (word 208),208]))
          (MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(p3,128);
                      memory :> bytes(word_sub stackpointer (word 208),208)])`,
  REWRITE_TAC[fst EDWARDS25519_PEPADD_ALT_EXEC] THEN
  ARM_ADD_RETURN_STACK_TAC EDWARDS25519_PEPADD_ALT_EXEC
    (REWRITE_RULE[fst EDWARDS25519_PEPADD_ALT_EXEC]EDWARDS25519_PEPADD_ALT_SAFE)
    `[X19; X20]` 208 THEN
