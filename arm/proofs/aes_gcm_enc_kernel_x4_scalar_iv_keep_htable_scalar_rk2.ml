...
(* print_literal_from_elf "arm/aes_gcm/aes_gcm_enc_kernel_x4_scalar_iv_keep_htable_scalar_rk2.o";; *)

let aes_gcm_enc_kernel_x4_scalar_iv_keep_htable_scalar_rk2_mc =
  define_from_elf "aes_gcm_enc_kernel_x4_scalar_iv_keep_htable_scalar_rk2_mc"
    "arm/aes_gcm/aes_gcm_enc_kernel_x4_scalar_iv_keep_htable_scalar_rk2.o";;

let AES_GCM_ENC_KERNEL_X4_SCALAR_IV_KEEP_HTABLE_SCALAR_RK2_EXEC = ARM_MK_EXEC_RULE aes_gcm_enc_kernel_x4_scalar_iv_keep_htable_scalar_rk2_mc;;
...
let SCALAR_IV_KEEP_HTABLE_SCALAR_RK2_SPLIT = prove
...
let CTR_BLOCK_BUILD_V = prove
 (`word_join (ivhi:int64) (ivlo:int64):int128 =
  ...
  DISCH_THEN(fun th -> MP_TAC(MATCH_MP SCALAR_IV_KEEP_HTABLE_SCALAR_RK2_SPLIT th)) THEN
...
let X11_SETUP = prove
 (`word_join (ivhi:int64) (ivlo:int64):int128 =
  ...
  DISCH_THEN(fun th -> MP_TAC(MATCH_MP SCALAR_IV_KEEP_HTABLE_SCALAR_RK2_SPLIT th)) THEN SIMP_TAC[]);;
...
(* In this scalar_rk variant the final AES round key (EL 10) is XORed in scalar    *)
(* registers X20/X21 into the input halves, and the block ciphertext (pre-byteswap) *)
(* is assembled in a vector register by two FMOVs: "fmov d29,x22 ; fmov v29.d[1],x23" *)
(* giving Q29 = word_insert (word_zx x22) (64,64) x23.  This lemma turns that         *)
(* FMOV-built form (both outer word_xor operand orders) into the                     *)
(* word_xor <9round> (rk10 xor inblock) shape XOR_AES128_CIPHER_RECONSTRUCT consumes. *)
let SCALAR_RK_RECONSTRUCT_FMOV = prove
 (`(word_xor
     (word_insert
        (word_zx (word_xor (word_subword (inb:int128) (0,64):int64)
                           (word_subword (word_reversefields 8 (rk10:int128)) (0,64):int64)):int128)
        (64,64)
        (word_xor (word_subword inb (64,64):int64)
                  (word_subword (word_reversefields 8 rk10) (64,64):int64)) :int128)
     (nineround:int128)
...
(* Split a 128-bit input-block read into two 64-bit halves at the folded          *)
(* "in_p + word(64*i + K)" address form.  In this variant the four unrolled input *)
(* blocks are loaded by "ldp x22,x23,[x0],#64" (block 0, post-indexed) then        *)
(* "ldp x22,x23,[x0,#-48/-32/-16]" (blocks 1..3, offset mode on the advanced x0);  *)
(* all four resolve their two scalar halves against this single folded form.       *)
...
(* Tail-loop input split: the tail block is loaded by the POST-INDEXED             *)
(* "ldp x22,x23,[x0],#16", whose second-register read address the ARM model leaves *)
(* as in_p + word((64*loop_count + 16*i) + 8) -- the "+ 8" is a separate num        *)
(* summand (the block offset has a symbolic i), so DON'T apply the numeric fold.    *)
...
(* save instructions) and ends at pc + 0x434 (first ldp of the postamble).   *)
(* The stackpointer is the value AFTER the sub sp, #0x120 adjustment, i.e.    *)
...
let AES_GCM_ENC_KERNEL_X4_SCALAR_IV_KEEP_HTABLE_SCALAR_RK2_CORRECT = prove
 (`!in_p out_p len_bits tag_p ivec_p key_p htable_p tag0 nonce rk inblock pc.
       ALLPAIRS nonoverlapping
        ...
        [(word pc, LENGTH aes_gcm_enc_kernel_x4_scalar_iv_keep_htable_scalar_rk2_mc);
         (in_p,  16 * val len_bits DIV 128); (key_p, 176); (htable_p, 192)] /\
       PAIRWISE nonoverlapping
        [(out_p, 16 * val len_bits DIV 128); (tag_p, 16); (ivec_p, 16)]
    ==>
    ensures arm
      (\s. aligned_bytes_loaded s (word pc) aes_gcm_enc_kernel_x4_scalar_iv_keep_htable_scalar_rk2_mc /\
  ...
  REWRITE_TAC[ALLPAIRS; PAIRWISE; ALL; fst AES_GCM_ENC_KERNEL_X4_SCALAR_IV_KEEP_HTABLE_SCALAR_RK2_EXEC] THEN
  ...
  REWRITE_TAC[htable_mem_4; GSYM CONJ_ASSOC] THEN CONJ_TAC THENL
   [ENSURES_INIT_TAC "s0" THEN
    ...
    ARM_STEPS_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_KEEP_HTABLE_SCALAR_RK2_EXEC (1--29) THEN
  ...
  REWRITE_TAC[htable_mem_4; GSYM CONJ_ASSOC] THEN CONJ_TAC THENL
   [ASM_CASES_TAC `loop_count = 0` THENL
     [POP_ASSUM SUBST_ALL_TAC THEN
      ARM_SIM_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_KEEP_HTABLE_SCALAR_RK2_EXEC [1] THEN
    ...
    ENSURES_WHILE_UP_TAC `loop_count:num` `pc + 0x0a4` `pc + 0x34c`
      `\i s.
     ...
     [ARM_SIM_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_KEEP_HTABLE_SCALAR_RK2_EXEC [1] THEN
      ...
      STRIP_ASSUME_TAC THENL
       [REWRITE_TAC[ARITH_RULE
        ...
        ALL_TAC] THEN
      (*** In this scalar_rk variant the input block is loaded as a scalar pair     ***)
      (*** "ldp x22,x23,[x0,#K]"; split each 128-bit input read into two 64-bit      ***)
      (*** halves so those loads resolve.                                           ***)
      REPEAT(FIRST_X_ASSUM(STRIP_ASSUME_TAC o CONV_RULE SPLIT_INPUT_CONV o
        check (fun th -> let c = concl th in
          is_eq c && free_in `in_p:int64` (lhs c) &&
          can (find_term (fun t -> is_const t && fst(dest_const t) = "bytes128"))
              (lhs c)))) THEN
      MAP_EVERY(fun n -> ARM_STEPS_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_KEEP_HTABLE_SCALAR_RK2_EXEC [n] THEN
      ...
      REWRITE_TAC[SCALAR_RK_RECONSTRUCT_FMOV] THEN
  ...
  ENSURES_WHILE_UP_TAC `loop_remain:num` `pc + 0x354` `pc + 0x41c`
    `\i s.
   ...
   [ENSURES_INIT_TAC "s0" THEN
    ...
    ASSUME_TAC THENL
     [REWRITE_TAC[ARITH_RULE `64 * a + 16 * b = 16 * (4 * a + b)`] THEN
      ...
      ALL_TAC] THEN
    (*** Split the tail block's input read for the POST-INDEXED scalar             ***)
    (*** "ldp x22,x23,[x0],#16" load (tail-specific conversion).                   ***)
...
let AES_GCM_ENC_KERNEL_X4_SCALAR_IV_KEEP_HTABLE_SCALAR_RK2_SUBROUTINE_CORRECT = prove
 (`!in_p out_p len_bits tag_p ivec_p key_p htable_p tag0 nonce rk inblock
  ...
  REWRITE_TAC[fst AES_GCM_ENC_KERNEL_X4_SCALAR_IV_KEEP_HTABLE_SCALAR_RK2_EXEC; htable_mem_4] THEN
  CONV_TAC(ONCE_DEPTH_CONV WORDLIST_FROM_MEMORY_CONV) THEN
  ARM_ADD_RETURN_STACK_TAC
    ~pre_post_nsteps:(11, 11)
    AES_GCM_ENC_KERNEL_X4_SCALAR_IV_KEEP_HTABLE_SCALAR_RK2_EXEC
    (CONV_RULE(ONCE_DEPTH_CONV WORDLIST_FROM_MEMORY_CONV)
       (REWRITE_RULE[fst AES_GCM_ENC_KERNEL_X4_SCALAR_IV_KEEP_HTABLE_SCALAR_RK2_EXEC; htable_mem_4]
          AES_GCM_ENC_KERNEL_X4_SCALAR_IV_KEEP_HTABLE_SCALAR_RK2_CORRECT))
