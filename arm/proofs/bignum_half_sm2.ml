...
let bignum_half_sm2_mc = define_assert_from_elf "bignum_half_sm2_mc" "arm/sm2/bignum_half_sm2.o"
[
  ...
  0x92607ce8;       (* arm_AND X8 X7 (rvalue (word 18446744069414584320)) *)
...
let BIGNUM_HALF_SM2_EXEC = ARM_MK_EXEC_RULE bignum_half_sm2_mc;;
...
let BIGNUM_HALF_SM2_CORRECT = time prove
 (`!z x n pc.
        ...
        ==> ensures arm
             (\s. aligned_bytes_loaded s (word pc) bignum_half_sm2_mc /\
                  read PC s = word pc /\
                  C_ARGUMENTS [z; x] s /\
  ...
  ARM_STEPS_TAC BIGNUM_HALF_SM2_EXEC (1--4) THEN
  ...
  SUBGOAL_THEN `bit 0 (n_0:int64) <=> ODD n` SUBST_ALL_TAC THENL
   [EXPAND_TAC "n" THEN REWRITE_TAC[BIT_LSB; ODD_ADD; ODD_MULT; ODD_EXP] THEN
    ...
    ALL_TAC] THEN
  ARM_ACCSTEPS_TAC BIGNUM_HALF_SM2_EXEC (5--11) (5--17) THEN
 ...
 (`!z x n pc returnaddress.
        ...
        ==> ensures arm
          ...
          (MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bignum(z,4)])`,
  ARM_ADD_RETURN_NOSTACK_TAC BIGNUM_HALF_SM2_EXEC
...
let full_spec,public_vars = mk_safety_spec
    ...
    BIGNUM_HALF_SM2_SUBROUTINE_CORRECT
...
let BIGNUM_HALF_SM2_SUBROUTINE_SAFE = time prove
 (`exists f_events.
     forall e z x pc returnaddress.
         ...
         ==> ensures arm
             (\s.
                  aligned_bytes_loaded s (word pc) bignum_half_sm2_mc /\
                  read PC s = word pc /\
                  read X30 s = returnaddress /\
                  C_ARGUMENTS [z; x] s /\
                  read events s = e)
