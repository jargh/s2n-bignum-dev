...
let EDWARDS25519_PEPADD_SAFE = time prove
 (`exists f_events.
       forall e p3 p1 p2 pc stackpointer.
           aligned 16 stackpointer /\
           ALL (nonoverlapping (stackpointer,192))
           [word pc,LENGTH edwards25519_pepadd_mc; p1,128; p2,96; p3,128] /\
           nonoverlapping (p3,128) (word pc,LENGTH edwards25519_pepadd_mc)
           ==> ensures arm
               (\s.
                    aligned_bytes_loaded s (word pc) edwards25519_pepadd_mc /\
                    read PC s = word (pc + 0x8) /\
                    read SP s = stackpointer /\
                    C_ARGUMENTS [p3; p1; p2] s /\
                    read events s = e)
               (\s.
                    ...
                    (exists e2.
                         ...
                         [p1,128; p2,96; p3,128; stackpointer,192]
       ...
       forall e p3 p1 p2 pc stackpointer returnaddress.
        ...
        ==> ensures arm
          ...
          (MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(p3,128);
                      memory :> bytes(word_sub stackpointer (word 208),208)])`,
  REWRITE_TAC[fst EDWARDS25519_PEPADD_EXEC] THEN
  ARM_ADD_RETURN_STACK_TAC EDWARDS25519_PEPADD_EXEC
    (REWRITE_RULE[fst EDWARDS25519_PEPADD_EXEC]EDWARDS25519_PEPADD_SAFE)
    `[X19; X20]` 208 THEN
