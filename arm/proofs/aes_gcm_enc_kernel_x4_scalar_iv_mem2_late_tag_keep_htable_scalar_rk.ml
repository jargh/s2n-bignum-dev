...
(* print_literal_from_elf "arm/aes_gcm/aes_gcm_enc_kernel_x4_scalar_iv_mem2_late_tag_keep_htable_scalar_rk.o";; *)

let aes_gcm_enc_kernel_x4_scalar_iv_mem2_late_tag_keep_htable_scalar_rk_mc =
  define_from_elf "aes_gcm_enc_kernel_x4_scalar_iv_mem2_late_tag_keep_htable_scalar_rk_mc"
    "arm/aes_gcm/aes_gcm_enc_kernel_x4_scalar_iv_mem2_late_tag_keep_htable_scalar_rk.o";;

let AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC = ARM_MK_EXEC_RULE aes_gcm_enc_kernel_x4_scalar_iv_mem2_late_tag_keep_htable_scalar_rk_mc;;
...
(* The scalar_rk ciphertext round-trip "stp x22,x23,[sp,#OFF]; ldr q29,[sp,#OFF]" *)
(* writes a clean 128-bit value as two 64-bit halves (x22 lo, x23 hi), so it needs *)
(* only the single bytes128 -> two-bytes64 split (unlike the 3-way counter store). *)
(* Fire this on the state right after the stp (before the ldr q29) so the reload   *)
(* resolves to word_join x23 x22 and the subsequent "str q0,[output]" records a    *)
(* fully-resolved output value instead of a stale stack read.                      *)
let MERGE_Q29_TAC off sname =
...
(* The scalar_rk final round loads the input block with "ldp x22,x23,[input,#K]" *)
(* (64-bit pair) rather than "ldr q,[input,#K]" (128-bit).  Split each 128-bit    *)
(* input-block assumption into its two address-normalised 64-bit halves so those  *)
(* scalar loads resolve during stepping (otherwise the ciphertext keeps a         *)
(* symbolic input operand and the output store fact is dropped as stale).         *)
...
(* Tail-loop variant: the single tail block's input read is a post-indexed       *)
(* "ldp x22,x23,[x0],#16", so no fixed offset needs folding into the address.     *)
...
(* Both operand orders of the outer word_xor are covered.                          *)
...
let AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_CORRECT = prove
 (`!in_p out_p len_bits tag_p ivec_p key_p htable_p tag0 nonce rk inblock pc
     stackpointer.
       ...
       ALLPAIRS nonoverlapping
        [(out_p, 16 * val len_bits DIV 128); (tag_p, 16); (ivec_p, 16);
         (word_add stackpointer (word 160), 128)]
        [(word pc, LENGTH aes_gcm_enc_kernel_x4_scalar_iv_mem2_late_tag_keep_htable_scalar_rk_mc);
    ...
    ==>
    ensures arm
      (\s. aligned_bytes_loaded s (word pc) aes_gcm_enc_kernel_x4_scalar_iv_mem2_late_tag_keep_htable_scalar_rk_mc /\
      ...
      (MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
       ...
       MAYCHANGE [memory :> bytes(out_p, 16 * val len_bits DIV 128);
                  ...
                  memory :> bytes(word_add stackpointer (word 160), 128)])`,
  ...
  REWRITE_TAC[ALLPAIRS; PAIRWISE; ALL; fst AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC] THEN
  ...
  REWRITE_TAC[htable_mem_4; GSYM CONJ_ASSOC] THEN CONJ_TAC THENL
   [ENSURES_INIT_TAC "s0" THEN
    ...
    (*** The scalar_rk variant loads the final round key (EL 10 rk) into X20/X21   ***)
    (*** via "ldp x20,x21,[x5,#160]" - a PAIR load of two 64-bit halves.  Split    ***)
    (*** the 128-bit key-cell precondition read at key_p+160 into its two bytes64   ***)
    (*** halves BEFORE stepping, so the ldp resolves X20/X21 to the closed values   ***)
    (*** word_subword(rev8(EL 10 rk))(0/64,64) rather than an old-state read that   ***)
    (*** DISCARD_OLDSTATE would drop.                                               ***)
    FIRST_X_ASSUM(STRIP_ASSUME_TAC o CONV_RULE(READ_MEMORY_SPLIT_CONV 1) o
      check (fun th -> let c = concl th in
        is_eq c &&
        can (term_match [] `read (memory :> bytes128 (word_add key_p (word 160))) s0 = xx`) c)) THEN
    ARM_STEPS_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC (1--33) THEN
    ...
    REPEAT CONJ_TAC THENL
     [(*** ivec memory read: recombine the two abbreviated halves ***)
      ...
      (*** X20/X21 (key10 halves) now auto-close in the leading ASM_REWRITE_TAC[] ***)
      (*** because the key-160 cell was pre-split into bytes64 halves before      ***)
      (*** stepping, so the ldp x20,x21 recorded closed read-facts.               ***)
  ...
  REWRITE_TAC[htable_mem_4; GSYM CONJ_ASSOC] THEN CONJ_TAC THENL
   [ASM_CASES_TAC `loop_count = 0` THENL
     [POP_ASSUM SUBST_ALL_TAC THEN
      ARM_SIM_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC [1] THEN
    ...
    ENSURES_WHILE_UP_TAC `loop_count:num` `pc + 0x0b4` `pc + 0x350`
      `\i s.
     ...
     [ARM_SIM_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC [1] THEN
      ...
      STRIP_ASSUME_TAC THENL
       [REWRITE_TAC[ARITH_RULE
        ...
        ALL_TAC] THEN
      (*** The scalar_rk final round loads each input block via "ldp x22,x23,       ***)
      (*** [input,#K]" (64-bit pair), so split the four 128-bit input assumptions   ***)
      (*** into address-normalised 64-bit halves before stepping.                   ***)
      REPEAT(FIRST_X_ASSUM(STRIP_ASSUME_TAC o CONV_RULE SPLIT_INPUT_CONV o
        check (fun th -> let c = concl th in
          ...
          can (find_term (fun t -> is_const t && fst(dest_const t) = "bytes128"))
              (lhs c)))) THEN
      (*** Step through the four unrolled blocks.  Each block has TWO stack        ***)
      (*** round-trips needing a merge: the 3-way counter store "str w14,[sp,        ***)
      (*** #OFF+12]; ldr q0,[sp,#OFF]" (counter, OFF descending 208/192/176/160)    ***)
      (*** and the scalar_rk ciphertext "stp x22,x23,[sp,#OFF2]; ldr q29,[sp,#OFF2]" ***)
      (*** (Q29, OFF2 descending 272/256/240/224).  Merge counters at steps          ***)
      (*** 4/40/79/118 and Q29 at 27/63/102/141.  Body is 167 steps (cbnz 0x350).    ***)
      MAP_EVERY(fun n -> ARM_STEPS_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC [n] THEN
            RULE_ASSUM_TAC(CONV_RULE(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV)))
          (1--8) THEN
      MERGE_CTR128_TAC 160 "s8" THEN
      MAP_EVERY(fun n -> ARM_STEPS_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC [n] THEN
            RULE_ASSUM_TAC(CONV_RULE(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV)))
          (9--9) THEN
      MERGE_CTR128_TAC 208 "s9" THEN
      MAP_EVERY(fun n -> ARM_STEPS_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC [n] THEN
            RULE_ASSUM_TAC(CONV_RULE(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV)))
          (10--17) THEN
      MERGE_CTR128_TAC 176 "s17" THEN
      MAP_EVERY(fun n -> ARM_STEPS_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC [n] THEN
            RULE_ASSUM_TAC(CONV_RULE(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV)))
          (18--22) THEN
      MERGE_Q29_TAC 272 "s22" THEN
      MAP_EVERY(fun n -> ARM_STEPS_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC [n] THEN
            RULE_ASSUM_TAC(CONV_RULE(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV)))
          (23--35) THEN
      MERGE_Q29_TAC 240 "s35" THEN
      MAP_EVERY(fun n -> ARM_STEPS_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC [n] THEN
            RULE_ASSUM_TAC(CONV_RULE(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV)))
          (36--38) THEN
      MERGE_Q29_TAC 256 "s38" THEN
      MAP_EVERY(fun n -> ARM_STEPS_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC [n] THEN
            RULE_ASSUM_TAC(CONV_RULE(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV)))
          (39--47) THEN
      MERGE_Q29_TAC 224 "s47" THEN
      MAP_EVERY(fun n -> ARM_STEPS_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC [n] THEN
            RULE_ASSUM_TAC(CONV_RULE(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV)))
          (48--54) THEN
      MERGE_CTR128_TAC 192 "s54" THEN
      MAP_EVERY(fun n -> ARM_STEPS_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC [n] THEN
            RULE_ASSUM_TAC(CONV_RULE(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV)))
          (55--167) THEN
  ...
  ASM_CASES_TAC `loop_remain = 0` THENL
   [POP_ASSUM SUBST_ALL_TAC THEN
    ...
    CONV_TAC(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV) THEN
    (*** Prove the (assumption-free) bit-level goal standalone and accept it.          ***)
    W(fun (_,w) -> ACCEPT_TAC(prove(w, REPEAT CONJ_TAC THEN CONV_TAC WORD_BLAST)));

    ALL_TAC] THEN

  (*** Loop setup for the tail loop ***)

  ENSURES_WHILE_UP_TAC `loop_remain:num` `pc + 0x358` `pc + 0x420`
    `\i s.
   ...
   [ARM_SIM_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC [1] THEN
    ...
    ASSUME_TAC THENL
     [REWRITE_TAC[ARITH_RULE `64 * a + 16 * b = 16 * (4 * a + b)`] THEN
      ...
      ALL_TAC] THEN
    (*** Split the tail block's 128-bit input read into two address-normalised    *)
    (*** 64-bit halves for the post-indexed scalar "ldp x22,x23,[x0],#16" load.    *)
    FIRST_X_ASSUM(STRIP_ASSUME_TAC o CONV_RULE SPLIT_INPUT_TAIL_CONV o
      check (fun th -> let c = concl th in
        is_eq c && free_in `in_p:int64` (lhs c) &&
        can (find_term (fun t -> is_const t && fst(dest_const t) = "bytes128"))
            (lhs c))) THEN
    (*** The single tail block has two stack round-trips: the 3-way counter store *)
    (*** "str w14,[sp,#172]; ldr q0,[sp,#160]" (merge s4, offset 160) and the      *)
    (*** scalar_rk ciphertext "stp x22,x23,[sp,#224]; ldr q29,[sp,#224]" (merge s27,*)
    (*** offset 224).  Body is 50 steps (cbnz 0x420 at step 51).                    *)
    MAP_EVERY(fun n -> ARM_STEPS_TAC AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC [n] THEN
      RULE_ASSUM_TAC(CONV_RULE(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV)))
     (1--5) THEN
    ...
    (*** Scalar counter reconstruction (tail loop, single block).  As in the main   ***)
    (*** loop, the scalar_rk final round assembles the ciphertext as                 ***)
    (*** word_xor (9-round) (word_join <inb_hi^rk10_hi> <inb_lo^rk10_lo>) from the    ***)
    (*** X22/X23 scalar XORs, so SCALAR_RK_RECONSTRUCT must fire before               ***)
    (*** XOR_AES128_CIPHER_RECONSTRUCT.                                               ***)
    REWRITE_TAC[ZX_COUNTER_UD; ZX_COUNTER_INC] THEN
    REWRITE_TAC[GSYM WORD_ADD] THEN
    REWRITE_TAC[CTR_BLOCK_BUILD_INSERT] THEN
    REWRITE_TAC[SCALAR_RK_RECONSTRUCT] THEN
    ...
    ASM_SIMP_TAC[WORD_SUB; LT_IMP_LE; ARITH_RULE `i < l ==> i + 1 <= l`] THEN
    DISCARD_STATE_TAC "s50" THEN
    REWRITE_TAC[ADD_ASSOC; ARITH] THEN
    (*** Counter reconstruction (tail loop, single block, mem2 3-way-merge specific): ***)
    (*** the AES-embedded counter block is NOT collapsed by the zx-nest              ***)
    (*** CTR_BLOCK_BUILD_INSERT applied earlier (WORD_SIMPLE_SUBWORD_CONV rewrote it   ***)
    (*** to the plain word_bytereverse(word cval) three-way join), so use the same    ***)
    (*** plain-form chain as the 4-block main loop: collapse subwords, fold the        ***)
    (*** increment, apply CTR_BLOCK_BUILD_INSERT_PLAIN, kill the double reversefields, ***)
    (*** then run the cipher/NIST reconstruction chain once.                          ***)
    CONV_TAC(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV) THEN
    REWRITE_TAC[GSYM WORD_ADD] THEN
    REWRITE_TAC[CTR_BLOCK_BUILD_INSERT_PLAIN] THEN
    REWRITE_TAC[WORD_REVERSEFIELDS_REVERSEFIELDS] THEN
...
let AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_SUBROUTINE_CORRECT = prove
 (`!in_p out_p len_bits tag_p ivec_p key_p htable_p tag0 nonce rk inblock
  ...
  REWRITE_TAC[fst AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC; htable_mem_4] THEN
  CONV_TAC(ONCE_DEPTH_CONV WORDLIST_FROM_MEMORY_CONV) THEN
  ARM_ADD_RETURN_STACK_TAC
    ~pre_post_nsteps:(11, 11)
    AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC
    (CONV_RULE(ONCE_DEPTH_CONV WORDLIST_FROM_MEMORY_CONV)
       (REWRITE_RULE[fst AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_EXEC; htable_mem_4]
          AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM2_LATE_TAG_KEEP_HTABLE_SCALAR_RK_CORRECT))
