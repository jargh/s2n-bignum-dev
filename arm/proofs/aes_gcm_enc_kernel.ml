...
[
  ...
  0x0f06e447;       (* movi.8b	v7, #0xc2 *)
  0x5f7854e7;       (* shl	d7, d7, #0x38 *)
...
(* ------------------------------------------------------------------------- *)
(* Correctness theorem (stub).                                               *)
(*                                                                           *)
(* Arguments (Standard ARM ABI):                                             *)
...
(*                                                                           *)
(* The kernel processes (len_bits/8)/16 complete 16-byte blocks:            *)
(*   - encrypts each block in counter mode (CTR with inc32)                 *)
(*   - folds each ciphertext block into the GHASH accumulator               *)
(*   - writes updated tag and counter back to memory                        *)
(* ------------------------------------------------------------------------- *)

let AES_GCM_ENC_KERNEL_CORRECT = prove(
  `!in_p out_p len_bits tag_p ivec_p key_p htable_p
    input_data tag0 ivec0
    ...
    ensures arm
      ...
      (\s. read PC s = word_add (word pc) (word (LENGTH aes_gcm_enc_kernel_mc)))
           (* TODO: strengthen postcondition with:                          *)
           (*   - ciphertext correctness: output = CTR(input, ivec, key)    *)
           (*   - tag update: tag = nist_ghash H tag0 ciphertext_blocks     *)
           (*   - ivec update: ivec = counter incremented nblocks times     *)
           (*   - return value: X0 = len_bits / 8                           *)
           )
