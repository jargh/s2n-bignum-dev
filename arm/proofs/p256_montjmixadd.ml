...
let P256_MONTJMIXADD_SAFE = time prove
 (`exists f_events.
       forall e p3 p1 p2 pc stackpointer.
           aligned 16 stackpointer /\
           ALL (nonoverlapping (stackpointer,192))
           [word pc,LENGTH p256_montjmixadd_mc; p1,96; p2,64; p3,96] /\
           nonoverlapping (p3,96) (word pc,LENGTH p256_montjmixadd_mc)
           ==> ensures arm
               (\s.
                    aligned_bytes_loaded s (word pc) p256_montjmixadd_mc /\
                    read PC s = word (pc + 0x8) /\
                    read SP s = stackpointer /\
                    C_ARGUMENTS [p3; p1; p2] s /\
                    read events s = e)
               (\s.
                    read PC s = word (pc + 0x1e70) /\
                    (exists e2.
       ...
       forall e p3 p1 p2 pc stackpointer returnaddress.
        ...
        ==> ensures arm
          ...
          (MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(p3,96);
                      memory :> bytes(word_sub stackpointer (word 208),208)])`,
  REWRITE_TAC[fst P256_MONTJMIXADD_EXEC] THEN
  ARM_ADD_RETURN_STACK_TAC P256_MONTJMIXADD_EXEC
    (REWRITE_RULE[fst P256_MONTJMIXADD_EXEC]P256_MONTJMIXADD_SAFE)
    `[X19;X20]` 208 THEN
