...
let bignum_sub_sm2_mc = define_assert_from_elf "bignum_sub_sm2_mc" "arm/sm2/bignum_sub_sm2.o"
[
...
let BIGNUM_SUB_SM2_EXEC = ARM_MK_EXEC_RULE bignum_sub_sm2_mc;;
...
let BIGNUM_SUB_SM2_CORRECT = time prove
 (`!z x y m n pc.
        ...
        ==> ensures arm
             (\s. aligned_bytes_loaded s (word pc) bignum_sub_sm2_mc /\
                  read PC s = word pc /\
                  C_ARGUMENTS [z; x; y] s /\
  ...
  ARM_ACCSTEPS_TAC BIGNUM_SUB_SM2_EXEC (1--8) (1--8) THEN
  ...
  ARM_STEPS_TAC BIGNUM_SUB_SM2_EXEC [9] THEN
  RULE_ASSUM_TAC(REWRITE_RULE[WORD_UNMASK_64; NOT_LE]) THEN
  ARM_ACCSTEPS_TAC BIGNUM_SUB_SM2_EXEC (10--17) (10--17) THEN
 ...
 (`!z x y m n pc returnaddress.
        ...
        ==> ensures arm
          ...
          (MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bignum(z,4)])`,
  ARM_ADD_RETURN_NOSTACK_TAC BIGNUM_SUB_SM2_EXEC BIGNUM_SUB_SM2_CORRECT);;
...
let full_spec,public_vars = mk_safety_spec
    ...
    BIGNUM_SUB_SM2_SUBROUTINE_CORRECT
    BIGNUM_SUB_SM2_EXEC;;

let BIGNUM_SUB_SM2_SUBROUTINE_SAFE = time prove
 (`exists f_events.
     forall e z x y pc returnaddress.
         nonoverlapping (word pc,72) (z,8 * 4)
         ==> ensures arm
             (\s.
                  aligned_bytes_loaded s (word pc) bignum_sub_sm2_mc /\
                  read PC s = word pc /\
                  read X30 s = returnaddress /\
                  C_ARGUMENTS [z; x; y] s /\
                  read events s = e)
