...
let bignum_mod_p256_4_mc = define_assert_from_elf "bignum_mod_p256_4_mc" "arm/p256/bignum_mod_p256_4.o"
[
...
let BIGNUM_MOD_P256_4_EXEC = ARM_MK_EXEC_RULE bignum_mod_p256_4_mc;;
...
let BIGNUM_MOD_P256_4_CORRECT = time prove
 (`!z x n pc.
      ...
      ==> ensures arm
           (\s. aligned_bytes_loaded s (word pc) bignum_mod_p256_4_mc /\
                read PC s = word pc /\
                C_ARGUMENTS [z; x] s /\
  ...
  ARM_ACCSTEPS_TAC BIGNUM_MOD_P256_4_EXEC (1--15) (1--15) THEN

  RULE_ASSUM_TAC(REWRITE_RULE
   [REAL_BITVAL_NOT; ADD_CLAUSES; VAL_WORD_BITVAL]) THEN

  SUBGOAL_THEN `carry_s9 <=> m < p_256` SUBST_ALL_TAC THENL
  ...
  ANTS_TAC THENL
   [REWRITE_TAC[p_256] THEN ASM_ARITH_TAC; DISCH_THEN SUBST1_TAC] THEN
...
let BIGNUM_MOD_P256_4_SUBROUTINE_SAFE = time prove
 (`exists f_events.
     forall e z x pc returnaddress.
         nonoverlapping (word pc,64) (z,8 * 4)
         ==> ensures arm
             (\s.
                  aligned_bytes_loaded s (word pc) bignum_mod_p256_4_mc /\
                  read PC s = word pc /\
                  read X30 s = returnaddress /\
                  C_ARGUMENTS [z; x] s /\
                  read events s = e)
