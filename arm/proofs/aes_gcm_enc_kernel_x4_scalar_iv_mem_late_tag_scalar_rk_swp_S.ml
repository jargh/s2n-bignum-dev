...
(*                                                                           *)
(* Structure:                                                                *)
(*   - the lemma substrate (ctr_block / aes_ctr_block / cipher_block /       *)
(*     nist_cipher_block / htable_mem_4, the AES / counter / GHASH-reduce    *)
(*     reconstruction lemmas, MERGE_CTR128_TAC) is in aes_gcm_utils.ml;      *)
(*     this file adds its own counter lemmas and SPLIT_INPUT_CONV.           *)
...
let AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_SCALAR_RK_SWP_S_SUBROUTINE_CORRECT = prove
 (`!in_p out_p len_bits tag_p ivec_p key_p htable_p tag0 nonce rk inblock
  ...
  REWRITE_TAC[fst SWPS_EXEC; htable_mem_4] THEN
  CONV_TAC(ONCE_DEPTH_CONV WORDLIST_FROM_MEMORY_CONV) THEN
  ARM_ADD_RETURN_STACK_TAC
    ~pre_post_nsteps:(11, 11)
    SWPS_EXEC
    (CONV_RULE(ONCE_DEPTH_CONV WORDLIST_FROM_MEMORY_CONV)
       (REWRITE_RULE[fst SWPS_EXEC; htable_mem_4]
          AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_SCALAR_RK_SWP_S_CORRECT))
...
(* _swp_S is single-buffered: STEADY runs loop_count-1 times, so the DRAIN entry  *)
...
let AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_SCALAR_RK_SWP_S_SAFE = prove
 (`exists f_events.
  ...
  CONJ_TAC THENL
   [(*** MAIN REGION pc+0x2c -> pc+0x61c (setup + pipelined loop). ***)
    ...
    (*** _swp_S is SINGLE-buffered: FILL does NOT prefetch a block, so X0/X2   ***)
...
let AES_GCM_ENC_KERNEL_X4_SCALAR_IV_MEM_LATE_TAG_SCALAR_RK_SWP_S_SUBROUTINE_SAFE = prove
 (`exists f_events.
...
Printf.printf "\n*** ENC SWP_S SUBROUTINE SAFE proven ***\n%!";;
