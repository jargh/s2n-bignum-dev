...
(* print_literal_from_elf "arm/aes_gcm/aes_gcm_enc_kernel_256_x4_scalar_iv_mem_late_tag_scalar_rk.o";; *)

let aes_gcm_enc_kernel_256_x4_scalar_iv_mem_late_tag_scalar_rk_mc =
  define_from_elf "aes_gcm_enc_kernel_256_x4_scalar_iv_mem_late_tag_scalar_rk_mc"
    "arm/aes_gcm/aes_gcm_enc_kernel_256_x4_scalar_iv_mem_late_tag_scalar_rk.o";;

let AES_GCM_ENC_KERNEL_256_X4_SCALAR_IV_MEM_LATE_TAG_SCALAR_RK_EXEC = ARM_MK_EXEC_RULE aes_gcm_enc_kernel_256_x4_scalar_iv_mem_late_tag_scalar_rk_mc;;
...
(*   X5 = key       AES-256 round keys (240 bytes = 15 x 16)                 *)
...
let AES_GCM_ENC_KERNEL_256_X4_SCALAR_IV_MEM_LATE_TAG_SCALAR_RK_CORRECT = prove
 (`!in_p out_p len_bits tag_p ivec_p key_p htable_p tag0 nonce rk inblock pc
     stackpointer.
       ...
       ALLPAIRS nonoverlapping
        ...
        [(word pc, LENGTH aes_gcm_enc_kernel_256_x4_scalar_iv_mem_late_tag_scalar_rk_mc);
    ...
    ==>
    ensures arm
      (\s. aligned_bytes_loaded s (word pc) aes_gcm_enc_kernel_256_x4_scalar_iv_mem_late_tag_scalar_rk_mc /\
  ...
  REWRITE_TAC[ALLPAIRS; PAIRWISE; ALL; fst AES_GCM_ENC_KERNEL_256_X4_SCALAR_IV_MEM_LATE_TAG_SCALAR_RK_EXEC] THEN
  ...
  REWRITE_TAC[htable_mem_4; GSYM CONJ_ASSOC] THEN CONJ_TAC THENL
   [ENSURES_INIT_TAC "s0" THEN
    ...
    ARM_STEPS_TAC AES_GCM_ENC_KERNEL_256_X4_SCALAR_IV_MEM_LATE_TAG_SCALAR_RK_EXEC (1--33) THEN
  ...
  REWRITE_TAC[htable_mem_4; GSYM CONJ_ASSOC] THEN CONJ_TAC THENL
   [ASM_CASES_TAC `loop_count = 0` THENL
     [POP_ASSUM SUBST_ALL_TAC THEN
      ARM_SIM_TAC AES_GCM_ENC_KERNEL_256_X4_SCALAR_IV_MEM_LATE_TAG_SCALAR_RK_EXEC [1] THEN
    ...
    ENSURES_WHILE_UP_TAC `loop_count:num` `pc + 0x0b4` `pc + 0x3f8`
      `\i s.
     ...
     [ARM_SIM_TAC AES_GCM_ENC_KERNEL_256_X4_SCALAR_IV_MEM_LATE_TAG_SCALAR_RK_EXEC [1] THEN
      ...
      STRIP_ASSUME_TAC THENL
       [REWRITE_TAC[ARITH_RULE
        ...
        ALL_TAC] THEN
      (*** In this scalar_rk variant the final AES round key is XORed in scalar     ***)
      (*** registers and the input block is loaded by "ldp x22,x23,[x0,#K]", i.e.   ***)
      (*** at 64-bit granularity.  Split each of the four input-block reads into    ***)
      (*** two address-normalised 64-bit halves so those scalar loads resolve.      ***)
      REPEAT(FIRST_X_ASSUM(STRIP_ASSUME_TAC o CONV_RULE SPLIT_INPUT_CONV o
        check (fun th -> let c = concl th in
          is_eq c && free_in `in_p:int64` (lhs c) &&
          can (find_term (fun t -> is_const t && fst(dest_const t) = "bytes128"))
              (lhs c)))) THEN
      (*** Step through the four unrolled blocks.  Each block (i) assembles its     ***)
      (*** counter block on the stack ("stp x11,x14,[sp,#OFF]" at steps 5/45/85/128, ***)
      (*** offsets 208/192/176/160 descending) then reloads it into Q0              ***)
      (*** ("ldr q0,[sp,#OFF]"); MERGE_CTR128_TAC bridges the two 64-bit stores.    ***)
      (*** We then DISCARD the counter's sp-slot facts so the ciphertext store to   ***)
      (*** the SAME slot lands cleanly.  (ii) After the 9 AES rounds the input      ***)
      (*** halves are XORed with key 10 in X22/X23 and round-tripped through the    ***)
      (*** same slot ("stp x22,x23,[sp,#OFF]" at steps 28/68/108/151, "ldr q29");   ***)
      (*** a second MERGE_CTR128_TAC resolves Q29 = word_join x23 x22.              ***)
      (*** AES-256: 4 blocks x 14 rounds; 8 scalar counter-scratch stores land at    ***)
      (*** steps 13/15/22/25/54/60/68/74 (offsets 160/192/160/176/192/176/208/208),  ***)
      (*** each followed by MERGE_CTR128_TAC.  Segment boundaries = those store steps.***)
      (*** DISCARD before a slot is reused by a later store (160 reused @22, 192 @54, ***)
      (*** 176 @60, 208 @74).  Total 209 steps (128 was 177; +32 = 4 blocks x 4 extra ***)
      (*** rounds x 2 insns).                                                         ***)
      MAP_EVERY(fun n -> ARM_STEPS_TAC AES_GCM_ENC_KERNEL_256_X4_SCALAR_IV_MEM_LATE_TAG_SCALAR_RK_EXEC [n] THEN
      ...
      (*** NB: NO DISCARD_MATCHING_ASSUMPTIONS here.  Unlike the 128 schedule, the 256 SLOTHY schedule    ***)
      (*** re-reads each counter-scratch slot LATE (e.g. ldr q10,[sp,#192] at step 77 needs the value     ***)
      (*** stored at step 54).  A premature DISCARD of a slot erases the value a later ldr q?,[sp,#OFF]    ***)
      (*** depends on, so its q-reg loses its concrete fact -> the eventual output store records a         ***)
      (*** dangling `read Q? s' ` that DISCARD_OLDSTATE then erases (dropping that output block).  Stack   ***)
      (*** access map (256): 160 ST13/LD17/ST22/LD39 ; 192 ST15/LD21/ST54/LD77 ; 176 ST25/LD30/ST60/LD63 ; ***)
      (*** 208 ST68/LD71/ST74/LD134.  So keep all scratch facts live through the loop body.               ***)
      MAP_EVERY(fun n -> ARM_STEPS_TAC AES_GCM_ENC_KERNEL_256_X4_SCALAR_IV_MEM_LATE_TAG_SCALAR_RK_EXEC [n] THEN
            RULE_ASSUM_TAC(CONV_RULE(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV)))
          (55--60) THEN
      MERGE_CTR128_TAC 176 "s60" THEN
      MAP_EVERY(fun n -> ARM_STEPS_TAC AES_GCM_ENC_KERNEL_256_X4_SCALAR_IV_MEM_LATE_TAG_SCALAR_RK_EXEC [n] THEN
            RULE_ASSUM_TAC(CONV_RULE(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV)))
          (61--68) THEN
      MERGE_CTR128_TAC 208 "s68" THEN
      MAP_EVERY(fun n -> ARM_STEPS_TAC AES_GCM_ENC_KERNEL_256_X4_SCALAR_IV_MEM_LATE_TAG_SCALAR_RK_EXEC [n] THEN
            RULE_ASSUM_TAC(CONV_RULE(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV)))
          (69--74) THEN
      MERGE_CTR128_TAC 208 "s74" THEN
      MAP_EVERY(fun n -> ARM_STEPS_TAC AES_GCM_ENC_KERNEL_256_X4_SCALAR_IV_MEM_LATE_TAG_SCALAR_RK_EXEC [n] THEN
            RULE_ASSUM_TAC(CONV_RULE(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV)))
          (75--209) THEN
      ...
      (*** The scalar_rk final round: the block ciphertext (pre-byteswap) is        ***)
      (*** word_xor (word_join <input_hi ^ key10_hi> <input_lo ^ key10_lo>) (Q0),   ***)
      (*** built from the scalar X22/X23 XORs.  SCALAR_RK_RECONSTRUCT turns that     ***)
      (*** into the "word_xor <9round> (rk10 xor inblock)" form the standard AES     ***)
  ...
  ENSURES_WHILE_UP_TAC `loop_remain:num` `pc + 0x40c` `pc + 0x4f8`
    `\i s.
   ...
   [ENSURES_INIT_TAC "s0" THEN
    ...
    ASSUME_TAC THENL
     [REWRITE_TAC[ARITH_RULE `64 * a + 16 * b = 16 * (4 * a + b)`] THEN
      ...
      ALL_TAC] THEN
    (*** Split the tail block's input read into two address-normalised 64-bit    ***)
    (*** halves for the POST-INDEXED scalar "ldp x22,x23,[x0],#16" load (uses the  ***)
    (*** tail-specific conversion; see SPLIT_INPUT_TAIL_CONV).                     ***)
    ...
    (*** ("stp ...,[sp,#160]" at step 4, "ldr q0,[sp,#160]"); merge the two       ***)
    (*** 64-bit stores, discard the slot, then the scalar final round round-trips ***)
    (*** the ciphertext through the same slot ("stp x22,x23,[sp,#160]" at step 14,***)
    (*** "ldr q29,[sp,#160]") -> Q29.  AES-256 tail = 59 steps (128 was 51).      ***)
    MAP_EVERY(fun n -> ARM_STEPS_TAC AES_GCM_ENC_KERNEL_256_X4_SCALAR_IV_MEM_LATE_TAG_SCALAR_RK_EXEC [n] THEN
      RULE_ASSUM_TAC(CONV_RULE(TOP_DEPTH_CONV WORD_SIMPLE_SUBWORD_CONV)))
     (1--4) THEN
...
(* ------------------------------------------------------------------------- *)
(* Split the compound 15-round-key precondition                              *)
(*   wordlist_from_memory(key_p,15) s = MAP (word_reversefields 8) rk         *)
(* into LENGTH rk = 15 together with 15 individual per-slot reads             *)
(*   read (memory :> bytes128 (word_add key_p (word (16*k)))) s               *)
(*        = word_reversefields 8 (EL k rk).                                   *)
(* The subroutine wrapper needs the per-read form: ARM_ADD_RETURN_STACK_TAC's *)
(* interior big-step propagates only atomic `read c s = v` facts through the  *)
(* prologue; a single list-equation (what WORDLIST_FROM_MEMORY_CONV yields)   *)
(* is left as an orphan and the round-key reads never reach the core theorem. *)
(* ------------------------------------------------------------------------- *)
...
let AES_GCM_ENC_KERNEL_256_X4_SCALAR_IV_MEM_LATE_TAG_SCALAR_RK_SUBROUTINE_CORRECT = prove
 (`!in_p out_p len_bits tag_p ivec_p key_p htable_p tag0 nonce rk inblock
  ...
  REWRITE_TAC[fst AES_GCM_ENC_KERNEL_256_X4_SCALAR_IV_MEM_LATE_TAG_SCALAR_RK_EXEC;
              htable_mem_4; KEY15_SPLIT] THEN
  ARM_ADD_RETURN_STACK_TAC
    ~pre_post_nsteps:(11, 11)
    AES_GCM_ENC_KERNEL_256_X4_SCALAR_IV_MEM_LATE_TAG_SCALAR_RK_EXEC
    (REWRITE_RULE[KEY15_SPLIT]
       (REWRITE_RULE[fst AES_GCM_ENC_KERNEL_256_X4_SCALAR_IV_MEM_LATE_TAG_SCALAR_RK_EXEC; htable_mem_4]
          AES_GCM_ENC_KERNEL_256_X4_SCALAR_IV_MEM_LATE_TAG_SCALAR_RK_CORRECT))
