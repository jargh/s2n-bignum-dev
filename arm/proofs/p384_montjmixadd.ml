...
let P384_MONTJMIXADD_SAFE = time prove
 (`exists f_events.
       forall e p3 p1 p2 pc stackpointer.
           aligned 16 stackpointer /\
           ALL (nonoverlapping (stackpointer,288))
           [word pc,LENGTH p384_montjmixadd_mc; p1,144; p2,96; p3,144] /\
           nonoverlapping (p3,144) (word pc,LENGTH p384_montjmixadd_mc)
           ==> ensures arm
               (\s.
                    aligned_bytes_loaded s (word pc) p384_montjmixadd_mc /\
                    read PC s = word (pc + 0x14) /\
                    read SP s = stackpointer /\
                    C_ARGUMENTS [p3; p1; p2] s /\
                    read events s = e)
               (\s.
                    read PC s = word (pc + 0x3f3c) /\
                    (exists e2.
                         read events s = APPEND e2 e /\
                         e2 = f_events p1 p2 p3 pc stackpointer /\
                         memaccess_inbounds e2
                         [p1,144; p2,96; p3,144; stackpointer,288]
                         [p3,144; stackpointer,288]))
               (MAYCHANGE
                [PC; X0; X1; X2; X3; X4; X5; X6; X7; X8; X9; X10; X11; X12; X13; X14; X15; X16; X17; X19; X20; X21; X22; X23; X24; X25; X26] ,,
                MAYCHANGE MODIFIABLE_SIMD_REGS ,,
                MAYCHANGE SOME_FLAGS ,,
                MAYCHANGE [events] ,,
                MAYCHANGE
                [memory :> bytes (p3,144); memory :> bytes (stackpointer,288)])`,
        ...
        ==> ensures arm
             ...
             (\s. read PC s = returnaddress /\
                  (exists e2.
                         ...
                         memaccess_inbounds e2
                         [p1,144; p2,96; p3,144;
                          word_sub stackpointer (word 352),352]
                         [p3,144;
                          word_sub stackpointer (word 352),352]))
          (MAYCHANGE_REGS_AND_FLAGS_PERMITTED_BY_ABI ,,
           MAYCHANGE [memory :> bytes(p3,144);
                      memory :> bytes(word_sub stackpointer (word 352),352)])`,
  REWRITE_TAC[fst P384_MONTJMIXADD_EXEC] THEN
  ARM_ADD_RETURN_STACK_TAC P384_MONTJMIXADD_EXEC
    (REWRITE_RULE[fst P384_MONTJMIXADD_EXEC]P384_MONTJMIXADD_SAFE)
